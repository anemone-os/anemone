...
兼容、Rust 约束和工程闭环之间走出一条可维护的系统设计路线。
...
**核心论点：** Anemone 的外部约束是 Linux ABI compatibility surface，内部
组织原则是 native object model、owner boundary 和 Rust as a design
constraint；全书后续章节都是这张地图的展开。

**覆盖范围：**

- 目标与非目标。
- Linux ABI surface vs Anemone native internal contract。
...
- 一个外部 failure 如何不直接决定设计，而是进入 canonical 文档和验证闭环。
...
**核心论点：** Anemone 在 Linux-visible syscall surface 上追求兼容，但通过
syscall adapter 把 UAPI、flag、errno 和结构体解析限制在边界层，避免 Linux
兼容细节污染内部 owner。

**覆盖范围：**

- syscall dispatch。
- Linux UAPI 与内部 API 的分层。
- errno、flag、struct compatibility。
- stage-aware compatibility。
- silent compatibility 的日志、注释和退出条件边界。
- native internal contract。
- 链接段注册表分发机制。
- Rust 过程宏如何收束 syscall metadata、分发表和边界样板。

**代表路径 / case study：**

- `openat`：flag parsing、file status、VFS 边界。
- `ioctl`：Linux 控制面如何路由到真实 owner。
- syscall macro / link-section registry：分散定义如何生成统一分发入口。
...
**核心论点：** Anemone 用 task、thread group、process / session / process
group 和 trap / user context 表达执行实体；这些对象提供身份、生命周期和
用户态边界，但不直接拥有调度策略或等待协议。
...
- `Task、ThreadGroup 和 Process Group 分别表达执行实体、共享语义和拓扑关系`。
- `Task 提供身份和上下文，scheduler 才拥有 runnable state`。
...
- `wait-core 统一阻塞协议，event source 只发布唤醒能力`。
...
- `page cache 是文件 backing 与地址空间 fault 之间的共享层`。
...
- arch-specific context 与 generic task / scheduler / MM 的接口。
...
- arch trait / machine descriptor 的短接口形状。
- trapframe layout assertion 或 offset guard 的短例子。

**边界提示：**

- 说明 Linux ARM 早期 machine description 是启发来源，不是长期工业终局。
- 未来更完整 DTB / sysfs / device topology 可放 `Beyond Anemone`。

### 9. 工程闭环：从外部信号到设计收敛

**核心论点：** Anemone 的可靠性不只来自代码实现，也来自 RFC、transaction
devlog、register、验证 floor 和 agentic coding 约束共同形成的反馈闭环。

**覆盖范围：**

- RFC / implementation gate / transaction devlog。
- register / current limitations。
- verification sources：测试、真实负载、source audit。
- 如何区分 bug、limitation、环境缺口。
- agentic coding 如何被 write set、review、验证和 canonical docs 约束。
- 不讲分数，不列测试矩阵。

**代表路径 / case study：**

- 一个外部 failure 如何进入 register / RFC，再通过 implementation 和 validation
  收口。
- mount tree、sched wait、fanotify 等完整闭环可作为候选。

**图候选：**

- `外部信号只有进入 canonical 文档层，才会改变 Anemone 的设计事实`。
- `RFC 和 transaction devlog 把 agentic coding 限制在可审查边界内`。

**代码片段候选：** 无。以文档流和验证路径为主。

**边界提示：**

- 本章只讲工程闭环；AI 使用披露和方法细节放附录。
- 不把验证章节写成比赛测例叙事。
...
### 附录 C：AI / Agentic Coding 使用说明

**核心论点：** AI 是受约束的工程参与者，不是设计责任主体；所有设计事实仍
回到 canonical docs、代码和验证。
...
| verification / RFC / devlog / register / limitations | 第 9 章 | 正文重点 | 工程闭环，不写测例分数。 |
| agentic coding governance | 第 9 章 / 附录 C | 分层处理 | 第 9 章讲闭环中的角色，附录讲使用披露和方法细节。 |
...
- 第 9 章 case study 选择 mount tree、sched wait、fanotify 或其他闭环更完整的线。
