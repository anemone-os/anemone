...
也不是 Linux 的 Rust 复刻；它是一个有清晰 owner boundary、ABI 策略、
验证闭环和工程纪律的现代 OS kernel。测试和真实负载是反馈源与证据来源，
不是叙事主线。
...
- 工程闭环如何把外部反馈写回 RFC、devlog、register 和 current limitations。
...
Linux ABI 与内部结构的分层、代表性路径或工程闭环时，才进入正文。小功能
可以作为例子，但不作为功能清单。
...
FileOps/ioctl、file-backed mmap fault、trap entry/return，或 RFC 到 limitation
的工程闭环。

图必须有论点。如果一张图的标题不能写成明确结论，而只能写成“某某结构图 /
...
保留一个中性的“验证与工程闭环”章节，但不以比赛为叙事前提。

推荐写法：Anemone 的开发过程持续使用多层测试与真实负载作为反馈源，包括
基础用户态测试、POSIX/Linux 兼容性测试、系统调用与文件系统压力路径，
以及面向实际用户态程序的运行验证。测试结果不直接决定设计取舍；它们提供
可复现的外部信号，推动问题进入 RFC、devlog、register 或 current limitations。
...
`Beyond Anemone` 可以作为少数章节的可选收束栏目，用来讨论 Anemone 当前
设计之外的行业参照、未来演进或更激进的系统设计。它不是 backlog，不承诺
实现，也不替代 current limitations；只有当它能帮助读者理解当前 trade-off
的上限或另一种设计空间时才使用。

## AI / Agentic Coding

AI / agentic coding 放在附录或后置工程方法章节，篇幅中等，不抢占内核设计主线。

重点不是“使用了 AI”，而是 RFC、transaction devlog、register、review 分级、
write set、验证 floor 如何把 agent 输出纳入工程闭环。agent 不能替代设计责任；
