...
Anemone 也保留自己的 native syscall 区间。`anemone-abi` 中，Linux syscall number 空间之后的 `SYS_ANEMONE_START` 用来放置 Anemone-specific syscall；当前已经有 `SYS_DBG_PRINT` 和 `SYS_POWER_SHUTDOWN` 这类只对 Anemone runtime 有意义的控制面。它们表达 Anemone 自己愿意稳定暴露的 runtime/control surface，而不是 Linux compatibility surface 的替代实现。
...
raw argument 不直接进入内部对象。`#[syscall]` 生成的 wrapper 会逐个调用 `parse_syscall_arg()`：普通参数走 `TryFromSyscallArg`，特殊参数可以用 `#[validate_with(...)]` 指定边界验证函数。解析失败会记录 task、sysno、handler、argument index 和 raw value 等边界诊断信息；目的不是展示日志，而是让 ABI 边界上的错误仍然可定位。
...
#boundary[
  syscall 参数是用户态输入，不是内部 API。越过 adapter 后，内部对象看到的是 fd、flag、mode、用户指针和访问能力的 typed 形状，而不是 raw register 的编码细节。
]
...
#tradeoff[
  stage-aware compatibility 允许 Anemone 接受部分 Linux 历史噪声，同时不把每个 flag 都实现成完整 Linux 内核语义。它的可审查性来自清楚的边界：用户可见行为在 adapter 附近确定，兼容位不越过 adapter 成为内部对象语义，诊断信息也指向同一个边界层。这样 silent compatibility 是受控 ABI 策略，而不是把差异藏进各 subsystem。
]
...
`ioctl` 因此横跨两层 contract：syscall boundary 负责让 Linux 控制面穿过 fd/VFS 层时保持窄接口，driver owner 负责解释私有命令、保护自己的状态并返回 Linux 可见结果。

== TradeOff: ABI adapter 的局部性与边界厚度

这样的 syscall 层的价值在于局部性。Linux ABI 兼容的数字、flag、errno、用户指针和历史角落集中在 adapter 附近；内部对象可以用更窄、更 Rust-friendly 的类型表达状态所有权。`Fd` 的 raw 值在边界层变成 `Fd` 类型，用户路径先通过 `c_readonly_path` 变成 `Box<str>`，文件访问模式变成 `OpenAccessMode`，`ioctl` 的 fd 权限变成 `IoctlFileAccess`。这些都不是形式主义，它们减少了跨层误用的机会。
...
Anemone 的 ABI boundary 不是完整 syscall 表或 errno matrix 的复刻。Linux-visible surface 是必须认真对待的外部契约；内部 owner boundary 决定了这个契约怎样被消化，而不是怎样被照搬。
