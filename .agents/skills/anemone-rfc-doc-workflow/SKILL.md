...
- Small change tracking issues are local to the record. Do not create standalone `tracking-issues.md`, `invariants.md`, or `implementation.md` under `changes/`. A contract-bearing small change is allowed only for one fully resolved atomic implementation cutover with unique owners, handoff, failure, cleanup, bounded write set, and bounded validation; otherwise repository-level accepted targets/contracts, non-trivial invariants, staged gates, multi-agent/checkpoint orchestration, or repeated document-layer review require the RFC workflow.
...
- A contract-bearing small change keeps its local target and cutover evidence in one record while effective text exists only under `docs/src/contracts/`. It must use one atomic code-and-contract checkpoint and retain the old contract on failure. If it needs a probe, multiple checkpoints, transitional contract, rolling stage resolution, target renegotiation, unresolved owner, or an Apollyon/Keter that cannot close in the same iteration, upgrade to RFC instead of expanding `changes/` into a small RFC.
...
1. Identify scope and artifacts.
   ...
   - Classify the work first:
     ...
     - Effective rule reused or changed across RFCs/modules: current contract plus RFC `Contract Impact` and a cutover gate, except for a fully resolved single-cutover contract-bearing small change.
...
2. Handle small change records.
   ...
   - For a qualified contract-bearing small change, add `Contract Impact / Cutover` with the effective baseline, stable IDs, change classification, new rule summary, one atomic cutover, failure boundary, and evidence. Keep effective text only in the linked current contract.
