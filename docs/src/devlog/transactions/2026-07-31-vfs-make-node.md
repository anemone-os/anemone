...
owner、全部 call-site cutover 与跨创建路径 proof 退出条件的no-umask limitation。后续
[umask小迭代](../changes/2026-07-27-umask-file-creation-mask.md)建立共享owner与未来creator复用义务；本次合流的
`mknodat` source integration完成该consumer接线后才退出限制。Stage 1 public
...
strict failure/crash atomicity仍是Active accepted limitations。前者随后由独立umask小迭代建立owner与复用
义务，并在本次合流完成`mknodat` source integration后退出。`VFS-MAKE-NODE-CUTOVER`因此原子Introduce
