...
`openat` 在pathname lookup、creation与`O_TRUNC`之前取得reservation；`EMFILE`必须先于
filesystem副作用返回。后续lookup、permission、backend open或publication失败时，由同一
reservation的Drop路径回滚slot，不能留下已创建路径或已截断文件。
...
- `openat` 将reservation移动到pathname/VFS operation之前，避免动态soft limit耗尽后先创建
  path或截断已有文件、再返回`EMFILE`。
...
- alpha/omega合流候选的owner-local KUnit在RV64运行334项、LA64运行339项，均打印
...
- RV64 `rlimit-test` 十一项全部通过并打印 `RLIMITTEST:END:all cases passed`：readback/
  ...
  errno、`CLONE_FILES` policy independence、exec preservation，以及`EMFILE`先于
  `O_CREAT`/`O_TRUNC`副作用。RV64随后完成orderly shutdown与SBI poweroff。
- LA64 运行同一十一项 matrix并全部通过，且应用在编译期不包含 legacy `getrlimit`；common
  `prlimit64` read/update/enforcement 路径通过。guest 完成 filesystem/network/device shutdown 后因当前
...
- `just fmt all --check`、`just test xtask`（65/65）、`git diff --check`与
