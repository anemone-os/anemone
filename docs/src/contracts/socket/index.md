...
**覆盖范围：** common Socket file/ABI/wait boundary、IPv4 UDP与Unix stream的共同dispatch，以及filesystem pathname Unix stream的state、namespace、address、stream与lifecycle
**不覆盖：** TCP、IPv6、Unix datagram/seqpacket/abstract namespace、ancillary data、mutable socket option、`SO_ERROR`/pending error、timeout或async I/O
**最后核验：** 2026-08-02

本目录登记`SOCKET-UNIX-CUTOVER`已经生效的最小共享规则。它不建立名为Socket core的并列runtime owner：general front只拥有共同外壳和operation orchestration，UDP与Unix仍分别拥有family-private事实。
...
- [UDP Socket](../net/udp-socket.md)：UDP Endpoint、datagram transaction与protocol readiness继续由network owner负责。
