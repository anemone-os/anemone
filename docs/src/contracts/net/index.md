...
**覆盖范围：** boot-time netdev publication、initial-domain logical membership、static IPv4 control plane、UDP与ICMP raw Socket/Endpoint protocol、production local/external handoff、frame ownership/progress、bounded global-Stack pump、attach与terminal shutdown handoff
**不覆盖：** runtime IP reconfiguration、runtime hotplug/detach/restart、完整teardown、connected/IPv6 UDP、TCP、任意raw protocol、hardware或SMP runtime guarantee
**最后核验：** 2026-08-03
...
- [IOMUX poll wait](../iomux/poll-wait.md)与[Epoll protocol](../epoll/protocol.md)：wait registration、cancellation与
  final readiness harvest；UDP与ICMP raw source只提供各自当前predicate与recheck hint。
