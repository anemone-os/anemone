...
**覆盖范围：** boot-time netdev publication、initial-domain logical membership、static IPv4 control plane、UDP Socket/Endpoint transaction与wait、production local/external handoff、frame ownership/progress、bounded global-Stack pump、attach与terminal shutdown handoff
**不覆盖：** runtime IP reconfiguration、runtime hotplug/detach/restart、完整teardown、connected/IPv6 UDP、TCP/raw socket、hardware或SMP runtime guarantee
**最后核验：** 2026-07-31
...
- [UDP Socket](./udp-socket.md)：`NET-PROTOCOL-BOUNDARY-001`、`NET-SOCKET-ENDPOINT-001`、
  `NET-UDP-TRANSACTION-001`与`NET-SOCKET-WAIT-001`。
...
- [IOMUX poll wait](../iomux/poll-wait.md)与[Epoll protocol](../epoll/protocol.md)：wait registration、cancellation与
  final readiness harvest；UDP source只提供当前predicate与recheck hint。
