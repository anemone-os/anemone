...
**覆盖范围：** boot-time netdev publication、initial-domain logical membership、frame ownership/progress、bounded global-Stack pump、attach与terminal shutdown handoff
...
本目录登记`net-frame-path` R1和`net-udp` R0 Stage 1已经cut over的最小共享规则，不声称枚举network领域全部不变量。
