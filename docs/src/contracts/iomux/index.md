...
**覆盖范围：** `ppoll` / `pselect6` 的单轮 readiness scan、source registration、阻塞与 final recheck
**不覆盖：** Linux `pollfd` / `fd_set` ABI、具体 source predicate、scheduler wait-core completion、epoll interest policy
**最后核验：** 2026-07-26
...
- [Poll wait 与 source registration](./poll-wait.md)：单轮 scan/register/final-recheck、typed register gate 与 source wake publication。
