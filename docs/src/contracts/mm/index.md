...
**Owner：** mm paging commit / user-address-space TLB completion / kernel heap / frame-allocation accounting / OOM policy
**覆盖范围：** 本轮按触达提取的user-address-space local/remote TLB completion、kernel heap layout/slab/backing handoff、global frame-pressure sampling、OOM victim policy与既有signal/exit handoff
