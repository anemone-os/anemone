...
`NEMOPHILA-WEAVE-001`、`NEMOPHILA-CLONE-001`、`NEMOPHILA-ARTIFACT-001`
**状态：** Active
...
Core Wasm interpreter / task clone / printk / procfs
**覆盖范围：** R0 trusted-good-module 的 artifact acquisition、transactional admission、published instance lifecycle、
typed weave、clone observer、value-only logging、management ABI 与只读诊断投影
**不覆盖：** untrusted module execution、执行配额或进度保证、force/automatic unload、source replacement/priority、
path-based load ABI、并发 writer 原子 snapshot、第二个 point/service、generic procfs dentry revocation或dynamic inode
materialization
**实现位置：** `anemone-abi/src/nemophila.rs`、`anemone-kernel/src/nemophila/`、
`anemone-kernel/src/fs/proc/nemophila/`、`anemone-kernel/src/task/clone/`、
...
**最后核验：** 2026-08-15；`NEMOPHILA-R0-CUTOVER` Effective
...
**规则：** R0 Host import分为provider-owned typed extension registration与kernel-owned service submission。当前唯一service是
value-only logging：Host验证level、guest memory range与UTF-8后，把borrowed value交给现有kernel logging owner；filter、record
bound、truncation、ring和presentation仍由logging决定。log submission不返回resource handle，不建立跨调用borrow，不决定module
publication、callback结果、poison或unload；已经提交的load diagnostic不因transaction rollback而撤销。
...
**违反表现：** logging Host保存guest pointer、返回kernel object、绕过现有logging policy、以日志成功/失败驱动runtime状态，或把
provider registration当作generic service/resource registry。

**验证 / Enforcement：** logging lowering/invalid-input KUnit、真实module load/callback logging与source audit。
...
**验证 / Enforcement：** provider/catalog/registration/cohort/concurrency/poison/retirement KUnit与canonical clone module guest证据。
