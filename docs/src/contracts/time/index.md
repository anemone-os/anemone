...
**覆盖范围：** 统一硬件计数、Hertz、clock 读值投影、realtime step，以及一次 soft timer 排队请求和取消协议
**不覆盖：** POSIX timer、RTC 接入、CPU-time timer 和 CPU usage 累计生命周期
**最后核验：** 2026-08-04

本目录只登记已经 cut over 的共享时间规则，不声称 RFC-20260803 的 POSIX timer 目标已经实现。
