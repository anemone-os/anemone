...
**Owner：** device RTC core、architecture clock source、common timekeeper、soft timer request service 与 ThreadGroup POSIX timer
**覆盖范围：** RTC boot seed、统一硬件计数、Hertz、clock 读值投影、realtime step、一次 soft timer 请求与 POSIX timer 生命周期
**不覆盖：** runtime RTC service/writeback、CPU-time timer 和 CPU usage 累计生命周期
**最后核验：** 2026-08-08

本目录只登记已经 cut over 的共享时间规则。RTC只通过boot-only provider handoff建立一次calendar anchor；运行期
RTC service仍不属于当前time contract。

## Owner Boundary

- architecture clock source 独占 raw counter 的平台校正和稳定 Hertz；common timekeeper 只消费一个跨 CPU
  不后退的计数域。
- device RTC core独占boot provider registry、selection、sealing与一次read；machine只提供optional preference，
  timekeeper不保存provider handle。
...
- [Clock derivation](./clock-derivation.md)：`TIMEKEEPER-CLOCK-001` 定义统一计数、整数换算、clock projection
  和分辨率。
- [RTC boot seed](./rtc-boot-seed.md)：`RTC-BOOT-SEED-001` 定义provider registration/finalization、machine
  preference、确定性选择、一次read与timekeeper handoff。
