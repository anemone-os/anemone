...
**Owner：** VFS inode kind、filesystem-type identity、legacy mount admission protocol 与 local advisory lock protocols
**覆盖范围：** 本轮按触达提取的 inode file-kind truth、Linux mode projection、canonical filesystem identity、mount source-kind admission、syscall-only fstype alias containment、inode-associated local flock与POSIX record lock
