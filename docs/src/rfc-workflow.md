...
- `Contract Impact`：按稳定 ID 声明 `Preserve`、`Refine`、`Replace`、`Remove` 或 `Scoped Exception`，并写明生效 gate；未改变的规则只链接，不复制正文。
...
若 RFC 影响共享 contract，文档层 review 还必须确认：影响表覆盖所有直接受影响 ID，提取范围已经形成最小闭包，owner 与跨域局部义务唯一，当前 effective 与 accepted target 没有混写，且每个 `Refine` / `Replace` / `Remove` 都有明确 cutover gate。未登记的相邻领域不因为本轮顺手整理而进入 write set。
...
事务日志必须注明所实现的 RFC 修订，以及本事务计划 Preserve / Refine / Replace / Remove / Scoped Exception 的 contract IDs 与 cutover gate。`R0` 的首次实现使用初始事务；Closed RFC 的 `R1` 及后续语义修订如果需要代码工作，应建立新的、可独立收口的事务。单纯补充执行证据或更正事务事实不产生 RFC 修订。
...
| Transaction devlog | 执行事实、checkpoint、review、验证、contract cutover、handoff | 重新定义 target 或 effective contract |
