...
- [RFC-20260801-socket-abstraction-and-unix-socket](./rfcs/socket-abstraction-and-unix-socket/index.md)：R1 Accepted；以现有
  ...
  6.6.32 conformance与实现期validation surface，不再构成R0 Review Hold；多阶段顺序、当前gate与停止边界由
  [实施路线](./rfcs/socket-abstraction-and-unix-socket/implementation.md)唯一记录。Stage 3与Checkpoint 3A/3B已关闭，
  Stage 4未激活；RFC在最终`SOCKET-UNIX-CUTOVER`前保持Not Cut Over，pending current-contract delta均未生效。
