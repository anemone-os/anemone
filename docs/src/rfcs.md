...
- [RFC-20260722-system-target-model](./rfcs/system-target-model/index.md)：Draft / Public Review；重分配Platform、SystemTarget、KernelConfig、BuildPreset、resolved build与QEMU invocation的唯一owner，保持Justfile/xtask单一编排面，并提议将当前[`BOOT-PROTOCOL-001`](./contracts/task/boot-protocol.md) Refine为typed `RootfsEntry | EmbeddedApp` ordinary VFS exec。Promotion preflight、current baseline提取与初始Implementation Resolution Gate已完成，Stage 1为Ready；RFC acceptance、transaction与实现授权均未发生。
