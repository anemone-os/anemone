...
- [RFC-20260713-sched-fair-stride](./rfcs/sched-fair-stride/index.md)：已提升为公开 RFC 草案，尚未开始实现；定义稳定 `Fair` class identity、经典 fixed-tick Stride、Linux nice weight、最小 pass heap、placement floor、yield handoff、compile-time default selector 和用户侧 RT/RR A/B 验收边界。公开 review 接受后再建立 transaction devlog。
