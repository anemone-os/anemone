...
- [RFC-20260720-unix-jobctl](./rfcs/unix-jobctl/index.md)：R1 Accepted for Implementation / Not effective；定义协作式 Unix job control 的 ThreadGroup-owned stop / continue phase、user-entry barrier、stopped / continued child report、Signal control ordering、procfs projection 与 integrated `UJ-CUTOVER`，并保留current reservation-first顺序造成的窄SIGKILL递送延迟。Stage 0 split-only、Stage 1 dormant foundation、Stage 2 integrated production vertical slice、Stage 3A conditional control closure与Stage 3B multi-member lifecycle / topology closure已关闭；candidate保持non-publishable，current contract未改变，Stage 4 Not Started。
