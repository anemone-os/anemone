...
- [RFC-20260722-tty-subsystem](./rfcs/tty-subsystem/index.md)：R1 已接受并由现有 [transaction](./devlog/transactions/2026-07-23-tty-subsystem.md)继续实施；提议建立 serial TTY 的专属 Terminal/FileOps、稳定 `/dev/ttyS<N>` 与 `/dev/tty`、termios/data-plane、controlling relation 和 BusyBox ash/vi 所需的 foreground job-control 包络。R1保持R0功能目标，只把本RFC的build/runtime acceptance收窄为RV64，LA64明确Not Run。Stage 0至Stage 3已关闭，[TTY data-plane contract](./contracts/tty/data-plane.md)中的五个ID为Active；relation/job-control四个ID与`TTY-JOBCTL-CUTOVER`仍Not Cut Over，Stage 4已解析为一个不拆checkpoint的Ready最终stage，尚未授权或激活。
