...
- [RFC-20260714-sched-dynamic-attributes](./rfcs/sched-dynamic-attributes/index.md)：R0 已接受进入实现；以 scheduler-owned config patch 与固定 owner-CPU `RunQueue` transaction 统一动态 nice、Fair/RT policy、RT priority、reset-on-fork 与 fixed-CPU affinity，并以 async IPI、persistent-phase one-shot completion 和临时全局 remote submission gate保持同步 syscall 语义。阶段 0 source audit 未命中停止条件，后续证据见 [R0 事务](./devlog/transactions/2026-07-15-sched-dynamic-attributes.md)。
