...
- [RFC-20260722-tty-subsystem](./rfcs/tty-subsystem/index.md)：公开 Draft；提议建立 serial TTY 的专属 Terminal/FileOps、稳定 `/dev/ttyS<N>` 与 `/dev/tty`、termios/data-plane、controlling relation 和 BusyBox ash/vi 所需的 foreground job-control 包络。当前没有 R0、transaction 或 contract cutover；Stage 0 Ready 仍需 R0 接受与单独授权。
