...
- [RFC-20260722-system-target-model](./rfcs/system-target-model/index.md)：R0 / Accepted for Implementation；重分配Platform、SystemTarget、KernelConfig、BuildPreset、resolved build与QEMU invocation的唯一owner，保持Justfile/xtask单一编排面，并计划在Stage 5将当前[`BOOT-PROTOCOL-001`](./contracts/task/boot-protocol.md) Refine为typed `RootfsEntry | EmbeddedApp` ordinary VFS exec。Stage 1的Checkpoint 1A-1D与Stage 2 Checkpoint 2A-2C已关闭；2D保持Ready / Not Activated，effective Boot Protocol仍未cut over。
