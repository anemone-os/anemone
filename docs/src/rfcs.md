...
- [RFC-20260808-jh7110-gmac](./rfcs/jh7110-gmac/index.md)：Accepted / R4；为 VisionFive 2 上任意有限数量的
  matching JH7110 GMAC 节点定义 per-node one-time driver、命名 `macirq`、coherent DMA、成功 publication
  candidate 在 attach admission 时连续消费的 `eth<N>` 和现有单接口 static IPv4 接入。四个实现 Gate 各自执行检查，但全部
