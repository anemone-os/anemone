...
- [RFC-20260722-tty-subsystem](./rfcs/tty-subsystem/index.md)：R0 已接受并建立 [transaction](./devlog/transactions/2026-07-23-tty-subsystem.md)；提议建立 serial TTY 的专属 Terminal/FileOps、稳定 `/dev/ttyS<N>` 与 `/dev/tty`、termios/data-plane、controlling relation 和 BusyBox ash/vi 所需的 foreground job-control 包络。Stage 0 source/config/oracle route审计已关闭，deferred carrier选用现有`KThreadHandle::wake()`并把其底层IRQ-off allocation债务留给wait-core/scheduler owner；Stage 1已解析为Ready / Not Started，仍需单独实现授权，全部`TTY-*`均未cut over。
