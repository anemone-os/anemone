...
- [RFC-20260722-system-target-model](./rfcs/system-target-model/index.md)：R2 / Accepted for Implementation；重分配Platform、SystemTarget、KernelConfig、BuildPreset、resolved build与QEMU invocation的唯一owner，保持Justfile/xtask单一编排面，并计划在Stage 5将当前[`BOOT-PROTOCOL-001`](./contracts/task/boot-protocol.md) Refine为typed `RootfsEntry | EmbeddedApp` ordinary VFS exec。Stage 1-4与Checkpoint 1A-1D、2A-2D、3A、4A已关闭；Stage 4增加closed Source app driver并复用公共artifact export，Stage 5保持Outline / Not Resolved，effective Boot Protocol仍未cut over。
