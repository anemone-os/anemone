...
- [RFC-20260629-vfs-direct-user-io](./rfcs/vfs-direct-user-io/index.md)：已接受进入实现准备；定义普通文件 `read` / `readv` / `pread*` 与后续 `write` / `writev` / `pwrite*` 的 direct userspace copy 边界、VFS-owned user-buffer cursor、fanotify transaction adapter，以及 read/write vertical slice gates。
