...
- [RFC-20260714-sched-dynamic-attributes](./rfcs/sched-dynamic-attributes/index.md)：R1 已接受进入实现；以 scheduler-owned config patch 与固定 owner-CPU `RunQueue` transaction 统一动态 nice、Fair/RT policy、RT priority、reset-on-fork 与 fixed-CPU affinity，并以 async IPI、persistent-phase one-shot completion 和临时全局 remote submission gate保持同步 syscall 语义。R1进一步把scheduler request收窄为single-owner `Box`，移除通用payload clone，并把broadcast误用固定为发送副作用前panic。阶段证据见 [当前事务](./devlog/transactions/2026-07-15-sched-dynamic-attributes.md)。
