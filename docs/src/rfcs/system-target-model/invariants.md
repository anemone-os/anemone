...
本文定义 R3 accepted target invariants 与 RFC-local proof obligations。它不描述
当前已经实现的 build behavior，也不构成 `docs/src/contracts/` 的 effective contract。

## Contract Impact

R3不新增长期contract delta。R0-R2只有一项明确的长期contract delta：Anemone Boot Protocol。构建配置
...
preset或low-level target tuple补齐platform，也不得把该例外扩散到
普通`qemu`execution。
...
`qemu dt refresh --platform ... --check`必须证明它直接选择QEMU platform、不读取system
...
- public promotion、R3 acceptance、R0-R2 transaction与`BOOT-PROTOCOL-001` Refine target cutover
  均已完成；R3 implementation closure与证据见current transaction。
