...
本文定义 public RFC Draft 的 target invariants 与 RFC-local proof obligations。它不描述
当前已经实现的 build behavior，也不构成 `docs/src/contracts/` 的 effective contract。

## Contract Impact

本 public Draft 只有一项明确的长期 contract delta：Anemone Boot Protocol。构建配置
...
effective baseline。该文档提取不改变运行行为；本 Draft 不提前执行 Refine target cutover。
...
**规则：** 每个supported Platform必须提交可review DTS，并显式声明`firmware`或
`embedded` delivery。Manifest、DTS和runtime FDT之间每项machine fact只能有一个规范
owner；其它表示只能是派生结果或带provenance的conformance snapshot。

- embedded：committed DTS为normative source，普通build运行`dtc`并消费生成DTB；
- firmware：runtime FDT为启动输入，committed DTS为conformance baseline，必须声明允许
  差异、provider/firmware provenance和runtime validation owner；
