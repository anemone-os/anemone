...
本文定义 R0 accepted target invariants 与 RFC-local proof obligations。它不描述
当前已经实现的 build behavior，也不构成 `docs/src/contracts/` 的 effective contract。

## Contract Impact

R0 只有一项明确的长期 contract delta：Anemone Boot Protocol。构建配置
...
effective baseline。该文档提取不改变运行行为；R0 acceptance 不提前执行 Refine target cutover。
