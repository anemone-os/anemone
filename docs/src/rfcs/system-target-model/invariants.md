...
本文定义 R1 accepted target invariants 与 RFC-local proof obligations。它不描述
当前已经实现的 build behavior，也不构成 `docs/src/contracts/` 的 effective contract。

## Contract Impact

R1 只有一项明确的长期 contract delta：Anemone Boot Protocol。构建配置
...
effective baseline。该文档提取不改变运行行为；R1 acceptance 不提前执行 Refine target cutover。
...
- firmware：runtime FDT为启动输入；当前supported Platform使用provider-derived conformance baseline，
  必须声明允许差异、provider/firmware provenance和runtime validation owner；
...
Platform maintainer在板级/U-Boot更新时拥有runtime FDT对照验证；未被live Platform引用且与硬件导出结果
...
Physical firmware-derived baseline还必须由closed Platform字段记录capture provenance、允许的runtime
差异与validation owner；只写`provider = "firmware"`不能完成authority closure。
