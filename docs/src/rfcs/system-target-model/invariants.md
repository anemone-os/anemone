...
| --- | --- | --- | --- | --- |
| `BOOT-PROTOCOL-001` | Refine | [`BOOT-PROTOCOL-001`](../../contracts/task/boot-protocol.md#boot-protocol-001--typed-initial-program-source统一收口到普通-vfs-exec)：两种variant均可携带完整非空argv；省略时以resolved path作为唯一argv并走ordinary exec | 同左 | Checkpoint R6A（已生效） |
...
6. Normal build不读取runtime disk/rootfs/network input或runtime bind组；只有QEMU Platform使用embedded
   delivery时，build可以展开provider字段引用的bind，并从selected provider topology snapshot生成build-local DTB。
...
QEMU provider声明物化该machine contract所需的literal或`{{name}}`参数与受控bind template；embedded DTB delivery
...
两种source均可在SystemTarget中省略`argv`或提供完整非空string array。显式`argv`包含`argv[0]`，
kernel不得隐式插入executable path，也不得要求`argv[0]`等于该path；省略时才以resolved executable
path作为唯一参数。配置值逐项原样进入exec argv，不做shell splitting。Rootfs entry metadata
`/.anemone/init`仍只保存executable path，不承载argv或新的复合格式；envp保持kernel现有固定值。
...
**规则：** Build、app/rootfs和execution各自只解析本 action 所需输入。QEMU provider字段使用的bind
可同时属于build与execution；runtime bind参数组只属于execution，不得为了方便把完整runtime
environment升级为所有build的前置条件。

- build不读取或要求rootfs/test disk/network backend/runtime bind组，并生成Platform要求的kernel outputs；
  selected QEMU Platform使用embedded DTB delivery时，build展开provider topology字段所引用的bind并生成build-local DTB；
- rootfs materialization只读取recipe及其声明的app/file input；固定路径可以来自先前action；
- QEMU execution要求selected platform的QEMU config、本次command实际需要的bind map及所需host tool；
...
### STM-QEMU-BIND-001 - QEMU bind 是具名 opaque-string 替换

**规则：** Selected Platform的QEMU provider string、固定`args`与`[[qemu.bind]]` template只使用
`{{name}}`这一种placeholder语法，本次build/QEMU action只使用`--bind name=value`提供opaque string。
不提供匿名`{{}}`、`xxx = { bind = "xxx" }`、typed declaration、default、range、unit、path/file check、
source kind、product role、block slot或disk subtype。

替换对每个原始string执行一次且不递归；value中的placeholder形状保持普通字符。固定provider字段和
`qemu.args`中的placeholder都是required。`[[qemu.bind]]`只有唯一name、显式optional flag和argv token
array template；template只能引用自己的`{{name}}`并至少引用一次。Required组缺值失败；optional组缺值时
整个token array原子省略。展开顺序由config声明顺序决定，CLI顺序不得改变argv topology。每个token
独立传给`std::process::Command::arg()`，不能经过shell或按空格二次切分。

Build在任何side effect前只展开provider的machine、CPU、SMP、memory与optional BIOS，并拒绝未被这些
字段消费的输入。QEMU在launch前展开相同provider字段、固定args和runtime组，并拒绝未被完整command
消费的输入。两种action都拒绝unknown、duplicate、empty或missing-required value；binding layer不解释
value的artifact type、architecture、slot、format、QEMU keyval或先前action result。

Bind name只能说明QEMU argv空位与guest-visible attachment，不得编码SystemTarget
或workload role。调用者负责在阅读selected platform config后提供语义正确的value；该人工映射不
复制成另一份tracked role/slot schema。

所有runtime bind template必须保持DT-neutral。Build-time provider DT物化只展开provider字段，不展开
`qemu.args`或runtime group，不提供dummy file/disposable backend，也不根据template猜测设备类型。若新增或修改bind会改变canonical provider DT，
...
明确接受完全人工映射。普通action只验证declaration、placeholder和bind map的机械正确性，不证明
bind value满足SystemTarget root selection、architecture、format、QEMU语法或先前action result。
一个非空但语义错误的value可以通过bind展开，随后在QEMU、guest boot或wrapper验证中失败；这项边界不得被文档或diagnostics误述为resolver
已经完成runtime artifact compatibility证明。

**Owner：** Platform QEMU config拥有literal/placeholder/declaration/template；build或QEMU invocation拥有
本次value；xtask对应action拥有消费检查与字符串展开。

**违反表现：** 同一配置同时出现匿名、object-bind与具名placeholder；typed/range/path validator拒绝
本应opaque的value；optional组只省略部分token；tracked config保存worktree-local image；target/preset复制bind declaration或
...
QEMU provider materialization只消费resolved architecture，以及展开本次provider字段bind后的machine、CPU、
SMP、memory与optional BIOS；不读取ordinary `qemu.args`、rootfs、测试盘、network backend、runtime bind组或
另一份selection。
...
Build-time command在`-machine`中加入build-owned temporary `dumpdtb` path，并展开provider字段引用的bind，
复用provider的machine、CPU、SMP、memory与optional BIOS；不加入ordinary `qemu.args`、nographic/serial/debug
presentation，也不展开runtime `[[qemu.bind]]` argv组。Temporary path和最终DTB都由build action拥有，成功校验后才能原子
...
**违反表现：** 恢复独立DT maintenance CLI；build按Platform name二次解析配置；provider dump读取runtime
bind组或用户master image；失败后保留旧DTB；把QEMU版本/path写进tracked config；为保留旧refresh实现
继续提交QEMU DTS。
...
Stable action semantic固定为：`build`生成selected Platform要求的kernel artifacts且只展开provider字段
引用的bind，不读取rootfs/runtime backend；普通`qemu`从selected target/platform取得QEMU config并展开
provider、fixed args与runtime bind group。普通`qemu`不接受裸`--platform`、
特殊`--image`或raw`--qemu-arg`绕过selection与tracked QEMU template。

每个system action只解析一次selection并固定`ResolvedSystemBuild`。`build/qemu --bind name=value`只填充
selected Platform具名placeholder；任何未知、重复、缺失required、空或未消费value都在action side effect前失败。
`qemu --show-bindings`解析selection/platform后显示name、required/optional与runtime template并退出，
...
QEMU bind value保持action-scoped。

### QEMU invocation binding

QEMU action record可以按name记录本次bind value及selected placeholder/declaration identity，用于诊断和
...
4. 若action为build或普通QEMU execution，按各自消费范围展开selected platform placeholder并验证本次bind map；
...
- QEMU placeholder/declaration的name/template无效；
- build或普通QEMU execution存在未知、重复、缺失、空值或未消费的bind；
...
tool与机械bind错误；完全人工映射中内容选错但value机械有效的情况是已接受例外，
...
- 为build-time DT dump展开runtime bind组、提供dummy path/backend，或让runtime attachment参与provider topology；
...
- public promotion、R0-R5 acceptance/implementation、R4A QEMU provider DT cutover与R6A
  `BOOT-PROTOCOL-001` argv Refine均已完成；R5已将未激活的R4B配置应用移出RFC acceptance boundary。
