...
**状态：** Draft / Public Review（文档协议未闭合）
**修订：** Draft
**负责人：** doruche
**最后更新：** 2026-07-22
...
**下一步：** Review 本公开 Draft 与 implementation gate；尚未通过初始
Implementation Resolution Gate 把 Stage 1 解析为 Ready；RFC acceptance、transaction creation
与代码实现授权均未发生。
...
envelope、feedback gate、停止条件与回写路径。该计划没有 resolved manifest；transaction
devlog、公共导航、代码实现与 contract cutover 仍未开始。
...
- [迁移实施计划](./implementation.md)：rolling stage envelope、feedback/preflight gate、
  validation floor、停止条件与回写路径；当前不含 resolved manifest。
- 本文：范围、方案、接受边界、备选与风险。
...
`implementation.md` 只描述后续实施边界。Public promotion 已完成；仍需先通过初始
Implementation Resolution Gate，把 Stage 1 完整解析为 Ready 并冻结 resolved manifest；随后
RFC acceptance、transaction creation 与 Stage 1 activation 分别完成前，不授权修改构建或
kernel 代码。
...
[迁移实施计划](./implementation.md) 已记录允许带入实现的不确定性。Public promotion 已完成；首个
Implementation Resolution Gate、RFC acceptance、transaction creation 与 Stage 1 activation
继续按此顺序保持独立；本 Draft 本身不授权修改构建或 kernel 代码。若 feedback 需要改变 target
...
已经提升为 public RFC Draft，但尚未进入 acceptance 或实现阶段。当前 Draft 已删除独立 package/output graph，并把 U-Boot
...
Boot Protocol baseline；下一步完成初始 Implementation Resolution Gate。Stage 1 达到 Ready 并完成 RFC acceptance 前不创建
