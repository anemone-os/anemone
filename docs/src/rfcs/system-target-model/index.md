...
- [迁移实施计划](./implementation.md)：rolling stage envelope、feedback/preflight gate、
  validation floor、停止条件与回写路径；Stage 1-2历史定义及resolved manifest的
  唯一权威。
- 本文：范围、方案、接受边界、备选与风险。
...
用户接受为R2并完成correction，Stage 4仍未解析。若后续feedback需要改变target invariant、owner、ABI、
...
Outline / Not Resolved。Stage 3关闭后的反馈确认physical baseline provenance、允许差异与复核责任无法由
