...
**状态：** Accepted for Implementation（Stage 1-3 Closed；Stage 4 Ready / Checkpoint 4A Not Activated）
**修订：** R2
**负责人：** doruche
**最后更新：** 2026-07-23
...
**影响契约：** [`BOOT-PROTOCOL-001`](../../contracts/task/boot-protocol.md#boot-protocol-001--rootfs-metadata选择初始用户程序)（Refine；当前仍由 effective baseline 生效，R2 target 尚未 cut over）。
**开放问题：** None；已确认问题已折回 target 或分流到
[迁移实施计划](./implementation.md) 的 feedback/preflight gate。
**下一步：** Stage 3与唯一Checkpoint 3A已关闭；Stage 4已由独立Resolution Gate解析为单一Checkpoint 4A，
保持Ready并等待独立activation。

## 文档状态

本文是 R2 accepted target 的 canonical source；它不是 current contract 或 resolved write set。
...
Stage 4已解析为Ready但尚未激活，`BOOT-PROTOCOL-001` contract cutover仍未发生。
...
- [迁移实施计划](./implementation.md)：rolling stage envelope、feedback/preflight gate、
  validation floor、停止条件与回写路径；Stage 1-3历史定义、Stage 4 Ready definition及resolved
  manifest的唯一权威。
- 本文：范围、方案、接受边界、备选与风险。
...
R2 feedback correction独立关闭后，Stage 4已解析为单一Checkpoint 4A并保持Ready / Not Activated。
...
- `firmware`：runtime FDT由firmware/provider提供；当前supported Platform使用provider-derived
  conformance baseline。物理capture provenance、允许的runtime差异和复核责任是必须由人类
  review的Platform维护事实，保存在baseline相邻说明与RFC证据中，不建立无软件consumer的配置字段；
...
- [迁移实施计划](./implementation.md) 已通过初始 `Implementation Resolution Gate` 把 Stage 1
  ...
  已依次把Stage 2与Stage 3完整解析为Ready，2A-2D与3A随后分别独立关闭；Stage 4已解析为单一
  Checkpoint 4A并保持Ready，Stage 5及更远阶段保持Outline。
...
DT authority renegotiation已由用户接受为R1，Checkpoint 3A与Stage 3随后关闭；关闭后的metadata反馈已由
用户接受为R2并完成correction；后续独立gate已把Stage 4解析为Ready / Not Activated。若后续feedback需要
...
R2是当前accepted target。Stage 1已按用户授权完成，Checkpoint 1A-1D已依次独立关闭；独立的
...
transaction记录实际checkpoint、resolution与R1 renegotiation证据；Stage 3与Checkpoint 3A已关闭，Stage 4已由
独立gate解析为单一Checkpoint 4A并保持Ready / Not Activated。Stage 3关闭后的反馈确认physical baseline
provenance、允许差异与复核责任无法由
当前软件自动维护；R2删除没有action consumer的typed metadata，同时保留`provider = "firmware"`的
fail-closed行为和人类review义务。
