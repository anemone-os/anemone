...
**影响契约：** [`BOOT-PROTOCOL-001`](../../contracts/task/boot-protocol.md#boot-protocol-001--typed-initial-program-source统一收口到普通-vfs-exec)（Refine；R6A已生效）。
**开放问题：** None。
**下一步：** None；决赛脚本与具体决赛Platform实例不属于本RFC。
...
- 将 Anemone Boot Protocol 从固定的 `/.anemone/init` entry 扩展为 typed
  ...
  以resolved executable path作为唯一参数。
...
- 允许QEMU provider的string字段与固定args使用同一`{{name}}` opaque-string替换，因此裸QEMU Platform
  可以把SMP、memory等machine参数交给本次action显式绑定，而产品Platform仍可用literal固定语义。
...
- 要求build只消费DT provider字段实际引用的bind；QEMU execution消费provider字段、固定args和runtime
  attachment组。两种action都拒绝未知、重复、空、缺失或未消费值，不对值做类型、范围、单位、文件或
  QEMU keyval验证。
...
- 不为 QEMU runtime path 建立 generic external-role/slot/disk binding hierarchy；只
  支持 QEMU-local argv template。
- 不为bind增加类型、上下限、单位、文件存在性、artifact类别或QEMU参数语义验证。
- 不增加initial-program envp配置或shell-style argv splitting。
...
QEMU provider string与固定argv中需要调用者提供opaque value的位置统一使用具名占位符
`{{name}}`。`[[qemu.bind]]`只为需要原子加入或省略的一组runtime argv提供容器；它不是
SystemTarget product role、provider-neutral input schema或generic workflow binding：
...
`template` 是 argv token array，不是第二种bind语法或shell command；每个token独立传给
`std::process::Command::arg()`，不得按空格二次切分。`{{name}}`是唯一替换语法，bind value是
opaque string并只做一次非递归替换。每个declaration的template只能引用自己的name，且至少出现
一次；`optional = true`时缺失该name会省略整个参数组，false时缺失失败。Name在同一QEMU section内
唯一，declaration配置顺序决定展开顺序，CLI顺序不得改变argv topology。
...
`build`与普通`qemu`使用同一`--bind name=value`词汇。Build只展开machine、CPU、SMP、memory与optional
BIOS这些provider字段，并拒绝未被这些字段消费的值；QEMU再展开provider字段、固定`args`与runtime
bind组，并拒绝未被完整command消费的值。两者都在副作用前拒绝未知、重复、缺失required或空值，
但不检查type、range、unit、path存在性、逗号或QEMU keyval含义。Tracked config只保存literal、placeholder
和group template，不保存本次value或机器专属探测结果。
...
```text
just build     (--preset <preset> | --target <target> --kernel-config <path> --profile <profile>) --bind <name>=<value>...
just qemu      (--preset <preset> | --target <target> --kernel-config <path> --profile <profile>) --bind <name>=<value>...
...
普通 `qemu` 从 target 引用的 platform 取得 QEMU provider，并要求调用者提供provider/fixed args引用的
全部bind和required runtime group；optional group可以省略。Physical
platform 没有 QEMU provider 时，普通 `qemu` 明确报告 action 不受支持，不为统一外观建立
generic launcher。

`--bind name=value` 只填充 selected platform 使用的具名placeholder；它不选择 product role、
...
选择 value，resolver/QEMU action 不证明该 value 对应 SystemTarget root selection、
architecture、format、QEMU语法或先前 build/rootfs action result。Value通过机械校验但内容选错时，可以在
QEMU/guest boot 或 wrapper 验证中失败；这不是本阶段 build model 承诺提前拒绝的 mismatch。
`--show-bindings` 在解析显式输入/platform后打印provider/fixed args中的required name及每个runtime
group的name、required/optional和argv template并退出，
不启动QEMU，也不要求先提供bind value。
不提供raw `--qemu-arg`；用户只能填充tracked string中的`{{name}}`，不能增加新argv
...
machine、CPU、SMP、memory和optional BIOS执行`dumpdtb`；这些provider字段中的具名bind在build side
effect前展开。Materializer不解析另一份Platform reference，也不读取ordinary `qemu.args`、SystemTarget
runtime role、rootfs/test disk或runtime group。输出先写入
...
QEMU bind value、debug switch 与 console presentation 属于
...
| `just xtask qemu --platform ... --image ...` | 普通execution由`just qemu <explicit-input> --bind name=value`取代；显式输入确定platform，所有动态QEMU value由该platform的具名placeholder声明，不保留特殊`--image` |
...
- QEMU bind 明确保持完全人工映射；普通 QEMU action 只验证 declaration/map/value 的机械
...
- Provider字段bind若缺值，build必须在side effect前拒绝；runtime bind组若改变provider DT则说明它已经
  改变Platform topology，必须停止配置变更并回到Platform review，不能用dummy backend参与build-time dump。
...
- QEMU template若经过shell或按空格切分，opaque value可能改变argv token结构；必须逐token传递。
  Binding层不理解QEMU keyval，因此value是否适合模板位置由Platform/wrapper/runtime验证承担。
...
R6是当前accepted target。R0-R2的Stage 1-6、R3A与R4A均已关闭；R6不重开历史stage或Completed transaction，
...
repository surface cutover。R5确认当时没有剩余实施阶段并关闭。R6以新的transaction加入具名opaque bind、
optional runtime argv组与两种initial-program source共享的完整argv；唯一Checkpoint R6A已完成并cut over。
