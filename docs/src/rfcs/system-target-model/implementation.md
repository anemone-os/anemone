...
source/baseline保持。R4A已完整解析为Ready但没有实施授权；R4B只保留SMP Platform/adopter迁移Outline。
...
**状态：** Accepted for Implementation；R4A Ready，R4B Outline。
...
`BOOT-PROTOCOL-001` Preserve。R0-R3 transaction均保持Completed；R4A activation时建立新的R4 transaction。

### Checkpoint R4A - QEMU provider DT cutover

**阶段成熟度：** Ready；本段是唯一authoritative Ready definition。当前没有implementation transaction、
activation preflight或代码实施授权。
