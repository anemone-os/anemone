...
**状态：** R2（Stage 1-4 Closed；Stage 5 Ready / Checkpoint 5A Not Activated）
**最后更新：** 2026-07-24
...
Checkpoint 4A并关闭。`Stage 4 -> Stage 5 Implementation Resolution Gate`现已完成；Stage 5解析为
单一Checkpoint 5A并达到Ready，但尚未激活。
...
- Stage 1 已由 `0B - Initial Implementation Resolution Gate` 完整解析为 `Ready`，在R0接受与
  ...
  独立resolution gate并完成4A后关闭；Stage 5已由独立resolution gate解析为`Ready`，Stage 6仍为
  `Outline`。
...
| Stage 5 | Ready | EmbeddedApp vertical slice 与 Boot Protocol cutover | Stage 1-4 Closed；Stage 4 -> Stage 5 resolution完成 | 单一Checkpoint 5A Not Activated |
...
Checkpoint 4A随后于2026-07-24经独立用户授权执行、验证和关闭。Stage 5 resolution未运行。
...
**阶段成熟度：** Ready；`Stage 4 -> Stage 5 Implementation Resolution Gate`已于2026-07-24完成，
单一Checkpoint 5A尚未激活。Ready只冻结实现与证明边界，不授予代码写入或contract cutover权限。
...
2. 运行受影响kernel build/KUnit或等价private定向测试：覆盖缺失`/.anemone`时创建、已有目录时overmount、
   非目录与各publication step失败的boot-fatal结果、temp在rename前不可见、published bytes/path/mode为
   预期值，以及backing root没有可执行truth。若现有KUnit不能安全构造mount/exec环境，以最小private helper
   test加后续QEMU smoke共同证明，不为了测试引入第二materializer。
...
- 上述模块内与5A直接相关的private/unit/KUnit tests
