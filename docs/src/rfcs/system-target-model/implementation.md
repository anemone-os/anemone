...
**状态：** Implemented / Closed；R4A Closed，原R4B由R5移出RFC。
...
- R4A完成DT schema/materialization/CLI cutover，并保持当时的Platform/SystemTarget/Preset identity；
  后续配置应用不属于该DT checkpoint。
...
canonical docs，将R4A标记Closed。当时R4B保持Outline且未自动解析或激活；该历史边界随后由R5处理。
...
R4A Closed；本次未运行`R4A -> R4B Implementation Resolution Gate`。

## R5：移除R4B应用范围

**状态：** Implemented / Closed（2026-07-24）。

用户确认hardware-semantic QEMU Platform与用途配置只是target model的一次应用，不属于本RFC target。
R5因此删除原R4B Outline及其resolution/activation入口。R4B从未进入Ready或Active，没有代码、contract
cutover或验证结果需要撤销；R4A的DT authority、delivery、materialization与DT-neutral bind规则保持有效。
