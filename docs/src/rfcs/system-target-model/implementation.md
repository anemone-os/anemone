...
**状态：** R2（Stage 1-3 Closed；Stage 4 Ready / Checkpoint 4A Not Activated）
**最后更新：** 2026-07-23
...
`Stage 3 -> Stage 4 Implementation Resolution Gate`已完成；Stage 4已解析为单一Checkpoint 4A并保持
Ready / Not Activated。
...
- Stage 1 已由 `0B - Initial Implementation Resolution Gate` 完整解析为 `Ready`，在R0接受与
  ...
  独立resolution gate解析为单一Checkpoint 4A并保持`Ready`；Stage 5与Stage 6仍为`Outline`。
...
| Stage 4 | Ready | Source app driver、app/rootfs workflow 与 physical-board closure | Stage 1-3 Closed；R2 correction Closed；Stage 3 -> Stage 4 resolution完成 | 单一Checkpoint 4A等待独立activation |
...
**阶段成熟度：** Ready；`Stage 3 -> Stage 4 Implementation Resolution Gate`已于2026-07-23独立完成。
Gate把剩余工作解析为单一Checkpoint 4A；该checkpoint尚未激活。
...
- VisionFive physical closure没有剩余代码迁移：Platform `[uboot]`仍唯一拥有header/load/entry/name/
  filename，normal `build`继续由现有post-link生成`build/anemoneImage-rv64`；rootfs manifest与README已经把
  同一selection的`build -> rootfs`顺序写成fixed-path handoff。Stage 1完整组合证据早于Stage 2 CLI cutover，
  Stage 3只验证了current explicit-preset build，没有重新materialize rootfs；因此4A必须在当前命令面实际重跑
  完整顺序并检查最终镜像内容。该部分没有独立代码交付，只作为4A的Preserve validation，不形成第二个board
  checkpoint，也不要求实机启动。
- Source、rootfs consumer与physical Preserve都不依赖前一个checkpoint的实际diff才能解析，且没有独立
  cutover或恢复边界。拆分只会制造中间生命周期状态，因此Stage 4冻结为单一Checkpoint 4A。
...
### Checkpoint 4A - Source app driver 与 workflow Preserve closure
...
- Rootfs不增加schema、adapter或artifact handoff；`stage_apps()`继续以空args调用公共`build_app()`并复制其
  exported artifact。VisionFive Platform、post-link实现、SystemTarget/preset、rootfs fixed-path manifest和
  命令顺序保持latest bytes；提交重命名、完整介质装配与Stage 5 EmbeddedApp均不进入4A。
...
1. `just xtask-test`覆盖Cargo与Source manifest正反例、Source manifest/CLI extra args拒绝、Cargo command
   ...
   missing、directory与non-regular input在export前失败。用于“无子进程”的fixture必须在被执行时明确失败，
   而默认Source action仍成功且export bytes/mode符合公共copy路径，避免只凭代码分支声称no-command；另核对
   `--disasm`仍是显式post-export路径，不被Source默认触发或作为admission条件。
...
4. 使用当前explicit preset按README从干净的本轮marker依次运行
   `just build --preset visionfive2-rv64-release`与
   `just rootfs mkfs -c conf/rootfs/visionfive2/rootfs.toml --sudo`；build失败时不得启动rootfs action。核对
   U-Boot header/load/entry/name、`build/anemoneImage-rv64` freshness，并从本轮rootfs image只读抽取
   `/boot/anemoneImage`，证明其bytes等于本轮Platform output。Developer-local base image、host tool或sudo不可用
   时4A保持Active，不得用旧artifact、Stage 1/3证据或只做source audit降级关闭。Physical board、QEMU runtime、
   kernel boot、LTP与final harness均Not Run，不误述为本stage证据。
5. Live app help、template、build-system skill、tracked app/rootfs consumer与Source/Cargo residual audit通过；
   independent latest-byte review核对single exporter、no-command、args fail-close、rootfs owner与physical
   Preserve。`git diff --check`、相对链接/lifecycle/write-set审计和`mdbook build docs`通过。
...
- physical audit发现必须修改Platform `[uboot]` owner/schema、post-link output、firmware/bootloader handoff或板级
  visible behavior，或不能在latest bytes上证明现有fixed-path顺序；
...
successor保持不变。Checkpoint 4A完成全部validation floor、independent review与transaction/RFC/navigation
write-back后，Checkpoint 4A与Stage 4同时Closed。关闭不运行、解析或激活
...
`scripts/xtask/src/tasks/rootfs/mkfs.rs`、`scripts/xtask/src/{config/platform.rs,
tasks/build/kernel_output.rs}`；全部tracked app manifests；`conf/platforms/visionfive2-rv64.toml`、
`conf/system-targets/visionfive2-rv64.toml`、`conf/build-presets/visionfive2-rv64-release.toml`、
`conf/rootfs/visionfive2/{rootfs.toml,README.md}`；disposable Source fixture、`build/apps/**`与`/tmp`输出。
