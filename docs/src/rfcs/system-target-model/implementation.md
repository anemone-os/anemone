...
单一Checkpoint 5A已获独立授权、完成验证与review并关闭。最终resolution gate把Stage 6解析为
单一Checkpoint 6A；6A随后完成current workflow回归、审计与生命周期write-back并关闭。
...
2026-07-24用户进一步接受R3：删除R2的implicit local/default selection与preset presentation-default
扩展点，并收口rootfs type、QEMU CPU和fmt scope的显式输入。R3由新的单一checkpoint与transaction
实施并关闭；下文R0-R3历史内容保留为执行证据，不再定义当前目标。
2026-07-24用户接受R4：QEMU machine model作为可执行DT authority时不再提交DTS mirror；QEMU
firmware delivery使用runtime FDT，QEMU embedded delivery由normal build自动`dumpdtb`并发布build-local
DTB。Build不展开QEMU bind，所有bind必须DT-neutral；`qemu dt` maintenance CLI整体删除。Physical DTS
source/baseline保持。R4A已完成并关闭；R4B只保留SMP Platform/adopter迁移Outline，未解析或激活。
...
- Preset不携带presentation defaults；action-local presentation input由每次调用显式提供。
  不为潜在需求建立开放字典、任意args、overlay或通用workflow DSL。
...
- DT authority按Platform分类。QEMU Platform由可执行provider拥有且不保留committed baseline；physical
  Platform可以保留stable source/baseline。一个Platform未完成分类不阻止另一个owner-local gate，但未分类
  Platform不得迁移或声称完成。
...
- Stage 1 已由 `0B - Initial Implementation Resolution Gate` 完整解析为 `Ready`，在R0接受与
  ...
  独立resolution gate并完成4A后关闭；Stage 5与Stage 6均由独立resolution gate解析并分别完成5A、6A后
  关闭。
...
| Stage 6 | Closed | Closure、现行workflow回归与adopter handoff | Stage 1-5 Closed；Stage 5 -> Stage 6 resolution完成 | 6A Closed；transaction Completed |
...
**阶段成熟度：** Closed；`Stage 5 -> Stage 6 Implementation Resolution Gate`已于2026-07-24完成，
单一Checkpoint 6A随后由完成Stage 6的独立用户授权激活、验证并关闭。

### Stage 5 -> Stage 6 Resolution 结论（2026-07-24）

Preflight重新读取R2 target/invariants、Stage 5最终diff与review/validation、current
`BOOT-PROTOCOL-001`、closed tracking issues、register/current limitations、current transaction，以及live
selection/build/app/rootfs/QEMU/DT owner、tracked config、wrapper、build docs和repo-local build skill。
`just --list`、`just xtask build --help`与`just xtask rootfs mkfs --help`确认current command surface；register
没有与本RFC重叠的active issue或limitation。

Stage 1-5已经完成全部能力和contract cutover，最终stage不再增加实现能力。Resolution把Stage 6收敛为单一
Checkpoint 6A：在current bytes上审计第二selection/resolver/host-path truth，汇总各supported Platform、
app/rootfs workflow与Boot Protocol的已有证据，执行VisionFive current explicit-preset `build -> rootfs`
物化与镜像内kernel equality回归，最后原子同步RFC/transaction/navigation/register/contract审计结果和adopter
handoff。各项之间没有可独立发布的半能力，因此不拆checkpoint或增加probe。

### Checkpoint 6A - Current workflow regression 与生命周期收口

**状态：** Closed（2026-07-24）。

**交付与验证：**

1. 审计live config parser、selection resolver、action CLI、Justfile、tracked defaults/schema/examples、pretest
   wrappers、rootfs recipe/docs和build skill；旧CLI只允许作为拒绝测试或历史证据存在，不得仍形成production
   selection、resolver、QEMU host-path truth或compatibility bridge。核对live help，并运行最终xtask tests。
2. 汇总transaction中五个supported Platform、Cargo/Source app、rootfs composition、QEMU/DT、Platform
   post-link与typed Boot Protocol的实际验证及明确Not Run边界；不得用旧stage证据冒充本轮重跑。
3. 从repository root和current `PATH`依次运行
   `just build --preset visionfive2-rv64-release`与
   `just rootfs mkfs -c conf/rootfs/visionfive2/rootfs.toml --sudo`。后者替换
   `build/rootfs/visionfive2/`；`conf/rootfs/visionfive2/rootfs-alpine.img`只读作为developer-local base。
   从本轮最终镜像只读抽取`/boot/anemoneImage`，逐字节等于同一轮
   `build/anemoneImage-rv64`才算通过。Developer-local base、`dtc`/cross toolchain/`mkimage`、libguestfs或
   sudo不可用时，6A保持Active并准确记录阻塞，不回溯重开已关闭的Stage 1-5。
4. 在最终文档字节上运行lifecycle/status、relative-link、whitespace、resolved write-set和mdBook检查；
   latest-byte review必须为Apollyon 0 / Keter 0，Euclid要么修复，要么有不影响owner/contract/acceptance的
   明确归属。随后原子将RFC置为Closed、transaction置为Completed并同步公共导航和当期devlog。

**停止 / 恢复：** 若residual审计发现仍生效的第二truth或required workflow不成立，在6A write set内只能
修正文档残留；任何production code/config/schema/wrapper/skill修复都先停止并报告具体owner、最小扩展、
contract影响与验证计划。若需要改变target invariant、owner、ABI、visible semantics、shared contract或
acceptance boundary，立即进入Target Renegotiation Gate。`BOOT-PROTOCOL-001`已经effective，本stage不再次
cut over或改写其语义。Final harness是Stage 6关闭后的独立adopter iteration；其未实现不阻塞本RFC关闭，也
不得回填为本stage能力。

**Resolved Write Set Manifest：**

- 本RFC `index.md`与`implementation.md`；
- 本RFC `invariants.md`（批准的closure-only扩展：仅同步RFC状态与已经完成的
  acceptance/transaction/`BOOT-PROTOCOL-001` cutover结果，不修改R2 target invariant）；
- current transaction与transaction index；
- `docs/src/rfcs.md`与`docs/src/devlog/2026-07-06_to_2026-07-19.md`。

Read-only audit / validation inputs包括本RFC `tracking-issues.md`、
`docs/src/contracts/task/boot-protocol.md`、register/current limitations、`docs/src/SUMMARY.md`、Justfile、
`scripts/xtask/`、`conf/`、`anemone-apps/`、`anemone-kernel/`、pretest wrappers、build-system skill、root
`kconfig`、developer-local VisionFive base image、host tools与generated `build/` outputs。不得提交或手工修补
validation-only/generated state。若6A需要tracked编辑read-only输入，按上述停止/expansion合同处理。

**Contract / Exit：** Contract cutover为None；`BOOT-PROTOCOL-001`保持Checkpoint 5A形成的effective
typed contract。6A验证、latest-byte review、write-set/lifecycle/residual审计与全部write-back通过后，
Checkpoint 6A和Stage 6关闭，RFC状态为Closed，transaction状态为Completed。Stage 6是最终stage，不存在可由
本closure自动进入的下一gate。

### Checkpoint 6A closure（2026-07-24）

6A在current bytes上完成68项xtask tests、三份canonical schema validation、全部live command help、
production residual/owner audit，以及VisionFive explicit-preset `build -> rootfs`完整回归。Build从committed
VisionFive DTS生成build-local DTB，完成release kernel与Platform-owned U-Boot post-link；rootfs action从只读
developer-local base生成15,032,385,536-byte运行副本。以同一sudo/libguestfs环境只读抽取最终镜像的
`/boot/anemoneImage`，与本轮`build/anemoneImage-rv64`逐字节相等，SHA-256均为
`07e1aa158bdb49d0a49b10d8ddc4bef0d9ffc6434c55c4b1fd1e979b3f7ade54`。Base/master前后
SHA-256均为`2f7e3529cee1f88fb88535c0dcb0b1a7ee463ebdb76131180623af0519a5e9fb`。

Evidence matrix汇总而不冒充重跑：五份supported Platform/SystemTarget和全部tracked preset由current
68项tests覆盖；RV64/LA64 normal build、四份QEMU Platform DT check与RV64真实QEMU正常关机保留Stage 2-3
证据；Cargo/Source机械export保留Stage 4证据；RootfsEntry、Embedded ELF、shebang reopen、
missing-interpreter boot-fatal与`BOOT-PROTOCOL-001` cutover保留Stage 5证据；VisionFive current command
surface与镜像内容由本6A重新验证。真实LA64 QEMU、physical board、完整LTP与final harness仍Not Run，不是
Stage 6 closure floor；final harness只作为后续独立adopter iteration。

Final latest-byte review、lifecycle/residual/write-set、relative-link、whitespace与mdBook检查通过，结论为
Apollyon 0 / Keter 0 / Euclid 0。用户批准的`invariants.md` closure-only扩展只同步Closed及已发生的
acceptance/transaction/Checkpoint 5A cutover事实，不改变R2 target或current contract。Tracking issues、
register/current limitations、`BOOT-PROTOCOL-001`正文、代码/config/schema/wrapper/build skill均经审计保持
零diff。Checkpoint 6A与Stage 6 Closed；RFC Closed，transaction Completed，不存在下一gate。

## R3 explicit-input cleanup

**状态：** Closed；单一Checkpoint R3A已完成。

### Feedback / target correction（2026-07-24）

用户拒绝implicit selection：除经典KernelConfig默认继承外，构建系统不应通过省略输入选择
developer-local或repository-default preset，也不应在preset中预留presentation defaults。Live-owner
审计同时确认三个同类surface：rootfs省略`fs.type`会选择folder，QEMU省略CPU会依赖provider default，
bare `fmt`会选择all scope。用户接受R3并明确两个边界：folder容量统一自动计算，不需要容量配置；QEMU
BIOS保持optional，因为部分target没有合适BIOS，省略的精确定义是“不发出`-bios`”。

R3不改变Platform/SystemTarget/KernelConfig/BuildPreset owner，不改变QEMU bind、DT delivery/authority、
Boot Protocol、kernel/runtime ABI或current contract。R0-R2 transaction已经Completed，不续写；本修订使用
独立transaction，且最终只提交一个commit。

### Checkpoint R3A - Explicit inputs and surface cleanup

**交付：**

1. 删除`selection show/set/clear`、local/default selection文件/schema/ignore规则、implicit resolver branch
   与对应测试；保留named `BuildPreset`、显式`--preset`、显式完整low-level tuple与single resolver。
2. Preset保持只有target、KernelConfig与kernel-only `CargoProfile`；presentation input继续是action-local
   CLI flag，且必须由调用者显式给出。
3. Rootfs `fs.type`必填；删除容量字段与`--size`生成路径，folder统一委托`virt-make-fs`自动计算，image
   仍要求base。自然可空的apps/dirs/files集合保持可省略。
4. QEMU CPU改为Platform必填字段，两份LA64 Platform固定`la464`；normal execution与DT maintenance总是
   发出`-cpu`。BIOS保持optional，None不发`-bios`，显式值原样传递。
5. `fmt`要求显式`all`、`kernel`或app scope；`all`覆盖kernel、standalone xtask与全部apps。同步schema、
   tracked manifests、help、build docs、repo-local build skill、RFC与公共导航。

**Resolved Write Set Manifest：** `Justfile`、`.gitignore`；`scripts/xtask/src/main.rs`、
`workspace.rs`、`config/{selection.rs,resolve.rs,rootfs.rs,platform.rs,kconfig.rs,build.rs}`、
`tasks/{conf.rs,fmt.rs,qemu.rs,rootfs/mkfs.rs}`；删除`conf/default-selection.toml`与
`conf/selection-schema.jsonc`；`conf/README.md`、`conf/platforms/schema.jsonc`、两份LA64 Platform、
LA64 provider-derived DTS、三份folder rootfs manifest；`.agents/skills/anemone-build-system/`的skill与
两份reference；本RFC四份canonical文档；新的R3 transaction、transaction index、`docs/src/rfcs.md`、
`docs/src/SUMMARY.md`与当前双周devlog。R0-R2 transaction、current contract、register/current
limitations、kernel/app/wrapper源码与其它Platform/DT均为read-only audit/validation input。

**Validation / Exit：** `just xtask-test`覆盖bare/mixed/partial输入拒绝、全部tracked preset、rootfs type/
size拒绝、QEMU required CPU与optional BIOS argv；核对live help与`just --list`；证明bare build/QEMU在生成或
launch前失败；运行显式RV64/LA64 build、四份QEMU Platform DT check、`just fmt all --check`与focused
scope、两份wrapper `bash -n`、schema解析、residual/source audit、`git diff --check`和`mdbook build docs`。
若`la464`导致DT drift或runtime/build不兼容，停止并回到target review；不得改用另一个隐式CPU。全部通过并
完成latest-byte review后，将RFC/R3A/transaction置为Closed/Completed；contract cutover为None。

**Implementation feedback / manifest expansion：** 首次`just fmt all --check`在进入新纳入的xtask/apps
检查前，被`kconfig_defs.rs`与`platform_defs.rs`中生成器稳定输出的行尾/末尾空白阻断。手改generated
文件无法形成长期修复。R3A因此在owner-local范围把`config/kconfig.rs`加入write set；`platform.rs`已经
在原manifest内。只修正生成字符串空白并通过显式build重新生成，不改变KernelConfig值、Platform常量、
public API或contract。

生成器修复后，显式`just fmt all`首次把standalone xtask纳入格式化，并机械更新既存rustfmt drift：
`config/system_target.rs`、`tasks/build/mod.rs`及`anemone-apps/{float-test,init,mmap-test,pg-test,
pthread-create-stress,signal-test}`对应Rust源码。仓库规则允许formatter产生的write-set外风格维护；这些
文件只接受rustfmt机械变更，full diff review必须确认零行为变化。

### R3A closure（2026-07-24）

R3A删除`selection` command、两份local/default selection文件/schema、ignore rule、implicit resolver
branch/source variants与对应测试；BuildPreset、显式preset、完整low-level tuple、single resolver和classic
KernelConfig默认继承保持。Rootfs type必填且size字段/argv已删除；folder统一自动计算。QEMU CPU必填，LA64
固定`la464`；BIOS保持optional。Fmt scope显式，all覆盖kernel、xtask和全部apps。

最终字节`just xtask-test`为64 passed / 0 failed；bare build/QEMU、partial/mixed input、bare/unknown fmt均在
mutation/launch前拒绝。显式preset与tuple的`--show-bindings`通过。RV64与LA64 release preset在沙箱外按
顺序build成功；沙箱内首次RV64 build仅因lwext4 C编译触发seccomp `Bad system call (159)`失败。一次错误的
并发双架构validation因共享`build/generated/kernel.lds`令RV64链接到LA64脚本而作废，未计入证据；随后两
架构均串行重跑成功。

四份QEMU Platform DT check全部通过：RV64保留`-cpu rv64 -bios default`，LA64使用`-cpu la464`且不发
`-bios`，baseline无drift。Platform/SystemTarget/BuildPreset schema解析、rootfs parser拒绝missing type与
size、两份wrapper `bash -n`、`just fmt all|kernel|user-test --check`、live help、residual/source audit、
`git diff --check`与`mdbook build docs`通过；mdBook只有既有large search-index warning。Formatter机械变更
经full diff review为零行为变化。Latest-byte review为Apollyon 0 / Keter 0 / Euclid 0。

真实QEMU guest runtime、rootfs materialization、physical board、LTP与final harness未运行，不作为R3A证据。
Contract cutover为None；`BOOT-PROTOCOL-001`保持不变。Checkpoint R3A、R3 revision与新transaction关闭。

## R4：QEMU provider DT 与 hardware-semantic Platform

**状态：** Accepted for Implementation；R4A Closed，R4B Outline。

### R4 feedback / target correction（2026-07-24）

用户确认QEMU machine model本身已经是可执行machine-fact authority。继续提交`dumpdtb`反编译得到的DTS，
再维护refresh/check、canonicalization与provider drift，只是在仓库中保存第二份QEMU镜像；SMP topology
variant还会把该镜像复制为多份baseline。用户接受以下R4 target：

- QEMU Platform不再提交DTS；firmware delivery直接消费runtime FDT，embedded delivery由normal build从
  selected provider自动生成build-local DTB；
- R4A保持current kernel handoff，只接受RV64 firmware与LA64 embedded delivery；反向组合不作为future
  schema capability开放；
- normal build不读取或展开QEMU bind，不为bind提供placeholder/dummy backend；所有bind必须DT-neutral；
- `qemu dt`、refresh/check、semantic diff与baseline write-back production surface全部删除；
- physical Platform在没有等价可执行provider输入时继续提交稳定DTS source/baseline；
- R4A先完成DT schema/materialization/CLI cutover；现有Platform/SystemTarget/Preset identity保持；R4B再
  迁移`qemu-virt-{rv64,la64}-smp-{1,8}`与business-semantic adopter配置。

该修订不改变runtime FDT delivery、kernel runtime ABI、Boot Protocol、U-Boot output、root/initial-program、
QEMU ordinary execution bind argv或physical DTS内容。`Contract Impact`为None，
`BOOT-PROTOCOL-001` Preserve。R0-R3 transaction均保持Completed；R4A由独立R4 transaction实施并关闭。

### Checkpoint R4A - QEMU provider DT cutover

**阶段成熟度：** Closed；本段保留R4A执行时使用的authoritative Ready definition与冻结manifest。

**目标：** 删除QEMU committed DTS与public DT maintenance surface；让normal build按selected Platform的
delivery/authority/provider唯一决定本轮DTB行为，并保持physical Platform与ordinary QEMU execution行为。

**交付：**

1. Platform DT config保持`delivery + authority + provider`，将committed `source`收缩为可选字段。
   `conf/platforms/schema.jsonc`只表达字段形状、枚举与`source` optionality；以下组合约束由xtask在完成
   Platform反序列化后、任何action side effect前校验，不在JSON schema增加条件分支：
   - `[qemu]`存在当且仅当`provider = "qemu"`；该组合要求`authority = "provider-derived"`且`source`
     缺失；QEMU Platform不得选择physical/normative source；
   - physical firmware provider-derived baseline要求`provider = "firmware"`、`delivery = "firmware"`且
     `source`存在，同时`[qemu]`缺失；
   - embedded normative source要求`provider`和`[qemu]`均缺失且`source`存在；
   - 在上述identity分类之外，`build.arch = "riscv64"`要求`delivery = "firmware"`，
     `build.arch = "loongarch64"`要求`delivery = "embedded"`；反向组合在action side effect前拒绝；
   - 其它组合同样在action side effect前拒绝。Rust authority类型从只描述source的`DtsAuthority`收口为
     描述整体DT contract的名称；不增加新的provider/source DSL。
2. Build DT materializer从完整resolved Platform分派：
   - 任意`firmware` delivery在prebuild开始时删除stale `build/generated/device-tree/platform.dtb`，不生成
     本轮DTB；committed physical baseline不是normal-build input；
   - physical embedded normative source继续执行`dtc`并以temporary + atomic rename发布；
   - QEMU embedded provider执行固定architecture QEMU程序，在`-machine`中加入build-owned temporary
     `dumpdtb` path，并只传入resolved machine、CPU、SMP、memory与optional BIOS；不加入ordinary
     `qemu.args`、console/debug参数或bind；
   - provider success后检查DTB output存在、是regular/non-empty并具有有效FDT header/size，再将raw DTB
     原子发布；不剥离`/chosen/rng-seed`，也不做deterministic canonicalization或double dump；
     tool/output/publish失败时清理temporary/final output并停止kernel compile，不能复用旧DTB。
3. 删除`qemu dt` Clap branch、refresh/check arguments、drift error/exit status、canonicalization、semantic
   diff、atomic source update、provenance renderer及其测试。保留ordinary QEMU execution、bind validation、
   `--show-bindings`、debug与fixed-program lookup；build仅复用窄的architecture -> QEMU program owner，不
   调用ordinary `gen_qemu_cmd()`。
4. 四份现有QEMU Platform删除`dtb.source`并保持当前machine/CPU/SMP/memory/BIOS/fixed args/binds不变；
   删除`qemu-virt-{rv64,la64}.dts`。Physical Platform及`visionfive2-board.dts`、`2k1000-board.dts`内容
   保持；schema只收缩`source` optionality，example文档化无source的QEMU firmware contract与有source的
   physical contract规则。
5. 同步`conf/README.md`、live help、build-system skill及两份reference、R4 canonical docs与public RFC
   navigation。历史R0-R3 transaction、旧RFC/background/devlog中的执行命令和证据保持只读，不批量改写。

**Topology-neutral invocation invariant：** R4A不增加schema flag。当前ordinary `qemu.args`以及
`kernel-image`、RV64 virtio-mmio block与LA64 PCI block attachment必须相对core topology snapshot生成相同
canonical provider DT。未来args/template变化若破坏等价，Platform config change必须停止；不得在R4A内
增加bind kind、dummy source、optional bind或第二套untyped topology argv来绕过。

**Resolved Write Set Manifest：**

- `scripts/xtask/src/config/platform.rs`；
- `scripts/xtask/src/tasks/{qemu.rs,build/mod.rs,build/device_tree.rs}`、`scripts/xtask/src/main.rs`；
- `conf/platforms/{schema.jsonc,example.toml,qemu-virt-rv64.toml,qemu-virt-rv64-pretest.toml,
  qemu-virt-la64.toml,qemu-virt-la64-pretest.toml}`，并删除
  `conf/platforms/qemu-virt-{rv64,la64}.dts`；
- `conf/README.md`；
- `.agents/skills/anemone-build-system/{SKILL.md,references/build-playbook.md,references/config-model.md}`；
- 本RFC的`index.md`、`invariants.md`、`implementation.md`、`tracking-issues.md`与`docs/src/rfcs.md`；
- R4 activation后新增
  `docs/src/devlog/transactions/2026-07-24-system-target-model-r4-qemu-dt.md`，并同步
  `docs/src/devlog/transactions/index.md`、`docs/src/SUMMARY.md`与
  `docs/src/devlog/2026-07-20_to_2026-08-02.md`。

`Justfile`、SystemTarget/BuildPreset/rootfs/wrapper、kernel、physical Platform/DTS、current contract、register、
R0-R3 transaction与其它RFC均为read-only audit/validation input。若真实实现必须修改上述owner，先停止并
上报write-set expansion，不把兼容旁路塞进现有manifest。仅由`just fmt`产生的机械格式化仍按仓库规则
处理并在transaction中逐文件登记。

**Validation / Proof：**

1. `just xtask-test`覆盖DT contract合法/非法矩阵、RV64 embedded与LA64 firmware拒绝、source路径校验、
   QEMU provider必须存在、firmware stale
   output删除、physical embedded `dtc`、QEMU embedded argv、provider failure/missing/invalid output、atomic
   publish/cleanup；ordinary QEMU bind/selection tests保持通过。
2. `just xtask qemu --help`与negative CLI证明`qemu dt`、refresh/check及drift exit status已删除；
   `--show-bindings`仍显示当前声明，ordinary QEMU argv逐token保持。
3. 串行build当前RV64 bare/pretest release，证明normal build不启动QEMU且不会留下platform DTB；先执行
   LA64 build再执行RV64 build，专门证明stale embedded DTB被删除。
4. 串行build当前LA64 bare/pretest release，捕获命令证明使用`la464`、`smp = 1`、`memory = 1G`、无BIOS，
   且没有ordinary `qemu.args`/bind；检查本轮published DTB、embedded kernel dependency和CPU topology。
5. Build VisionFive与2K1000 release，证明physical firmware baseline不成为build DTB、2K1000 normative DTS
   仍通过`dtc`嵌入，U-Boot output行为不变；physical runtime Not Run不误报为验证通过。
6. 使用worktree-local/disposable kernel与disk input，对两架构current pretest Platform依次比较core
   topology snapshot、加入ordinary fixed args、再加入完整三bind attachment后的canonical provider DT；
   只接受volatile `/chosen/rng-seed`差异，证明ordinary args与三个bind均DT-neutral。不得使用或修改
   LOCAL.md中的只读master image。
7. RV64/LA64各一次pretest boot smoke，确认RV64 runtime FDT与LA64新embedded provider DTB可启动；不运行
   LTP/final harness。执行`just fmt all --check`、schema/residual/source audit、wrapper `bash -n`、
   `git diff --check`与`mdbook build docs`，最后做latest-byte review。

**停止条件：**

- 任一current ordinary arg或bind改变canonical provider DT；
- QEMU embedded dump必须读取runtime path/backend、展开bind或要求新的generic topology schema；
- RV64/physical firmware build仍有kernel consumer要求build-local DTB；
- LA64无法从本轮provider output启动，或provider output不能在kernel compile前可靠验证/发布；
- physical Platform必须改变DTS内容、delivery、U-Boot或runtime contract；
- 实现需要tracked QEMU path/version、HostEnvironment resolver或恢复committed QEMU baseline。

命中停止条件时保持live R3行为，记录证据并回到R4 target review；不得保留半切换schema、一个architecture
使用新路径而另一个仍靠baseline，或以dummy bind/source让测试表面通过。

**Exit / closure：** 全部交付、验证与latest-byte review通过后，在同一checkpoint更新R4 transaction与
canonical docs，将R4A标记Closed；R4仍保持Accepted for Implementation，R4B保持Outline。R4A关闭不自动
解析或激活R4B。

#### R4A closure（2026-07-24）

R4A已按冻结manifest完成：QEMU Platform删除committed DTS与maintenance CLI，firmware build清理stale
DTB，LA64 embedded build从selected QEMU topology自动验证并原子发布raw DTB，physical source路径保持。
四份QEMU manifest、schema/example、build skill与公共导航已同步；ordinary QEMU bind surface保持。

验证覆盖55项xtask tests、两架构QEMU bare/pretest与两份physical release build、两架构topology-neutral
snapshot、CLI/schema/source/residual/wrapper/fmt/docs检查。RV64既有成功boot证据覆盖runtime FDT、255项KUnit与
init/user-test；该窗口随后进入现有LTP profile，LTP输出不计入R4A证据。一次更短复跑在既有`openat` KUnit
出现`AlreadyExists` panic，用户要求停止继续测试并确认以成功boot证据收口；没有把该现象归因于R4A。
LA64 runtime按用户明确指示Not Run，physical runtime、完整LTP与final harness同样不属于本checkpoint证据。

Latest-byte review为Apollyon 0 / Keter 0 / Euclid 0。Contract cutover为None，`BOOT-PROTOCOL-001` Preserve。
R4A Closed；本次不运行`R4A -> R4B Implementation Resolution Gate`。

### R4B - Hardware-semantic QEMU Platform 与 adopter cutover

**阶段成熟度：** Outline；R4A关闭后通过独立`R4A -> R4B Implementation Resolution Gate`解析。当前不是
Ready write set或实施授权。

**目的：** 删除每架构bare/pretest machine复制，建立
`qemu-virt-{rv64,la64}-smp-{1,8}`四个hardware-semantic Platform；SMP1/SMP8分别使用
`max_phys_cpu_id = 0/7`，且都声明`kernel-image/disk-x0/disk-x1`三个DT-neutral bind。Business-semantic
SystemTarget/BuildPreset/wrapper表达pretest/final用途，Platform不保存workload语义。

**受保护边界：** R4A DT materialization、ordinary bind argv、physical Platform、KernelConfig
`max_logical_cpus`、root/initial-program与final harness仍由各自owner拥有。R4B resolution必须先核对final
target当前root/entry是否只是行为保持映射，并单独决定`-no-reboot`应作为统一QEMU execution policy还是
其它owner；不得把它继续当作SMP hardware identity。Rootfs manifest默认不因命名迁移改名。

**解析触发与证据：** R4A Closed后的live config/reference audit、两架构SMP1/SMP8 provider dump与boot
evidence、所有target/preset/wrapper consumer清单。若final root/entry需要真实EmbeddedApp/harness语义，
该adopter变化必须独立解析，不得借Platform rename宣称final contract已经闭合。
