...
**状态：** Implemented; accepted limitations tracked in register
**负责人：** doruche
**最后更新：** 2026-06-19
**领域：** fs / vfs / mount
**事务日志：** [2026-06-18-mount-tree-legacy-api](../../devlog/transactions/2026-06-18-mount-tree-legacy-api.md)
**开放问题：** 无 active design blocker；剩余 shared/slave/unbindable propagation、mount flag matrix、fstype alias bridge、ROFS mmap/writeback 和 unmount cleanup 均已登记为 accepted limitations。
**下一步：** 后续按 register/current limitations 分拆 follow-up gate；`mount-legacy` LTP group 保持宽覆盖以保留 TPASS 分数和观测面。
...
- `anemone-kernel/src/fs/mount/` 目录承载 mount view、legacy data、attrs flags 和 mount tree owner。`MountFlags` 迁移桥已删除；`RDONLY` 由 `Mount` 的 per-mount attrs 单一真相源承载，filesystem backend 不再接收 per-mount attrs。
...
- `MountFlags` 阶段迁移桥已删除；`Mount` 直接持有 `MountAttrFlags` 或等价 attrs storage，`FileSystemOps::mount()` 不再接收 per-mount attrs。
...
阶段 1/2 迁移期间曾允许把 `MountAttrFlags::RDONLY` 映射到旧 `MountFlags::RDONLY` 以复用旧 call path；阶段 3 已删除 `MountFlags` 作为并列 per-mount flag 类型。filesystem backend 不得通过 `FileSystemOps::mount()` 观察 per-mount attrs；backend 只处理 source、legacy `MountData` 和未来明确属于 superblock / filesystem instance 的配置。

fstype normalization 在 syscall adapter 内完成。`tmpfs -> ramfs` 这类 alias 可以作为明确的兼容入口；`ext2` / `ext3` / `vfat -> ramfs` 属于当前 LTP 兼容桥，必须有日志、文档化边界和退出条件。归一化后的内部请求不得让 `MountTree`、`MountAttrFlags`、`FileSystemOps::mount()` 或具体 filesystem backend 知道原始 alias。
...
本 RFC 第一版实现已按 [迁移实施计划](./implementation.md) 关闭阶段 1-7；执行事实、checkpoint、review 结论和验证证据记录在 [事务日志](../../devlog/transactions/2026-06-18-mount-tree-legacy-api.md)。已实现的用户可见成功语义仍受 [不变量需求](./invariants.md) 约束；未实现或未完整验证的 Linux 兼容能力已登记到 [current limitations](../../register/current-limitations.md)，后续应按 limitation 拆分 follow-up gate，而不是在本 RFC 中继续扩大第一版目标。

本 RFC 的实现反馈不能被用来削弱目标、不变量或验收边界。lazy-detach final cleanup、fanotify observer cleanup、full shared subtree propagation、mount flag matrix、fstype alias bridge 和 readonly mmap/writeback 等剩余能力已经分流到 register；若后续实现发现 accepted contract 本身错误，仍必须回写 RFC canonical 文本、登记 limitation / open issue，或停止当前 gate，而不是让弱语义静默成为实现结果。
...
- `MountTree` transaction 仍需要继续防止后续 feature 绕过 topology owner；lazy detach 后旧 `Arc<Mount>` 仍可能存活。
- `/proc/mounts` 顺序和路径显示仍会影响 LTP cleanup；第一版只提供最小六列 live view，不承诺 mountinfo / namespace 视图。
- exact errno matrix 仍需在 follow-up 中对照 LTP/Linux，尤其是未实现 flag、environment setup 和 unsupported data option。
...
第一版 legacy mount API 已关闭实现事务，当前收口状态为：

- `index.md`、`invariants.md`、`implementation.md` 对状态所有权、锁边界和 staged gates 保持一致。
