...
**下一步：** 完成公开 RFC review；进入实现阶段时建立 transaction devlog，然后从阶段 1 开始实现。
...
本 RFC 已提升为公开 Draft，并且其目录即为当前共享评审的 canonical source。接受本 RFC 意味着 legacy mount API 可以按 [迁移实施计划](./implementation.md) 分阶段推进，并且每个成功返回的用户可见操作都必须满足 [不变量需求](./invariants.md)。
