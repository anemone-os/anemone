...
`MountTree` 普通 topology writer 使用睡眠式 `tx_lock: Mutex<()>` 串行化，`MountTreeInner` 由 `inner: SpinLock<MountTreeInner>` 保护。第一版 topology 发布机制固定为单一 placement lock 加 writer gate，而不是 COW / snapshot-style publish。当前 repo 的 `RwLock` 是 spin-based，不适合作为可能扩展的 mount topology transaction lock。
...
anonymous fs initcall 的 first-root publish 是唯一 early-root 特殊能力：它必须通过显式 fs-private API 调用，且只允许在 root 尚未发布、没有普通 mount writer 能并发的启动窗口内使用。不得用 `can_sleep`、IRQ/preempt 状态或 panic 状态推断进入特殊路径；普通 root mount、attach、unmount、remount 和 move 必须走 `tx_lock`。
...
- 不得用 `can_sleep`、IRQ/preempt 状态或 panic 状态推断来绕过 `MountTree` writer gate；early-root publish 必须是显式、fs-private、boot-only capability。
