...
> **共享接口对齐（2026-07-14）：** [Sched RT Class R1](../sched-rt-class/index.md) 将 `PendingResched` 收窄为 core-only single bit，并从 preempted-current class transaction 删除 pending 参数。该变化对 Fair 只有 trait / caller 机械适配；tick pass charge、heap、placement floor、yield 和接受边界不变，因此不形成 Fair 算法语义修订。
...
当前 scheduler class graph 已落地 method-first `Scheduler` trait、`RunQueue` facade、class-specific `SchedEntity` payload、core-only typed `PendingResched` 和 capability-gated entity mutation。production precedence 为 `Realtime > Fair > Idle`；所有 fresh non-idle task 通过 `SchedEntity::new_default()`，再由编译期 `sched_default_policy = "fair" | "rt_rr" | "rt_fifo"` 选择 class / policy。
...
- 通过算法不变量关闭 fixed-set proportionality / progress，通过 production-path KUnit关闭 heap order、pass arithmetic、yield handoff 和 wake placement；通过用户运行 `fair-test` 与真实 LTP workload验证 production 接线和无整体回归。
...
如果 kernel preemption 延迟 full pick，后续每个 timer tick 仍各自推进一次 pass。scheduler-core pending slot 只是合并的 full-pick request，不承担 service accounting truth，也不向 Fair 传播 Tick cause；延迟消费不能丢失期间已经发生的 tick charge。
...
- `requeue_preempted_current()`：清除 current 后按现有 pass 入堆，最后按完整 post-state 刷新 floor，不额外收费。`task_tick()` 已完成对应 charge；cross-class arrival preempt 也不能伪造一个 Fair service unit。该 transaction 不读取 core pending snapshot。
...
- unequal-weight fixed-set service ratio和 eventual progress 的算法不变量；不使用独立重写 selector / pass loop 的 synthetic KUnit充当 production proof；
...
- Linux nice 极端 weight 比约 5917:1，低 weight task 的合法 service 间隔可能很长。控制方式是把它作为用户已接受的比例语义，以 fixed-set 算法不变量和 production-path KUnit分工关闭 eventual progress，不擅自压缩 weight 表，也不为测试重写 synthetic scheduler。
