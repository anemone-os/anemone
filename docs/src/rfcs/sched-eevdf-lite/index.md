...
- 使用 eligibility / `rq_vtime` 约束，避免第一版退化成 deadline-only 调度器；具体 `rq_vtime` 公式通过 Gate P2 probe 验证后回写。
...
- 为 eligibility fallback 记录 anomaly，保证异常路径可观测。
...
`request_resched(cause)` 合并 bit，而不是 last-writer-wins；`Tick` 不能被后续 runnable arrival 擦掉。trap / idle tail 通过 `take_pending_resched()` 或等价 API 取得 `PendingResched` 后进入 `schedule_preempt(pending)`。如果 preempt 因 `Waiting/PrePark` deferred，必须恢复同一组 pending bits，避免抢占请求被吞掉或被重新压成 generic bool。

`PendingResched` 是普通值语义 flags，不是 processor state capability。scheduler class 可以在 `requeue_preempted_current(task, now, pending)` 中读取它来区分 tick、runnable arrival 或二者同时存在，但 restore pending request 只属于 scheduler core 的 deferred-preempt 路径。
...
`rq_vtime` 是 eligibility 的公平时钟基准，不能只是 `min(vruntime)` 的别名。草案接受 Gate P2 在实现期验证具体公式，但公式必须说明：

- enqueue / dequeue / pick / `account_current(now)` 后的更新点。
- current task 被 pick 出 queue 后是否仍参与公平时钟。
- runnable set 变化如何影响 `rq_vtime`。
- 如何防止 deadline-only 偏置。
- no-eligible fallback 何时允许出现，何时视为 formula failure。
...
account_current(now):
    ...
    curr.vruntime += delta_exec * NICE_0_WEIGHT / curr.weight
    curr.exec_start = now
    if curr.vruntime >= curr.deadline:
        curr.deadline = curr.vruntime + slice / weight_normalized
    update rq_vtime
```
...
- `enqueue_new()`：只接受 fresh normal entity；如果没有有效 `vruntime`，放置到 `rq_vtime` 附近。
...
- 当前任务耗尽 virtual slice。
- 存在更早 eligible deadline 的 runnable task 值得抢占。

`sched_yield()` 使用 bounded penalty。第一版可以选择把 yielding task 的 deadline 后推到当前 runqueue 中较后的 eligible deadline 附近，或后推一个 base-slice virtual delta；不得直接改 nice / weight，不得把 `vruntime` 推到不可恢复的最差位置。验证必须覆盖 yielding task 不会立即无界选回，也不会永久饿死。
...
nice-to-weight 第一版采用固定 Linux 表，不提供 selector。若未来需要替换权重表，应单独走 follow-up。
...
2. `PendingResched` 已覆盖 tick 与 runnable-arrival 抢占来源，deferred preempt 会恢复同一组 pending bits，且 scheduler class 不接收 `ScheduleMode` 或 wait-core private identity。
