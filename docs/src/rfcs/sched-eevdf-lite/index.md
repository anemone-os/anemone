...
- 为 eligibility fallback 和 virtual-time saturation 记录 anomaly；每次记录都通过 `kerrln!` 输出 reason 和累计次数，连续 fallback 达到 Kconfig threshold 时额外输出 streak 摘要。anomaly 是 EEVDF-lite 本地诊断概念，不是 Linux / EEVDF 标准状态，也不得反向驱动调度决策。
...
- consecutive-fallback error-summary threshold
