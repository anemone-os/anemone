...
**下一步：** 如果后续要从 task-exit hint 加单阈值 gate 扩展到 allocator direct reclaim、LRU、限额或通用 shrinker registry，应新增 follow-up RFC。

## 摘要

本 RFC 记录 `2b0e3900279895c3d8eb604e463249a02c3bddc9` 中落地并在 2026-06-15 调整触发策略的 inode shrinker。它是当前 VFS 的第一版后台 inode cache 回收系统：task exit 提交合并请求，`inode-shrink-0` kthread 先根据物理页占用是否严格超过 `io_shrink_threshold` 判断是否需要扫描；超过阈值时才遍历可见 namespace 中的 mounted superblock，按 superblock cache snapshot 尝试回收不 busy 的 resident inode。

本文中的 shrinker 不是完整 generic shrinker 框架。它只覆盖 superblock resident inode cache、ext4 file-backed page cache 计数、显式 eviction path，以及 task-exit hint 上的单一物理内存占用阈值 gate；不管理 slab、不做通用 page cache shrink，也不把 task exit 次数当作必须逐一执行的 work item。
...
本次提交同时引入 [kthread](../kthread/index.md) 服务基础设施，因此 inode shrinker 可以作为第一个后台 consumer 落地：它把 task exit 视为回收 hint，把重复请求合并成一个 pending slot，并在 worker 中根据当前物理页占用决定是否执行可中断的 superblock/inode 扫描。
...
- 让 `io_shrink_threshold` 进入 kconfig，默认 50；task exit 只提交 hint，worker 在物理页占用严格超过阈值时才执行扫描。
...
task exit 路径调用 `submit_inode_shrink_request()`。如果 shrinker 尚未初始化，提交静默忽略；如果 service 正在 stopping，也静默接受停止状态。worker 消费 hint 后读取 `frame_allocator_stats()`，只有 `used_pages * 100 > total_pages * io_shrink_threshold` 时才执行 `shrink_inodes()`；低于或等于阈值时直接返回，不保留额外 backlog。默认阈值是 50。真正扫描时，worker 遍历 `mounted_superblocks()` 返回的 visible namespace superblock。每个 superblock 先按 flags 判断是否允许 shrink，再调用 `cached_inode_snapshot(include_indexed)` 获取候选列表。
...
- `io_shrink_threshold` 是否仍是 worker 内部的严格大于百分比 gate；
...
- worker 只在 task exit hint 到来时采样物理内存占用；没有新的 exit hint 时不会主动 reclaim。后续如果需要 timer、allocator OOM direct reclaim 或 LRU/priority reclaim，应作为 follow-up 设计。
