...
candidate。Stage 1 -> Stage 2 Resolution Gate现已完成，Stage 2的四个checkpoint、完整路线、验证和
resolved manifest已经冻结为Ready；该解析不授权实现，也没有current contract因此生效。
...
- 实现激活前重新记录branch、HEAD、dirty state、active platform、本节manifest以及BusyBox staging状态。
  当前resolution输入中的RV64 glibc artifact是BusyBox 1.33.1、RISC-V double-float、static ELF，SHA-256
  `1ef4811837a8abdfe717db94d3a9c4e518233227ed0bb0cfe358b24d625664bd`，并已通过可执行`--list`核对
  `ash`、`stty`、`vi`；LA64 glibc static artifact的SHA-256为
  `52fe6e248922e345cbd46543690a7e7c9000dc87c9d87cec3f0e7162b2f2ef94`，applet/runtime仍须由相应wrapper
  在使用前核对。artifact和测试盘master始终只读，不提交仓库，也不在公共接口中记录个人挂载路径。
...
**目的与交付：** 新增单一`tty-test` app及其最小launcher、两份TTY rootfs manifest、稳定BusyBox staging说明和
RV64/LA64 repository wrapper；完成自动data-plane/termios/readiness/devfs/boot matrix、BusyBox `stty`/`vi`
...
`anemone-rs/src/{sys/linux.rs,os/linux.rs}`、`conf/rootfs/tty-acceptance-{rv64,la64}.toml`、
`conf/rootfs/tty-acceptance.md`、`scripts/run-tty-test-{rv64,la64}.sh`；本Stage全部kernel文件仅允许修复本阶段
...
**自动matrix：** 两架构都必须验证`/dev/ttyS0`为4:64、`/dev/console`为5:1、boot三fd为real shared Terminal；
...
`VERASE/VKILL/VEOF`检查可见编辑与read boundary。LA64重复自动matrix；若要用LA64 vi作为cutover证据，必须先
在wrapper内完成version/applet核对并记录独立结果。人工观察不能替代自动matrix。

**Review / validation floor：** `git diff --check`、`just fmt --check`、
`just app build tty-test --arch riscv64`、`just app build tty-test --arch loongarch64`；随后分别运行
`./scripts/run-tty-test-rv64.sh --busybox <rv64-busybox> --sdcard <rv64-master> --mode auto --log
build/tty-stage2-rv64.log`和`./scripts/run-tty-test-la64.sh --busybox <la64-busybox> --sdcard <la64-master>
--mode auto --log build/tty-stage2-la64.log`，再以同一RV64输入执行`--mode vi`人工checklist。两份rootfs和
kernel build由wrapper按7.5 harness contract完成；最后执行`mdbook build docs`和contract/link/manifest/
bypass audit。必须区分source/KUnit/build、
agent-run QEMU、user-run vi与Not Run；任一架构自动matrix、RV64 vi、artifact identity或proof不足时不cutover。
...
- `TTY-DATA-CUTOVER`只在四个checkpoint均关闭、两架构自动matrix与RV64人工vi达到floor后执行；它原子创建
...
- 新建`anemone-apps/tty-test/**`、`conf/rootfs/tty-acceptance-{rv64,la64}.toml`、
  `conf/rootfs/tty-acceptance.md`、`scripts/run-tty-test-{rv64,la64}.sh`；
...
- BusyBox source/config、用户提供的RV64/LA64 artifacts、测试盘master和mount均只读；只把版本、架构、hash、
...
- implementer一次只激活一个checkpoint并按7.5子集追加transaction；reviewer依次审查discipline/worker、
  ...
  双架构、artifact、bypass与contract atomicity审计。任何角色都不能把Ready或长期scope envelope当作执行授权。
