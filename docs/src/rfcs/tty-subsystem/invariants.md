...
`TTY-DATA-CUTOVER` 与 `TTY-JOBCTL-CUTOVER` 是 prospective 语义切换单元，不是当前已经获准执行的 implementation stage。[迁移实施计划](./implementation.md)负责按滚动gate解析具体stage、write set、验证floor、停止条件和transaction evidence；在对应cutover完成前，下表中的`Introduce`项仍不是current contract。

| Contract ID | 变化 | 当前规则 | Target 摘要 | 生效 Gate |
| --- | --- | --- | --- | --- |
| `TTY-PORT-001` | Introduce | None（尚未生效） | NS16550A 固定提供 console + `TtyPort`、不注册 raw `CharDev`；port 唯一拥有硬件、TTY RX 与 boot-applied configuration | `TTY-DATA-CUTOVER` |
| `TTY-TERM-001` | Introduce | None（尚未生效） | 同一 endpoint 的 open file 共享唯一 terminal semantic truth | `TTY-DATA-CUTOVER` |
| `TTY-INPUT-001` | Introduce | None（尚未生效） | RX handoff、discipline、read 与 readiness 保持有序、有界、无 lost wake 且不复制 input truth | `TTY-DATA-CUTOVER` |
| `TTY-OUTPUT-001` | Introduce | None（尚未生效） | TTY 输出是 byte stream；console/TTY 共用 UART 时由 driver 唯一序列化 TX | `TTY-DATA-CUTOVER` |
| `TTY-ENDPOINT-001` | Introduce | None（尚未生效） | serial endpoint 在完整初始化后单向发布，名称与 identity 稳定到重启 | `TTY-DATA-CUTOVER` |
