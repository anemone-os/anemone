...
**影响契约：** Gate 1 已 Introduce `TIMEKEEPER-CLOCK-001`；R0 其余 target 仍提议 Introduce
`TIMEKEEPER-STEP-001`、`SOFT-TIMER-REQUEST-001`、`POSIX-TIMER-001`，并 Refine
`SIGNAL-PENDING-001`
...
以下是 R0 contract delta。Gate 1 已完成唯一一项 cutover；其它条目在对应 gate 前仍不覆盖当前契约：
...
| `SOFT-TIMER-REQUEST-001` | Introduce | None（尚未生效） | soft timer 只拥有一次排队请求，支持按句柄物理删除；长期对象状态仍由提交者拥有 | Gate 2 `ST-REQUEST-CUTOVER` |
...
R0 已接受 target、owner、ABI 与 validation boundary。Gate 0--1 已关闭，`TC-CLOCK-CUTOVER` 已激活
`TIMEKEEPER-CLOCK-001`；Gate 2 是下一个已授权 checkpoint，其余 contract delta 保持 pending。RFC 最终
