...
**状态：** Accepted for Implementation / Stage 0-1 Closed / Stage 2 Ready / Not Authorized / Epoll Not Effective
**修订：** R0
**负责人：** doruche
**最后更新：** 2026-07-26
...
**下一步：** 等待独立授权后才能激活 Stage 2；Ready 不授权实现或 `EPOLL-CUTOVER`。
...
Ready / Not Started，但没有激活代码或 cutover；三个 `EPOLL-*` target ID 仍未生效。
...
[Stage 2 Ready](./implementation.md#stage-2-readyepoll-coreabi-与最终-cutover)，当前等待独立实现授权。
...
解析为 Ready / Not Started。影响 owner、ABI、可见语义或接受边界的反馈仍必须回到
RFC review。
...
[实施计划](./implementation.md#stage-2-readyepoll-coreabi-与最终-cutover)；Stage 2 实现仍未授权，
