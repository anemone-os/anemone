...
**下一步：** R1 的 Stage 0 至 Stage 4 checkpoint 已关闭并记录在事务日志；Stage 5 / `UJ-CUTOVER` 保持 Not Started。candidate 仍为 non-publishable，cutover 前不更新 effective contract。
...
本目录是公开 RFC 的 canonical target source。R1 acceptance 只接受 target、实施边界和验证计划；它不更新 effective contract。各Stage的执行事实、review、验证和 checkpoint 由事务日志记录。
...
当前为 R1 Accepted for Implementation。逻辑数据模型与首选实现基线已经形成，但具体字段布局、owner-private API、container、锁实现和后续 implementation stage 仍允许在上述 target 边界内根据源码与 probe 证据调整。Stage 0 行为保持型 Signal module split、Stage 1 dormant ThreadGroup/user-entry foundation、Stage 2 integrated production vertical slice、Stage 3A conditional control closure、Stage 3B multi-member lifecycle / topology closure与Stage 4 validation closure均已关闭；candidate在`UJ-CUTOVER`前保持non-publishable，current effective rule不变。Stage 5保持Not Started，不能从Stage 4 closure自动进入。
...
| R1 | 2026-07-21 | Accepted for Implementation | 保留current reservation-first顺序：旧reserved `SIGCONT`可将后来pending的`SIGKILL`延迟到下一次mandatory kernel entry；不削弱Stopped gate、terminal lifecycle precedence或SIGKILL pending ownership。 | [R1修订与Stage 3A收口](../../devlog/transactions/2026-07-20-unix-jobctl.md#stage-3a-r1-target-correction---2026-07-21) |
