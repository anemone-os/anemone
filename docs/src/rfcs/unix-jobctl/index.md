...
**下一步：** R0 的 Stage 0 split-only checkpoint 已关闭并记录在事务日志；Stage 1 仍未开始。`UJ-CUTOVER` 前不更新 effective contract。
...
本目录是公开 RFC 的 canonical target source。R0 acceptance 只接受 target、实施边界和验证计划；它不更新 effective contract。Stage 0 的执行事实、review、验证和 checkpoint 由事务日志记录。
...
当前为 R0 Accepted for Implementation。逻辑数据模型与首选实现基线已经形成，但具体字段布局、owner-private API、container、锁实现和后续 implementation stage 仍允许在上述 target 边界内根据源码与 probe 证据调整。Stage 0 行为保持型 Signal module split 已关闭；尚未发生 `UJ-CUTOVER`、runtime job-control 语义或 current effective rule 变化，Stage 1 仍未开始。
