...
**下一步：** R0 的 Stage 0、Stage 1 与 Stage 2 checkpoint 已关闭并记录在事务日志；Stage 3A 未开始且未获授权。Stage 2 candidate保持non-publishable，`UJ-CUTOVER` 前不更新 effective contract。
...
本目录是公开 RFC 的 canonical target source。R0 acceptance 只接受 target、实施边界和验证计划；它不更新 effective contract。Stage 0、Stage 1 与 Stage 2 的执行事实、review、验证和 checkpoint 由事务日志记录。
