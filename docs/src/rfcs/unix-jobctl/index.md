...
**事务日志：** None；公开 RFC promotion 已完成，尚未进入实现。
...
**下一步：** 按本 RFC 与[迁移实施计划](./implementation.md)完成公开 target review；接受前不得创建 transaction、更新 current contract pending successor 或进入 S0 代码实现。
...
本 Draft 相对已经提取的 current contract 描述 target delta；current behavior 继续以仓库 `docs/src/contracts/` 为准。本目录不是 effective authority。
...
本目录是公开 RFC 的 canonical target source。公开 promotion 不代表 target 已接受、实现已开始或 current contract 已 cut over；implementation plan 仍不授权 transaction 或代码修改。
...
当前为公开 Draft，逻辑数据模型与首选实现基线已经形成，但具体字段布局、owner-private API、container、锁实现和 implementation stage 仍允许在上述 target 边界内根据源码与 probe 证据调整。尚未运行实现验证、QEMU 或 LTP，未建立 transaction，也未改变 live effective semantics、current contract pending successor 或 register 状态；current contract 页只记录经源码核验的既有规则。
