...
**下一步：** R1 的 Stage 0、Stage 1、Stage 2、Stage 3A 与 Stage 3B checkpoint 已关闭并记录在事务日志；Stage 4保持Not Started。candidate保持non-publishable，`UJ-CUTOVER` 前不更新 effective contract。
...
当前为 R1 Accepted for Implementation。逻辑数据模型与首选实现基线已经形成，但具体字段布局、owner-private API、container、锁实现和后续 implementation stage 仍允许在上述 target 边界内根据源码与 probe 证据调整。Stage 0 行为保持型 Signal module split、Stage 1 dormant ThreadGroup/user-entry foundation、Stage 2 integrated production vertical slice、Stage 3A conditional control closure与Stage 3B multi-member lifecycle / topology closure已关闭；candidate在`UJ-CUTOVER`前保持non-publishable，current effective rule不变。Stage 4保持Not Started。
