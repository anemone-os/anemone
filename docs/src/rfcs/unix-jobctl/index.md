...
**下一步：** R1 Stage 0至Stage 5与`UJ-CUTOVER`已关闭；TTY、orphaned-pgrp、ptrace、stopped/continued `si_uid = 0`和SIGCHLD publication-order限制按register与独立follow-up边界推进，不自动进入下一RFC。
...
本 R1 target 相对已经提取的 current contract 描述 target delta；current behavior 继续以仓库 `docs/src/contracts/` 为准。本目录不是 effective authority。
...
- 不改变temporary-mask classifier已经claim的reserved target相对后来pending signal的既有优先级；尤其不要求本RFC为后来pending的`SIGKILL`越过该reservation新增跨owner最终仲裁。
...
本目录仍是R1 accepted target与历史delta的canonical source；Stage 5之后的effective规则以current contracts为准。各Stage执行事实、review、验证和`UJ-CUTOVER`证据由事务日志记录。
...
reserved `SIGCONT`仍按action-selection时的live disposition处理。它可以在`Stopping / Stopped`期间提交custom handler frame，或通过ignore / default no-frame consume完成temporary-mask cleanup；但frame、trapframe和mask commit都不携带user-entry permit，最终architecture transition仍必须被live jobctl gate阻止。一次reserved retirement结束当前ordinary scan，不能顺带消费其它普通异步信号。handler frame构造失败继续服从既有no-return terminal path；已经提交的terminal lifecycle即使在frame提交后到达也必须在user entry前支配。jobctl park以live phase为durable predicate，在wait publication前后重验，避免与`SIGCONT` / terminal transition丢wake。`SIGSTOP`不需要走该fetch/action-selection路径；任意`SIGCONT` occurrence的后续action都不得重放generation-time resume。
...
R1进一步保留current `SIGNAL-TEMP-MASK-002`的reservation-first顺序：若旧reserved `SIGCONT`与后来pending的`SIGKILL`并存，reserved action可以先retire。只有真实的新`SIGCONT`已经把phase恢复为`Running`时，custom handler frame才可能通过live gate执行用户指令；pending `SIGKILL`不会丢失、被屏蔽或转移，并在下一次mandatory kernel entry重新仲裁。该延迟原则上可由handler副作用观察，且不承诺固定时间上界，但不允许绕过`Stopping / Stopped` gate或已经提交的terminal lifecycle。本RFC接受该既有Signal优先级，不为它扩大jobctl owner或建立新的final-arbitration协议。

## Contract Impact

完整逐ID历史delta、current落点与`UJ-CUTOVER`结果见[目标与不变量](./invariants.md#contract-impact)。Stage 5已经把job-control core、user-entry closure、child report/wait ABI、SIGCHLD和procfs projection作为一个integrated unit写入current contracts；不存在partial cutover。
...
- 保留current reservation-first顺序；旧reserved `SIGCONT`可以把后来pending的`SIGKILL`递送延迟到下一次mandatory kernel entry，但不能使SIGKILL丢失、绕过Stopped gate或覆盖已经提交的terminal lifecycle；
...
R1已实现并Closed。Stage 0行为保持型Signal module split、Stage 1 dormant foundation、Stage 2 integrated slice、Stage 3A conditional control、Stage 3B multi-member lifecycle/topology、Stage 4 repair/validation与Stage 5 current-contract switch全部关闭；`UJ-CUTOVER`作为单个integrated unit生效。实现物理shape仍可在不改变current contract的前提下做owner-local维护；任何owner、ABI、visible semantics、accepted limitation或共享contract变化都需要新的RFC/change workflow。
...
| R1 | 2026-07-21 | Implemented / Closed | 保留current reservation-first顺序：旧reserved `SIGCONT`可将后来pending的`SIGKILL`延迟到下一次mandatory kernel entry；不削弱Stopped gate、terminal lifecycle precedence或SIGKILL pending ownership。 | [R1修订与Stage 3A收口](../../devlog/transactions/2026-07-20-unix-jobctl.md#stage-3a-r1-target-correction---2026-07-21)、[Stage 5 cutover](../../devlog/transactions/2026-07-20-unix-jobctl.md#stage-5-uj-cutover与事务收口---2026-07-21) |
