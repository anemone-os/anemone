...
**下一步：** 阶段 1A / 1B / 2、signal-owned classifier / stable handoff、`rt_sigsuspend`、`ppoll` / `pselect6` typed outcome 迁移与 `rt_sigtimedwait` 本地 waited-set dequeue 边界修复已完成，并通过 Gate 1 / Gate 2 / Gate 3 / Gate 4 / Gate 5 / Gate 6 / Gate 7 review；下一步按 [Agent 编排建议](./backgrounds/agent-orchestration.md) 启动 Agent 8 的旁路审计、验证证据整理和事务收口。
