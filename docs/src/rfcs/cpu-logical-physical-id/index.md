...
**开放问题：** None
**下一步：** None。后续若需要 CPU hotplug 或任意高位稀疏 hart ID，另走 RFC review。
...
- 通过槽位内建 `CachePadded<T>` 的 `CpuTable` / `PhysCpuTable` 把 per-CPU 静态表的索引身份和缓存布局编码进类型。
...
`device/cpu.rs` 拥有 `CpuTable<MonoOnce<PhysCpuId>>` 槽位、逻辑 CPU 原子计数和封存标志。`CpuTable` 内部为每个元素保存 `CachePadded<T>`，索引仍直接返回 `T`。early scan 期间只有 BSP 调用 `register_cpu()`：槽位先完成一次初始化，再推进已初始化前缀长度；`finish_cpu_registration()` 最后以 Release 写封存标志，运行期读者以 Acquire 检查封存后才能读取前缀。registry 不需要锁，也不在启动期分配堆内存。registry 封存后：
...
`CpuTable<T, const N: usize = MAX_LOGICAL_CPUS>` 只实现 `CpuId` 索引，`PhysCpuTable<T, const N: usize = { MAX_PHYS_CPU_ID + 1 }>` 只实现 `PhysCpuId` 索引；两者都直接保存 `[CachePadded<T>; N]`，但索引仍返回 `T`。registry、`PERCPU_BASES`、两架构 `STACK0` 与 `GUARDED_STACK_TOPS` 都使用默认容量。两种 table 允许显式覆盖 `N`，构造和索引函数都内联；`PhysCpuTable` 保持 transparent layout。`STACK0` 另外用编译期断言证明 `RawKernelStack` 大小等于汇编使用的栈步长，且 `CachePadded<RawKernelStack>` 不改变该大小。
...
入口汇编继续用固件物理 ID 选择 `PhysCpuTable<RawKernelStack>` 中的 `STACK0[physical_id]`。`RawKernelStack` 自身 4 KiB 对齐且大小为整页倍数，因此外层 64-byte `CachePadded` 不增加元素步长；两架构的编译期断言保护该汇编 ABI。`remap_boot_stack()` 遍历注册的逻辑 CPU，通过映射取得其物理 ID，并为同一物理栈 backing 建立 guarded alias；`GUARDED_STACK_TOPS` 同样使用内建 padding 的 `PhysCpuTable`。
...
- 固定 per-CPU 表必须按索引身份使用逻辑 `CpuTable` 或物理 `PhysCpuTable`；两者的每个槽位都必须内建 cache padding，调用点不能手工选择裸长度常量或再次嵌套 `CachePadded<T>`。
...
代码迁移、静态无锁 registry、物理/逻辑容量拆分和 typed per-CPU tables 已实现，执行事实见事务日志。用户提供的 VisionFive 2 panic 证明旧 `MAX_CPUS` 同时充当逻辑数量与物理数组长度会在 `PhysCpuId(3)` 索引长度 3 的 guarded-stack 表时越界；容量拆分和 wrapper 修正后，用户已确认同一 VisionFive 2 运行路径通过。逻辑和物理表最终统一为槽位内建 cache padding 的 `CpuTable` / `PhysCpuTable`，避免调用点用嵌套泛型隐藏布局；本次最终布局调整与 LoongArch correction build 按用户要求未由 agent 运行测试，不作为本 RFC 的关闭阻塞项。
