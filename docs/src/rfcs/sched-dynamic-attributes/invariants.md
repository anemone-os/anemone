...
14. `Arc<SchedRequest>` 只有一个可 `take()` 的 request body；transport clone不能复制 mutation或completion capability。
...
逐 syscall 的事实表与 testcase mapping见 [Linux 6.6 Scheduler UAPI Matrix](./backgrounds/linux-6.6-sched-uapi-matrix.md)。以下 phase order属于本 RFC Draft 的 binding proposal，不是implementation建议。
...
`Arc` transport envelope 不得让 `Sender<T>` 语义变成 multi-producer。多个引用可以保活 request，但只有唯一执行路径可以消费 producer capability。
...
receive wait adapter 固定为 `Latch`，不使用 `Event`。当前 `Event::listen_uninterruptible()` 对 wait-core `Force` 只重试predicate，不向caller暴露outcome，因此无法产生 `RecvError::Forced`；Event的可复用listener queue、exclusive/non-exclusive与quota policy也不属于single-consumer one-shot。不得为了本channel扩展Event outcome API。constructor dormancy由persistent channel phase与receive-time registration保证，不依赖选择哪一种wait adapter。
...
4. 若value/closed已在fast path与registration之间发布，不安装trigger；receiver必须cancel + finish刚建立的latch，再走terminal no-switch path。
5. trigger成功安装后才允许schedule；wake后先finish latch，再以Acquire检查channel phase。
6. 普通channel trigger后若phase仍empty是常开断言级bug；wait-core force是唯一允许“wait已完成但channel仍非terminal”的路径。

这组check -> begin -> recheck/register关闭三种race：send-before-receive、send-between-begin-and-register、send-after-register。channel value本身是持久结果，不依赖trigger保存跨时间permit。
...
- `recv_uninterruptible()` 消耗receiver；一旦它创建latch，normal value、sender closed、force与内部错误路径都必须在返回前cancel/finish该轮，不能触发`Latch` drop assertion。
- receiver 已关闭时 `send(self, value)` 返回原 value，不写入 abandoned slot。
- 普通信号不能完成uninterruptible receive。
- wait-core force可以先于channel terminal phase结束wait round；force winner关闭receiver、exactly-once drop已发布或随后竞争到的payload，并返回`RecvError::Forced`，scheduler syscall不得把它映射为普通成功或普通`EINTR`。
...
- `IpiPayload::SchedulerRequest(Arc<SchedRequest>)` 是 transport envelope，不是 scheduler state。
...
- force winner必须先关闭receiver、detach trigger并finish Latch，再返回和释放gate。旧request envelope随后仍可留在IPI queue或handler中，但late send只能观察receiver closed，不能再触发placement；不得据此断言所有时序下物理上最多一个request envelope in flight。
...
- `REMOTE_SCHED_REQUEST_GATE` 的具体类型是 `sched/api` 私有的全局 `Mutex<()>`；现有Mutex已经以CAS fast path和内部Event slow path实现sleepable竞争，本RFC不增加自定义`AtomicBool + Event` gate。Mutex内部Event只用于request发布和Latch建立前的不可中断lock acquisition，不需要暴露Force outcome；取得guard后不再进入该Event wait。gate只串行remote request的submit-to-receive-return窗口，不保护scheduler state。
...
- gate guard刻意跨越one-shot receive park持有；这是合法的唯一前置guard。其安全性依赖receive内不再获取sleepable lock、IPI handler/completion path不获取gate，以及receive先finish自己的Latch再返回并drop guard。
...
- 一旦进入`recv_uninterruptible()`，normal value、sender closed与force路径都由receive在返回前cancel/finish自己创建的latch；调用侧只在receive返回后释放gate。
...
- result 为 `ESRCH`。
- request-held Arc 不改变 zombie 语义。
...
- 为one-shot改造`Event`使其暴露Force outcome，或在现有`Mutex<()>`之外重复实现`AtomicBool + Event` remote gate。
...
- remote request gate保证仍有开放receiver并可能触发placement的published request count不超过一，且local/getter路径不依赖该gate；
...
- 决定性one-shot KUnit覆盖send-before-receive、send-between-latch-begin-and-trigger-registration、send-after-registration、sender-drop-before-receive、receiver-drop-before-send、force winner、closed-receiver late send与payload exactly-once drop；role matrix、payload transition、placement、permission与affinity由各自focused test覆盖；
- SMP=2 runtime覆盖两个CPU并发互调调度属性时被remote gate串行，且不存在两个仍有开放receiver、completion仍可能触发wait-core placement的published remote request；Force释放gate后的旧closed-receiver envelope允许尚未执行完，其late completion不能触发placement由one-shot KUnit与request/guard source review证明，不要求不稳定的user-space Force smoke；
