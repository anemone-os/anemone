...
本 RFC R1 还定义 fixed-CPU 架构下的 affinity 兼容语义：真实保存 allowed CPU mask，但只有包含 task 当前固定 `cpuid` 的 mask 才能成功；需要迁移的请求返回 `EINVAL`。主动迁移、load balancing 和 CPU hotplug 不在第一版范围内。
...
| `sched/request` | sched | local/remote submission、remote gate、single-owner request lifecycle 与 completion |
| `sched/api` | sched | 全部调度 UAPI 的解析、目标选择编排、permission snapshot、ABI 投影与 multi-target result folding |
| `task/api/clone` | task | clone 生命周期；通过 sched 的 unpublished-task 接口请求继承或 reset 后的新实体 |
| `exception/ipi` | exception | request transport 与 owner request execution 转发；不解释 patch 或修改 entity |
...
未知 policy、非法 policy/parameter 组合和不支持的 `sched_attr` feature 不得静默映射到 Fair 或 RT。逐 syscall layout、size、pointer ordering、errno precedence 与 LTP/POSIX evidence 见 [Linux 6.6 Scheduler UAPI Matrix](./backgrounds/linux-6.6-sched-uapi-matrix.md)；下列规则是 R0/R1 binding ABI boundary。
...
R0/R1 的 syscall-level `flags` 只接受 0；`sched_attr.sched_flags` 只接受 `SCHED_FLAG_RESET_ON_FORK`。reclaim、deadline overrun、keep-policy、keep-params、util-clamp 和未知 flags 全部返回 `EINVAL`。特别地，R1 仍不支持 `SCHED_FLAG_KEEP_PARAMS`：它需要新的 owner-side “replace policy while keeping latest parameters” semantic patch，不能由 syscall 读取 stale snapshot 拼出完整 config。
...
`Box<SchedRequest>` 唯一拥有 transport request；业务 body 也只有一份：
...
handler 在进入 transaction 前 `take()` body；`Some -> None` 是 request execution capability 的唯一消费点。`IpiPayload` 不提供通用 `Clone`，request payload不能产生第二个owner；body slot继续防御duplicate execute、空 body或double complete，这些都是常开断言级kernel bug。
...
- `IpiPayload::SchedulerRequest(Box<SchedRequest>)` 不允许 broadcast，`IpiPayload` 不提供包含该variant的通用clone能力。
- broadcast只显式复制允许广播的payload；把scheduler request交给broadcast是内核调用错误，必须在任何allocation/enqueue前断言失败，不能伪装成可恢复`IpiError`。
- `Box` 表达request envelope的唯一所有权并保持payload尺寸有界；single-use body slot继续是execute/complete capability的唯一消费点。
