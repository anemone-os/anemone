...
**下一步：** 先 review 本 RFC 的边界、不变量和实施顺序；通过后按 [迁移实施计划](./implementation.md) 建立 VFS ioctl 分发和通用块设备 ioctl。
...
`sys_ioctl()` 只负责 fd 查找、用户指针 ABI 边界和最外层兼容。它可以读取调用者的 `FileDesc`，但不能把 `FileDesc`、`ProcFile`、当前 task 或文件表对象继续传入 VFS/设备层；分发前必须把这些事实压缩成短生命周期、值语义的 `IoctlCtx`，再交给打开文件的 `FileOps::ioctl`。`IoctlCtx` 只携带目标 fd 的访问能力快照、用户空间 copy helper，以及受控的 fd 参数解析能力，供 `LOOP_SET_FD` 这类 ioctl 使用。
...
Linux ioctl 常量、结构体布局、用户指针读写应集中在 syscall 或设备文件 ABI 边界。`FileOps::ioctl` 可以按现有 vtable 风格接收 `&File`，但不能接收裸 `cmd`/`arg` 后自行回到当前 task 文件表，也不能接收 `Arc<FileDesc>` 或其他 task/fd 层对象；它接收由 `sys_ioctl()` 构造的 `IoctlCtx`：
...
- copy-in/copy-out helper，用于读写用户态 `loop_info`、`loop_info64`、`loop_config` 和块设备查询输出。
- 受控的 arg-fd 解析 helper，用于 `LOOP_SET_FD` 从调用者文件表取得 backing file，并只返回窄化后的 `IoctlArgFile` / `BackingFileHandle` 与访问能力快照，不暴露 raw fd、`FileDesc` 或文件表。
