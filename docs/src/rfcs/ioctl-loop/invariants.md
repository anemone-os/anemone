...
- VFS ioctl、统一 block ioctl、loop device pool 和 loop private ioctl 已按同一 owner boundary 实现并合入主线，阶段 review 未保留 Apollyon / Keter blocker。
- rv64 LTP 运行证据确认 built-in loop discovery 已找到 `/dev/loop0`；后续失败推进到本 RFC 非目标中的 sysfs / partscan 与测试环境层。
- 完整手工 mkfs/mount smoke 的原始日志未保存在本事务中；2026-07-22 用户确认第一阶段早已完成。剩余扩展能力已登记在 register，不阻塞本 RFC 关闭。
