...
本页定义 Anemone 跨 RFC、跨模块长期生效的不变量如何组织和维护。契约层只保存当前已经生效的共享语义；RFC 保存一次变化的目标、理由、contract delta 和迁移证明；实际 cutover 证据可以来自原子 change record、RFC closure、Git/PR 或按需 transaction。

契约层不是全仓库不变量百科。既有 RFC 不批量迁移；只有当后续 RFC 第一次复用、扩展或替换某条既有共享不变量时，才提取本次变化所需的最小 contract 闭包。

可复制的文档形状见 [当前契约模板](./contract-template.md)。开发如何声明和切换 contract delta，见[开发工作流](./development-workflow.md#current-contract-与-rfc-target)。
...
1. 找出本 RFC 直接 Introduce、Refine、Replace、Remove 或 Scoped Exception 的共享规则；未变化依赖只链接，不登记 `Preserve`。
...
- 文档头固定给出 contract ID、状态、owner、参与领域、覆盖范围、不覆盖范围、实现位置、依赖和最后核验。
...
RFC 的 `index.md`（以及按需 `invariants.md`）保存 target delta 和 proof obligations，但不维护整个领域的 current consolidated contract：

1. `Contract Impact`：按稳定 ID 声明 Introduce / Refine / Replace / Remove / Scoped Exception、目标摘要和生效 gate；未变化规则放在 `Dependencies` 中链接。
2. `Target Invariants`：精确定义尚未生效的新规则；证明内容较长时移到按需 `invariants.md`。
...
- [MM 当前契约](./contracts/mm/index.md)
  - [User Fault Local TLB Completion](./contracts/mm/user-fault-local-tlb.md)
