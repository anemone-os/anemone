...
mod tests {
    ...
    #[test]
    fn test_monoflow_basic() {
        let mf = unsafe { MonoFlow::new(42) };

        let v = mf.with(|data| *data);
        assert_eq!(v, 42);

        mf.with_mut(|data| *data += 1);
    ...
    #[test]
    /// This test will panic in debug builds due to reentrant access.
    fn test_monoflow_reentrancy() {
        let mf = unsafe { MonoFlow::new(42) };
