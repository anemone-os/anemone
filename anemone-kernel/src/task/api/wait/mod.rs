...
use anemone_abi::{
    process::linux::{resource::RUsage, wait},
    time::linux::TimeVal,
};
...
pub(super) fn wait_for_child_status(
    ...
    loop {
        ...
        if signal_interrupted {
            kdebugln!("wait: wait interrupted by signal");
...
pub(super) fn write_rusage(
    rusage: Option<VirtAddr>,
...
fn rusage_from_cpu(cpu_usage: ThreadGroupCpuUsage) -> RUsage {
    // The current stage-1 accounting only owns user/system CPU time. Keep all
    // other Linux rusage fields deterministically zero until their owners add
    // real accounting rather than guessing values at the wait ABI boundary.
    RUsage {
