...
This library is intended to work with operating system kernels, and thus panic immediately on any errors, since there is no way for kernel code to recover from them during early boot. Also, we assume that once the device tree is parsed and unflattened, it will last for the entire lifetime of the kernel.
