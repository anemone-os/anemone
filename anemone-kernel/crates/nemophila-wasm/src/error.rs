...
impl Error {
    ...
    /// Creates a new `Error` representing an explicit program exit with a
    /// classic `i32` exit status value.
    ///
    /// # Note
    ///
    ...
    /// Consumes `self` to downcast the [`Error`] into the `T: HostError` if
    /// possible.
    ///
    /// Returns `None` otherwise.
    #[inline]
...
pub enum ErrorKind {
    ...
    /// # Note
    ///
    /// This error kind is meant for internal uses only in order to resume a
    /// call after a host function trapped. This should never actually reach
    /// user code thus we hide its documentation.
    #[doc(hidden)]
    ...
    ///
    /// This error kind is meant for internal uses only in order to resume a
    /// call after a running out of fuel. This should never actually reach
    /// user code thus we hide its documentation.
    #[doc(hidden)]
...
impl ErrorKind {
    ...
    /// Returns a dynamic reference to [`HostError`] if [`ErrorKind`] is a
    /// [`HostError`].
