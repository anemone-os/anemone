...
impl ModuleParser {
    ...
    ///
    /// 1) Populate the [`ModuleParser`] with a [`Validator`] prior to calling
    ///    this method, OR;
    /// 2) Make sure that the provided `stream` yields valid WebAssembly
    ///    bytecode.
    ///
    ...
    ///   the Wasm module header is required for translating the Wasm code
    ///   section.
    ///
    /// # Errors
    ///
