...
use crate::{
    core::UntypedVal, error::ErrorKind, errors::MemoryError, func::WasmFuncEntity,
    memory::DataSegment, module::FuncIdx, value::WithType, AsContext, AsContextMut, ElementSegment,
    Error, Extern, ExternType, Func, Global, Instance, InstanceEntity, InstanceEntityBuilder,
    Memory, Ref, Table, Val,
};
...
impl Module {
    ...
    /// The given `externals` are joined with the imports in the same order in
    /// which they occurred.
    ///
    /// # Note
    ///
    /// This is a very low-level API. For a more high-level API users should use
    /// the corresponding instantiation methods provided by the [`Linker`].
    ///
    /// # Errors
    ///
    /// If the given `externals` do not satisfy the required imports, e.g. if an
    /// externally provided [`Func`] has a different function signature than
    /// required by the module import.
    ///
    ...
    /// Executes the [`Instance`]'s `start` function if any and finalizes
    /// instantiation.
    fn start(
    ...
    /// Extract the Wasm imports from the module and zips them with the given
    /// external values.
    ///
    /// This also stores imported references into the [`Instance`] under
    /// construction.
    ///
    /// # Errors
    ///
    /// - If too few or too many external values are given for the required
    ///   module imports.
    /// - If the zipped import and given external have mismatching types, e.g.
    ///   on index `i` the module requires a function import but on index `i`
    ///   the externals provide a global variable external value.
    /// - If the externally provided [`Table`], [`Memory`], [`Func`] or
    ///   [`Global`] has a type mismatch with the expected module import type.
    ///
    /// [`Func`]: [`crate::Func`]
    ...
    /// Extracts the Wasm functions from the module and stores them into the
    /// [`Store`].
    ///
    /// This also stores [`Func`] references into the [`Instance`] under
    /// construction.
    ///
    ...
    /// Extracts the Wasm tables from the module and stores them into the
    /// [`Store`].
    ///
    /// This also stores [`Table`] references into the [`Instance`] under
    /// construction.
    ///
    /// [`Store`]: struct.Store.html
    fn extract_tables(
    ...
    /// Extracts the Wasm linear memories from the module and stores them into
    /// the [`Store`].
    ///
    /// This also stores [`Memory`] references into the [`Instance`] under
    /// construction.
    ///
    /// [`Store`]: struct.Store.html
    fn extract_memories(
    ...
    /// Extracts the Wasm global variables from the module and stores them into
    /// the [`Store`].
    ///
    /// This also stores [`Global`] references into the [`Instance`] under
    /// construction.
    ///
    /// [`Store`]: struct.Store.html
    ...
    /// Evaluates the given initializer expression using the partially
    ...
    /// Extracts the Wasm exports from the module and registers them into the
    /// [`Instance`].
    ...
    /// Initializes the [`Instance`] tables with the Wasm element segments of
    /// the [`Module`].
    fn initialize_table_elements(
        ...
        for segment in &self.module_header().element_segments[..] {
            ...
            if let ElementSegmentKind::Active(active) = segment.kind() {
                ...
                //       fit into the table at the given offset but also that the element
                // segment       consists of at least 1 element member.
    ...
    /// Initializes the [`Instance`] linear memories with the Wasm data segments
    /// of the [`Module`].
