...
pub enum DataSegmentInner {
    /// An active data segment that is initialized upon Wasm module
    /// instantiation.
...
impl DataSegment {
    /// Returns the bytes of the [`DataSegment`] if passive, otherwise returns
    /// `None`.
...
pub struct DataSegments {
    ...
    /// to properly pre-reserve space for the bytes and thus finishing
    /// construction of the [`DataSegments`] would highly likely reallocate
    /// and mass-copy which we prevent by simply using a `Vec<u8>` instead.
    bytes: Vec<u8>,
}
...
pub enum InitDataSegment<'a> {
    Active {
        /// The linear memory that is to be initialized with this active
        /// segment.
        memory_index: MemoryIdx,
        /// The offset at which the data segment is initialized.
        offset: &'a ConstExpr,
