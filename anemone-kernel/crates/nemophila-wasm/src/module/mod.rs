...
impl Module {
    ...
    /// - If the Wasm bytecode violates restrictions set in the [`Config`] used
    ///   by the `engine`.
    ...
    /// Validates `wasm` as a WebAssembly binary given the configuration (via
    /// [`Config`]) in `engine`.
    ///
    /// This function performs Wasm validation of the binary input WebAssembly
    /// module and returns either `Ok` or `Err` depending on the results of
    /// the validation. The [`Config`] of the `engine` is used for Wasm
    /// validation which indicates which WebAssembly features are valid and
    /// invalid for the validation.
    ///
    /// # Note
    ///
    /// - The input `wasm` must be in binary form, the text format is not
    ///   accepted by this function.
    /// - This will only validate the `wasm` but not try to translate it.
    ///   Therefore `Module::new` might still fail if translation of the Wasm
    ///   binary input fails to translate via the Wasmi [`Engine`].
    /// - Validation automatically happens as part of [`Module::new`].
    ///
    /// # Errors
    ///
    /// If Wasm validation for `wasm` fails for the given [`Config`] provided
    /// via `engine`.
    ///
    /// [`Config`]: crate::Config
    ...
    /// # Note
    ///
    /// The returned iterator will yield no items if
    /// [`Config::ignore_custom_sections`] is set to `true` even if the
    /// original Wasm module contains custom sections.
    ///
    ///
...
/// Each [`ImportType`] describes an import into the Wasm module with the
/// `module/name` that it is imported from as well as the type of item that is
/// being imported.
#[derive(Debug)]
