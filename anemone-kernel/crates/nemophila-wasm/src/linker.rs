...
impl LinkerError {
    /// Creates a new [`LinkerError`] for when an imported definition was not
    /// found.
    ...
    /// Creates a new [`LinkerError`] for when an imported definition has an
    /// invalid type.
...
impl<T> Definition<T> {
    /// Returns the [`Extern`] item if this [`Definition`] is
    /// [`Definition::Extern`].
    ///
...
impl<T> Linker<T> {
    ...
    /// Configures whether this [`Linker`] allows to shadow previous definitions
    /// with the same name.
    ///
    /// Disabled by default.
    ...
    /// # Errors
    ///
    /// If there already is a definition under the same name for this
    /// [`Linker`].
    pub fn define(
        &mut self,
        module: &str,
    ...
    /// Creates a new named [`Func::new`]-style host [`Func`] for this
    /// [`Linker`].
    ///
    ...
    ///
    /// This method creates a host function for this [`Linker`] under the given
    /// name. It is distinct in its ability to create a [`Store`]
    /// independent host function. Host functions defined this way can be
    /// used to instantiate instances in multiple different [`Store`]
    /// entities.
    ///
    /// The same applies to other [`Linker`] methods to define new [`Func`]
    /// instances such as [`Linker::func_new`].
    ///
    /// In a concurrently running program, this means that these host functions
    /// could be called concurrently if different [`Store`] entities are
    ...
    ///   [`Linker`].
    /// - Returns `None` if the definition is a [`Linker`] defined host
    ///   function.
    ///
    /// # Panics
    ///
    /// If the [`Engine`] of this [`Linker`] and the [`Engine`] of `context` are
    /// not the same.
    pub fn get(
        &self,
    ...
    /// Convenience wrapper to define an entire [`Instance`]` in this
    /// [`Linker`].
    ///
    /// This is a convenience wrapper around [`Linker::define`] which defines
    /// all exports of the `instance` for `self`. The module name for each
    /// export is `module_name` and the field name for each export is the
    /// name in the `instance` itself.
    ///
    /// # Errors
    ///
    /// - If any item is re-defined in `self` (for example the same
    ///   `module_name` was already defined).
    /// - If `instance` comes from a different [`Store`](crate::Store) than this
    ///   [`Linker`] originally was created with.
    ///
    /// # Panics
    ///
    /// If the [`Engine`] of this [`Linker`] and the [`Engine`] of `store` are
    /// not the same.
    pub fn instance(
        &mut self,
    ...
    /// Instantiates the given [`Module`] using the definitions in the
    /// [`Linker`].
    ///
    /// # Panics
    ///
    ...
    pub fn instantiate_and_start(
        ...
        // Not clear that user can't import the same external lots of times to inflate
        // this.
...
pub struct LinkerInner<T> {
    ...
    /// which is why we do not use the interpreter's internal
    /// [`crate::collections::Map`] here.
    ///
...
impl<T> LinkerInner<T> {
    /// Configures whether this [`LinkerInner`] allows to shadow previous
