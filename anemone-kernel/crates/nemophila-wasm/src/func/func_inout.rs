...
impl<'a> FuncResults<'a> {
    ...
    /// # Panics
    ///
    /// If the number of results dictated by `T` does not match the expected
    /// amount.
    ...
    /// Encodes the results of the host function invocation given the `values`
    /// slice.
    ///
    /// # Panics
    ///
...
/// followed by
/// [`FuncResults::encode_results`] or
/// [`FuncResults::encode_results_from_slice`] at the end of their execution.
#[derive(Debug)]
...
impl<'a> FuncInOut<'a> {
    ...
    /// # Panics
    ///
    /// If the number of host function parameters and items in `values` does not
    /// match.
