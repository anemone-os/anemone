...
impl<T> HostFuncTrampolineEntity<T> {
    /// Creates a new host function trampoline from the given dynamically typed
    /// closure.
    pub fn new(
        // engine: &Engine,
        ty: FuncType,
    ...
    /// Creates a new host function trampoline from the given statically typed
    /// closure.
...
impl Func {
    ...
    ///
    /// This is typically used to create a host-defined function to pass as an
    /// import to a Wasm module.
    ///
    /// - `ty`: The signature that the given closure adheres to, used to
    ///   indicate what the inputs and outputs are.
    /// - `func`: The native code invoked whenever this Func will be called. The
    ///   closure is provided a [`Caller`] as its first argument which allows it
    ///   to query information about the [`Instance`] that is associated to the
    ///   call.
    ///
    /// # Note
    ///
    /// - The given [`FuncType`] `ty` must match the parameters and results
    ///   otherwise the resulting host [`Func`] might trap during execution.
    /// - It is the responsibility of the caller of [`Func::new`] to guarantee
    ///   that the correct amount and types of results are written into the
    ///   results buffer from the `func` closure. If an incorrect amount of
    ///   results or types of results is written into the buffer then the
    ///   remaining computation may fail in unexpected ways. This footgun can be
    ///   avoided by using the typed [`Func::wrap`] method instead.
    /// - Prefer using [`Func::wrap`] over this method if possible since
    ///   [`Func`] instances created using this constructor have runtime
    ///   overhead for every invocation that can be avoided by using
    ///   [`Func::wrap`].
    pub fn new<T>(
        mut ctx: impl AsContextMut<Data = T>,
        ty: FuncType,
    ...
    /// Verify that the `inputs` and `outputs` value types match the function
    /// signature.
    ///
    ...
    /// # Errors
    ///
    /// If the function signature of `self` does not match `Params` and
    /// `Results` as parameter types and result types respectively.
