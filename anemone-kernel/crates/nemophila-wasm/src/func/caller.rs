...
pub struct Caller<'a, T> {
    ...
    /// This usually is `None` if the host function was called from the host
    /// side.
    instance: Option<Instance>,
}

impl<'a, T> Caller<'a, T> {
    /// Creates a new [`Caller`] from the given store context and [`Instance`]
    /// handle.
    ...
    /// Returns the remaining fuel of the [`Store`](crate::Store) if fuel
    /// metering is enabled.
    ///
    ...
    /// Sets the remaining fuel of the [`Store`](crate::Store) to `value` if
