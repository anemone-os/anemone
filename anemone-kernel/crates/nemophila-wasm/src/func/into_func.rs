...
/// Types and type sequences that can be used as return values of host
/// functions.
pub trait WasmRet {
    #[doc(hidden)]
    type Ok: WasmTyList;
...
mod tests {
    ...
    /// Utility trait for the fallback case of the `implements_wasm_results`
    /// macro.
    pub trait ImplementsWasmRetFallback {
        const VALUE: bool = false;
    }
    impl<T> ImplementsWasmRetFallback for ImplementsWasmRet<T> {}
    /// Utility trait impl for the `true` case of the `implements_wasm_results`
    /// macro.
