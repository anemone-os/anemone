...
/// A reference to a compiled function stored in the [`CodeMap`] of an
/// [`Engine`](crate::Engine).
...
impl CodeMap {
    ...
    /// # Note
    ///
    /// Before using the [`CodeMap`] all [`EngineFunc`]s must be initialized
    /// with either of:
    ///
    ...
    /// Compile `func` or wait for result if another process already started
    /// compilation.
    ///
    /// # Errors
    ///
    ...
    /// Returns the [`CompiledFuncRef`] of `func` if possible, otherwise returns
    /// `None`.
    #[inline]
    ...
    /// Returns the [`UncompiledFuncEntity`] of `func` if possible, otherwise
    /// returns `None`.
    ///
    ...
    ///
    /// Thus any shared [`CompiledFuncRef`] can safely outlive the internal
    /// `Mutex` lock.
    #[inline]
    ...
    pub fn get_uncompiled(&mut self) -> Option<UncompiledFuncEntity> {
        ...
        match mem::replace(self, Self::Compiling) {
            ...
            _ => {
                ...
                //         Since this is a performance critical path we need to leave out this
                // check.
                unsafe {
...
pub struct UncompiledFuncEntity {
    ...
    ///
    /// This is `Some` if the [`UncompiledFuncEntity`] is to be validated upon
    /// compilation.
...
impl UncompiledFuncEntity {
    ...
    fn compile(
        ...
        /// # Note
        ///
        /// This fuel amount was chosen after extensive worst-case translation
        /// benchmarking.
        ...
        /// The amount of fuel required to validate and compile a function body
        /// per byte.
