...
/// - This structure is intentionally cheap to copy. Most of its API has a
///   `&self` receiver, so can be shared easily.
#[derive(Debug, Clone)]
pub struct Engine {
...
impl EngineWeak {
    ...
    ///
    /// Returns `None` if strong references (the [`Engine`] itself) no longer
    /// exist.
...
impl Engine {
    ...
    /// Returns `true` if both [`Engine`] references `a` and `b` refer to the
    /// same [`Engine`].
    ...
    ///
    /// Returns a range of [`EngineFunc`]s to allow accessing the allocated
    /// [`EngineFunc`].
    ...
    ///
    /// - Uses the internal [`Config`] to drive the function translation as
    ///   mandated.
    /// - Reuses translation and validation allocations to be more efficient
    ///   when used for many translation units.
    ///
    /// # Parameters
    ///
    /// - `func_index`: The index of the translated function within its Wasm
    ///   module.
    /// - `engine_func`: The index of the translated function in the [`Engine`].
    /// - `offset`: The global offset of the Wasm function body within the Wasm
    ///   binary.
    /// - `bytes`: The bytes that make up the Wasm encoded function body of the
    ///   translated function.
    /// - `module`: The module header information of the Wasm module of the
    ///   translated function.
    ...
    /// Returns reusable [`FuncTranslatorAllocations`] and
    /// [`FuncValidatorAllocations`] from the [`Engine`].
    ...
    /// Recycles the given [`FuncTranslatorAllocations`] and
    /// [`FuncValidatorAllocations`] in the [`Engine`].
    ...
    /// # Note
    ///
    /// The initialized function will not be compiled after this call and
    /// instead be prepared to be compiled on the fly when it is called the
    /// first time.
    ///
    /// # Panics
    ///
    ...
    /// # Note
    ///
    /// - Assumes that the `params` and `results` are well typed. Type checks
    ///   are done at the [`Func::call`] API or when creating a new
    ///   [`TypedFunc`] instance via [`Func::typed`].
    /// - The `params` out parameter is in a valid but unspecified state if this
    ///   function returns with an error.
    ///
    /// # Errors
    ///
    /// - If `params` are overflowing or underflowing the expected amount of
    ///   parameters.
    /// - If the given `results` do not match the length of the expected results
    ///   of `func`.
    ...
    /// Executes the given [`Func`] resumably with parameters `params` and
    /// returns.
    ///
    /// Stores the execution result into `results` upon a successful execution.
    /// If the execution encounters a host trap it will return a handle to the
    /// user that allows to resume the execution at that point.
    ///
    /// # Note
    ///
    /// - Assumes that the `params` and `results` are well typed. Type checks
    ...
    /// Resumes the given `invocation` after running out of fuel given the
    /// `params`.
    ///
    /// Stores the execution result into `results` upon a successful execution.
    /// If the execution encounters a host trap it will return a handle to the
...
pub struct EngineInner {
    ...
    /// ideally recycles old ones since creation of a new stack is rather
    /// expensive.
...
impl core::fmt::Debug for ReusableAllocationStack {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("ReusableAllocationStack")
            ...
            // Note: FuncValidatorAllocations is missing Debug impl at the time of writing this
            // commit.       We should derive Debug as soon as FuncValidatorAllocations
            // has a Debug impl in future       wasmparser versions.
...
impl EngineInner {
    ...
    /// # Note
    ///
    /// The initialized function will be compiled and ready to be executed after
    /// this call.
    ///
    /// # Panics
    ///
