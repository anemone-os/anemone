...
/// Extension trait to provide more optimized divide and remainder
/// implementations.
pub trait DivRemExt: Sized {
    ...
    /// Optimized variant of Wasm `i{32,64}.div_s` for immutable non-zero `rhs`
    /// values.
    fn div_s(self, rhs: Self::NonZeroS) -> Result<Self, Error>;
    /// Optimized variant of Wasm `i{32,64}.div_u` for immutable non-zero `rhs`
    /// values.
    fn div_u(self, rhs: Self::NonZeroU) -> Self;
    /// Optimized variant of Wasm `i{32,64}.rem_s` for immutable non-zero `rhs`
    /// values.
    fn rem_s(self, rhs: Self::NonZeroS) -> Result<Self, Error>;
    /// Optimized variant of Wasm `i{32,64}.rem_u` for immutable non-zero `rhs`
    /// values.
