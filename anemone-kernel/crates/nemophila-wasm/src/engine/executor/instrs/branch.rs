...
impl Executor<'_> {
    ...
    /// Fetches the branch table index value and normalizes it to clamp between
    /// `0..len_targets`.
    ...
    pub fn execute_branch_table_span(&mut self, index: Slot, len_targets: u32) {
        ...
        match *self.ip.get() {
            ...
            //       They are executed as the next conventional instruction in the pipeline, no
            ...
            unexpected => {
                // Safety: Wasmi translator guarantees that one of the above `Op` variants
                // exists.
                unsafe {
    ...
    /// Executes a generic fused compare and branch instruction with immediate
    /// `rhs` operand.
