...
impl Executor<'_> {
    ...
    fn execute_copy_2_impl(&mut self, results: FixedSlotSpan<2>, values: [Slot; 2]) {
        ...
        // We need `tmp` in case `results[0] == values[1]` to avoid overwriting
        // `values[1]` before reading it.
    ...
    /// Internal implementation of [`Op::CopyMany`] execution.
