...
impl Executor<'_> {
    /// Updates the [`InstructionPtr`] of the caller [`CallFrame`] before
    /// dispatching a call.
    ///
    /// # Note
    ///
    ...
    /// - This is done by encoding an [`Op::TableGet`] instruction word
    ///   following the actual instruction where the [`index::Table`] parameter
    ///   belongs to.
    /// - This is required for some instructions that do not fit into a single
    ///   instruction word and store a [`index::Table`] value in another
    ///   instruction word.
    fn pull_call_indirect_params(&mut self) -> (u64, index::Table) {
        ...
        match *self.ip.get() {
            ...
            unexpected => {
                // Safety: Wasmi translation guarantees that correct instruction parameter
                // follows.
                unsafe {
    ...
    fn dispatch_compiled_func<C: CallContext>(
        ...
        // [`ValueStack::alloc_call_frame`] which might invalidate all live
        // [`FrameSlots`].
        let caller = self
            .stack
            .calls
    ...
    fn prepare_compiled_func_call<C: CallContext>(
        ...
        match <C as CallContext>::KIND {
            ...
            CallKind::Tail => {
                ...
                // Safety:
                //
                // We provide `merge_call_frames` properly with `frame` that has just been
                // allocated on the value stack which is what the function
                // expects. After this operation we ensure that `self.sp` is
                // adjusted via a call to `init_call_frame` since it may have been
    ...
    /// Returns the `results` [`SlotSpan`] of the top-most [`CallFrame`] on the
    /// [`CallStack`].
    ///
    /// # Note
    ///
    /// We refer to the top-most [`CallFrame`] as the `caller` since this method
    /// is used for tail call instructions for which the top-most
    /// [`CallFrame`] is the caller.
    ///
    ...
    /// # Note
    ///
    /// This uses the value stack to store parameters and results of the host
    /// function call. Returns an [`ErrorKind::ResumableHostTrap`] variant
    /// if the host function returned an error and there are still call
    /// frames on the call stack making it possible to resume the
        ...
        match <C as CallContext>::KIND {
            CallKind::Nested => {
                ...
                for (result, value) in results.zip(returned) {
                    ...
                    // call frames because we just allocated the host call frame and can be sure
