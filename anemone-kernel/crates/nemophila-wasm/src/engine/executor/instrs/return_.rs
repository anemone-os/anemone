...
impl Executor<'_> {
    ...
    /// Returns the [`FrameSlots`] of the caller and the [`SlotSpan`] of the
    /// results.
    ///
    /// The returned [`FrameSlots`] is valid for all [`Slot`] in the returned
    /// [`SlotSpan`].
    ...
    /// Execute an [`Op::ReturnI64Imm32`] returning a single 32-bit encoded
    /// `i64` value.
    ...
    /// Execute an [`Op::ReturnF64Imm32`] returning a single 32-bit encoded
    /// `f64` value.
