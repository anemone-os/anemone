...
/// Tells if execution loop shall continue or break (return) to the execution's
/// caller.
type ControlFlow = ::core::ops::ControlFlow<(), ()>;

/// Executes compiled function instructions until execution returns from the
/// root function.
///
/// # Errors
///
...
impl Executor<'_> {
    ...
    fn get_stack_slot(&self, slot: Slot) -> UntypedVal {
        ...
        //         - This is done by updating the `sp` pointer whenever the heap
        //           underlying the value stack is changed.
    ...
    /// Shifts the instruction pointer to the next instruction and returns
    /// `Ok(())`.
    ///
    /// # Note
    ///
...
impl Executor<'_> {
    ...
    fn invalid_instruction_word(&mut self) -> Result<(), Error> {
        // Safety: Wasmi translation guarantees that branches are never taken to
