...
impl ValueStack {
    ...
    /// Returns the root [`FrameSlots`] pointing to the first value on the
    /// [`ValueStack`].
    ...
    ///
    /// This may heap allocate in case the [`ValueStack`] ran out of
    /// preallocated memory.
    ///
    /// # Errors
    ///
    ...
    pub fn drop(&mut self, amount: usize) {
        ...
        // Safety: we just asserted that the current length is large enough to not
        // underflow.
        unsafe { self.values.set_len(self.len() - amount) };
    }

    /// Drop the last `amount` cells of the [`ValueStack`] and returns a slice
    /// to them.
    ///
    /// # Panics (Debug)
    ///
    ...
    ///
    /// Returns the [`BaseValueStackOffset`] and [`FrameValueStackOffset`] of
    /// the allocated [`EngineFunc`].
    ///
    /// # Note
    ///
    /// - All live [`FrameSlots`] might be invalidated and need to be
    ///   reinstantiated.
    /// - The parameters of the allocated [`EngineFunc`] are set to zero and
    ///   require proper initialization after this call.
    ///
    /// # Errors
    ///
    /// When trying to grow the [`ValueStack`] over its maximum size limit.
    ...
    /// Removes the slice `from..to` of [`UntypedVal`] cells from the
    /// [`ValueStack`].
    ///
    ...
    /// # Safety
    ///
    /// - This invalidates all [`FrameSlots`] within the range `from..` and the
    ///   caller has to make sure to properly reinstantiate all those pointers
    ///   after this operation.
    /// - This also invalidates all [`FrameValueStackOffset`] and
    ///   [`BaseValueStackOffset`] indices within the range `from..`.
    #[inline(always)]
