...
use crate::{
    collections::HeadVec, engine::executor::InstructionPtr, ir::SlotSpan, Instance, TrapCode,
};
use alloc::vec::Vec;

#[cfg(doc)]
use crate::{
    engine::executor::stack::ValueStack, engine::EngineFunc, ir::Op, ir::Slot, Global, Memory,
    Table,
};
...
pub struct CallStack {
    ...
    /// # Note
    ///
    /// A [`TrapCode::StackOverflow`] is raised if the recursion limit is
    /// exceeded.
...
impl CallStack {
    ...
    /// # Note
    ///
    /// - The top-most [`CallFrame`] on the [`CallStack`] is referred to as the
    ///   `callee`.
    /// - The second top-most [`CallFrame`] on the [`CallStack`] is referred to
    ///   as the `caller`.
    ///
