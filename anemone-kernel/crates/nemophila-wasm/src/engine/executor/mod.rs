...
use crate::{
    engine::{
        CallParams, CallResults, EngineInner, ResumableCallBase, ResumableCallHostTrap,
        ResumableCallOutOfFuel,
    },
    func::HostFuncEntity,
    ir::{Slot, SlotSpan},
    store::CallHooks,
    CallHook, Error, Func, FuncEntity, Store, StoreContextMut,
};
...
impl EngineInner {
    /// Executes the given [`Func`] with the given `params` and returns the
    /// `results`.
    ///
    /// Uses the [`StoreContextMut`] for context information about the Wasm
    /// [`Store`].
    ///
    /// # Errors
    ///
    ...
    /// Executes the given [`Func`] resumably with the given `params` and
    /// returns the `results`.
    ///
    /// Uses the [`StoreContextMut`] for context information about the Wasm
    /// [`Store`].
    ...
    /// Resumes the given [`Func`] with the given `params` and returns the
    /// `results`.
    ///
    /// Uses the [`StoreContextMut`] for context information about the Wasm
    /// [`Store`].
    ...
    /// Resumes the given [`Func`] after running out of fuel and returns the
    /// `results`.
    ///
    /// Uses the [`StoreContextMut`] for context information about the Wasm
    /// [`Store`].
...
impl<'engine> EngineExecutor<'engine> {
        ...
        Results: CallResults,
    {
        ...
        match store.inner.resolve_func(func) {
            FuncEntity::Wasm(wasm_func) => {
                // We reserve space on the stack to write the results of the root function
                // execution.
    ...
    /// Resumes the execution of the given [`Func`] using `params` after a host
    /// function trapped.
    ///
    /// Stores the execution result into `results` upon a successful execution.
    ///
    ...
    /// Resumes the execution of the given [`Func`] using `params` after running
    /// out of fuel.
    ///
    /// Stores the execution result into `results` upon a successful execution.
    ///
    ...
    /// Executes the top most Wasm function on the [`Stack`] until the [`Stack`]
    /// is empty.
    ///
    /// # Errors
    ///
    ...
    /// Writes the results of the function execution back into the `results`
    /// buffer.
    ///
    /// # Note
    ///
    ...
    /// # Panics
    ///
    /// - If the `results` buffer length does not match the remaining amount of
    ///   stack values.
    #[inline(always)]
