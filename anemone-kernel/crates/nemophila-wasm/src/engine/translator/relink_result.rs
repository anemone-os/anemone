use crate::{
    ...
    module::ModuleHeader,
    Engine, Error, FuncType,
};

/// Extension trait for [`Op`] to conditionally relink result [`Slot`]s.
pub trait RelinkResult {
    /// Relinks the result [`Slot`] of `self` to `new_result` if its current
    /// `result` [`Slot`] equals `old_result`.
    ///
    /// # Note (Return Value)
    ///
...
impl RelinkResult for Op {
    fn relink_result(
        ...
        // Note: for call instructions we have to infer with special handling if they
        // return       a single value which allows us to relink the single
        // result register.
        match self {
            ...
            instr => {
                // Fallback: only relink results of instructions with statically known single
                // results.
