...
use self::{
    ...
    stack::{
        ...
        ElseReachability, IfControlFrame, IfReachability, ImmediateOperand, LoopControlFrame,
        Operand, OperandIdx, Stack, StackAllocations, TempOperand,
    },
...
use crate::{
    ...
    engine::{
        translator::{
            comparator::{
                CompareResult as _, LogicalizeCmpInstr, NegateCmpInstr, ReplaceCmpResult,
        ...
        BlockType, CompiledFuncEntity, TranslationError,
    },
    ir::{
        index, Address, Address32, AnyConst16, BoundedSlotSpan, BranchOffset, BranchOffset16,
        Comparator, ComparatorAndOffset, Const16, Const32, FixedSlotSpan, IntoShiftAmount,
        Offset16, Offset64, Offset64Lo, Op, Sign, Slot, SlotSpan,
    },
...
pub struct FuncTranslator {
    ...
    /// # Note
    ///
    /// Technically this is not needed since the information is redundant given
    /// via the `module` field. However, this acts like a faster access
    /// since `module` only holds a weak reference to the engine.
    engine: Engine,
    ...
    /// - `false`: The currently translated code is unreachable and can be
    ///   skipped.
    ///
    /// # Note
    ///
...
impl WasmTranslator<'_> for FuncTranslator {
    ...
    fn finish(
        ...
        // Note: `update_branch_offsets` might change `frame_size` so we need to compute
        // it prior.
        //
        // Context:
        // This only happens if the function has so many instructions that some
        // conditional branch operators need to be encoded as their fallbacks
        // which requires to allocate more function local constant values, thus
        // increasing the size of the function frame.
...
impl FuncTranslator {
    ...
    /// Applies `f` to the [`FuncType`] of the function that is currently
    /// translated.
    ...
    /// Returns the [`SlotSpan`] of a call instruction before manipulating the
    /// operand stack.
    ...
    /// # Note
    ///
    /// - The top-most `len` operands on the [`Stack`] will be [`Operand::Temp`]
    ///   upon completion.
    ...
    /// # Note
    ///
    /// - The top-most `depth` operands on the [`Stack`] will be
    ///   [`Operand::Temp`] upon completion.
    /// - Does nothing if an [`Operand`] is already an [`Operand::Temp`].
    ...
    /// # Note
    ///
    /// - Before pushing the results, the [`Stack`] is truncated to the
    ///   `frame`'s height.
    /// - Not all control frames have temporary results, e.g. Wasm `loop`s, Wasm
    ///   `if`s with a compile-time known branch or Wasm `block`s that are never
    ///   branched to, do not require to call this function.
    ...
    /// Encodes a copy instruction for the top-most `len_values` on the stack to
    /// `results`.
    ///
    /// # Note
    ///
    /// - This does _not_ pop values from the stack or manipulate the stack
    ///   otherwise.
    ...
    /// Tries to merge `copy_instr` with the succeeding copy instruction if one
    /// exists.
    ///
    ...
    fn try_merge_copy_instr(last_copy: Op, result: Slot, value: Slot) -> Option<Op> {
        ...
        if value == last_result {
            // Case: cannot merge since the succeeding copy overwrites the result of
            // `copy_instr`
            return None;
        }
    ...
    /// Tries to merge an [`Op::Copy2`] and an [`Op::Copy`] and returns the
    /// result.
    ///
    /// Returns `None` if merging was not possible.
    ...
    /// Tries to merge an [`Op::CopySpan`] and an [`Op::Copy`] and returns the
    /// result.
    ///
    /// Returns `None` if merging was not possible.
    ...
    /// Returns the copy instruction to copy the given immediate `value` to
    /// `result`.
    ...
    ///
    /// - This won't encode a copy if the resulting copy instruction is a no-op.
    /// - Encodes either `copy`, `copy2`, `copy_span` or `copy_many` depending
    ///   on the amount of noop copies between `results` and `values`.
    fn encode_copy_many(
        &mut self,
        results: SlotSpan,
        len: u16,
    ...
    ///
    /// Returns the stripped `results` [`SlotSpan`] and `values` slice of
    /// [`Operand`]s.
    ...
    /// Returns `true` if there are overlapping copies with `results` and
    /// `values`.
    ///
    /// # Examples
    ///
    /// - `[ 0 <- 1, 1 <- 1, 2 <- 4 ]` has no overlapping copies.
    /// - `[ 0 <- 1, 1 <- 0 ]` has overlapping copies since register `0` is
    ///   written to in the first copy but read from in the next.
    ...
    /// Returns `true` if the [`ControlFrame`] at `depth` requires copying for
    /// its branch parameters.
    ///
    /// # Note
    ///
    /// Some instructions can be encoded in a more efficient way if no branch
    /// parameter copies are required.
    ...
    /// Convert the [`Operand`] at `depth` into an [`Operand::Temp`] by copying
    /// if necessary.
    ///
    /// # Note
    ///
    ...
    /// # Note
    ///
    /// - Preferrably, this encodes the immediate `operand` into a `copy`
    ///   instruction with the immediate encoded inline.
    /// - If the immediate `operand` cannot be encoded as `copy` with inline
    ///   immediate a function local constant [`Slot`] will be allocated and
    ///   returned.
    /// - Returns the associated [`Slot`] if `operand` is an [`Operand::Temp`]
    ///   or [`Operand::Local`].
    ...
    /// Populate the `buffer` with the `table` targets including the `table`
    /// default target.
    ///
    ...
    /// Encodes a Wasm `br_table` that has to copy `len_values` branching
    /// values.
    ///
    /// # Note
    ///
    ...
    /// Store the top-most [`Operand`]s on the [`Stack`] into the operands
    /// buffer.
    ...
    /// Tries to form a [`SlotSpan`] from the top-most `n` operands on the
    /// [`Stack`].
    ///
    ...
    /// Tries to form a [`SlotSpan`] from the top-most `len` operands on the
    /// [`Stack`] or copy to temporaries.
    ///
    /// Returns `None` if forming a [`SlotSpan`] was not possible.
    ...
    fn translate_end_block(&mut self, frame: BlockControlFrame) -> Result<(), Error> {
        ...
        if frame.is_branched_to() {
            // No need to reset `last_instr` if there was no branch to the end of a Wasm
            // `block`.
    ...
    fn translate_end_loop(&mut self, _frame: LoopControlFrame) -> Result<(), Error> {
        ...
        // Nothing needs to be done since Wasm `loop` control frames always only have a
        // single exit.
        //
        // Note: no need to reset `last_instr` since end of `loop` is not a control flow
        // boundary.
        Ok(())
    }
    ...
    /// Translates the end of a Wasm `else` control frame where only one branch
    ...
    fn translate_end_unreachable(&mut self, _frame: ControlFrameKind) -> Result<(), Error> {
        debug_assert!(!self.stack.is_control_empty());
        // We reset `last_instr` out of caution in case there is a control flow
        // boundary.
        self.instrs.reset_last_instr();
        Ok(())
    }
    ...
    /// # Note
    ///
    /// This applies op-code fusion that replaces the result of the previous
    /// instruction instead of encoding a copy instruction for the
    /// `local.set` or `local.tee` if possible.
    ...
    /// Tries to replace the result of the previous instruction with
    /// `new_result` if possible.
    ///
    /// Returns `Ok(true)` if replacement was successful and `Ok(false)`
    /// otherwise.
    ...
    /// # Note
    ///
    /// This Wasm operation is a no-op. Ideally we only have to change the types
    /// on the stack.
    ...
    /// # Note
    ///
    /// - Turns immediates that cannot be 16-bit encoded into function local
    ///   constants.
    ...
    /// # Note
    ///
    /// - Turns immediates that cannot be 32-bit encoded into function local
    ///   constants.
    /// - The behavior is different whether `memory64` is enabled or disabled.
    ...
    /// Evaluates `consteval(lhs, rhs)` and pushed either its result or
    /// tranlates a `trap`.
    ...
    /// Translates integer division and remainder Wasm operators to Wasmi
    /// bytecode.
    ...
    /// # Note
    ///
    /// - This applies some optimization that are valid for copysign
    ///   instructions.
    ...
    /// # Note
    ///
    /// - This applies constant propagation in case `condition` is a constant
    ///   value.
    /// - If both `lhs` and `rhs` are equal registers or constant values `lhs`
    ///   is forwarded.
    /// - Fuses compare instructions with the associated select instructions if
    ///   possible.
    fn translate_select(&mut self, type_hint: Option<ValType>) -> Result<(), Error> {
        ...
        if true_val.is_same(&false_val) {
            // Optimization: both `lhs` and `rhs` either are the same register or constant
            // values and               thus `select` will always yield this
            // same value irrespective of the condition.
            self.stack.push_operand(true_val)?;
            return Ok(());
        }
        if let Operand::Immediate(condition) = condition {
            // Optimization: since condition is a constant value we can const-fold the
            // `select`               instruction and simply push the selected
            // value back to the provider stack.
    ...
    /// Create either [`Op::CallIndirectParams`] or
    /// [`Op::CallIndirectParamsImm16`] depending on the inputs.
    ...
    /// Translates Wasm integer `store` and `storeN` instructions to Wasmi
    /// bytecode.
    ///
    /// # Note
    ///
    /// This chooses the most efficient encoding for the given `store`
    /// instruction. If `ptr+offset` is a constant value the pointer address
    /// is pre-calculated.
    ///
    /// # Usage
    ///
    ...
    /// Encodes Wasm integer `store` and `storeN` instructions as Wasmi
    /// bytecode.
    ...
    /// Encodes a Wasm integer `store` and `storeN` instructions as Wasmi
    /// bytecode.
    ///
    /// # Note
    ///
    ...
    /// # Note
    ///
    /// This optimizes for cases where the Wasm linear memory that is operated
    /// on is known to be the default memory.
    ...
    /// Encodes a Wasm `store` instruction with immediate address as Wasmi
    /// bytecode.
    ///
    /// # Note
    ///
    ...
    /// Translates a Wasm `i64.binop128` instruction from the `wide-arithmetic`
    /// proposal.
    ...
    /// Translates a Wasm `i64.mul_wide_sx` instruction from the
    ...
    /// Try to optimize a `i64.mul_wide_sx` instruction with one [`Slot`] and
    /// one immediate input.
    ///
