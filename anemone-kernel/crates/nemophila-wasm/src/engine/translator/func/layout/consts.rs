...
/// - All constant values are also deduplicated so that no duplicates are stored
///   in a [`ConstRegistry`]. This also means that deciding if two [`Slot`]
///   values refer to the equal constant values can be efficiently done by
///   comparing the [`Slot`] indices without resolving to their underlying
///   constant values.
...
impl ConstRegistry {
    /// The maximum index for [`Slot`] referring to function local constant
    /// values.
    ///
    /// # Note
    ///
    ...
    /// The mininmum index for [`Slot`] referring to function local constant
    /// values.
    ///
    /// # Note
    ///
    ...
    /// Allocates a new constant `value` on the [`ConstRegistry`] and returns
    /// its identifier.
    ///
    /// # Note
    ///
    /// If the constant `value` already exists in this [`ConstRegistry`] no new
    /// value is allocated and the identifier of the existing constant
    /// `value` returned instead.
    ///
    /// # Errors
    ///
    /// If too many constant values have been allocated for this
    /// [`ConstRegistry`].
    ...
    /// Returns an iterator yielding all function local constant values of the
    /// [`ConstRegistry`].
    ///
    /// # Note
    ///
    /// The function local constant values are yielded in their allocation
    /// order.
...
impl<'a> ConstRegistryIter<'a> {
    /// Creates a new [`ConstRegistryIter`] from the given slice of
    /// [`UntypedVal`].
