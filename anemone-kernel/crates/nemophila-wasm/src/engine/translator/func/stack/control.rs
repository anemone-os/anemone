...
pub struct ControlStack {
    ...
    /// fuel consumption instruction since this information is accessed
    /// commonly.
    ///
    /// [`Op`]: crate::ir::Op
    ...
    /// This is `true` if an `if` with else providers was just popped from the
    /// stack.
    ///
    /// # Note
    ///
    /// This means that its associated `else` operands need to be taken care of
    /// by either pushing back an `else` control frame or by manually
    /// popping them off the control stack.
...
impl ControlStack {
    ...
    /// Returns an exclusive reference to the [`ControlFrame`] at `depth` if
    /// any.
...
pub enum AcquiredTarget<'stack> {
    /// The branch targets the function enclosing `block` and therefore is a
    /// `return`.
...
impl<'stack> AcquiredTarget<'stack> {
    /// Returns an exclusive reference to the [`ControlFrame`] of the
    /// [`AcquiredTarget`].
...
pub struct BlockControlFrame {
    ...
    height: StackHeight,
    /// This is `true` if there is at least one branch to this
    /// [`BlockControlFrame`].
    is_branched_to: bool,
    /// The [`BlockControlFrame`]'s [`Op::ConsumeFuel`] if fuel metering is
    /// enabled.
    ///
    /// # Note
    ///
...
pub struct LoopControlFrame {
    ...
    /// [`LoopControlFrame`].
    is_branched_to: bool,
    /// The [`LoopControlFrame`]'s [`Op::ConsumeFuel`] if fuel metering is
    /// enabled.
    ///
    /// # Note
    ///
...
pub struct IfControlFrame {
    ...
    /// [`IfControlFrame`].
    is_branched_to: bool,
    /// The [`IfControlFrame`]'s [`Op::ConsumeFuel`] if fuel metering is
    /// enabled.
    ///
    /// # Note
    ///
...
impl IfControlFrame {
    ...
    /// # Note
    ///
    /// The `else` branch is unreachable if the `if` condition is a constant
    /// `true` value.
...
pub enum IfReachability {
    ...
    /// # Note
    ///
    /// This case happens only in case the `if` has a `false` constant
    /// condition.
    OnlyElse,
}
