...
///
/// This is the internal version of [`Operand`] with information that shall
/// remain hidden to the outside.
...
pub struct OperandStack {
    ...
    /// Stores the first occurrences of every local variable on the
    /// [`OperandStack`] if any.
    ...
    ///
    /// This field is required to optimize
    /// [`OperandStack::preserve_all_locals`].
    len_locals: usize,
}
...
impl OperandStack {
    ...
    /// Converts and returns the [`Operand`] at `depth` into a
    /// [`Operand::Temp`].
    ///
    /// # Note
    ///
    /// - Returns the [`Operand`] at `depth` before being converted to an
    ///   [`Operand::Temp`].
    ...
    /// Converts and returns the [`StackOperand`] at `index` into a
    /// [`StackOperand::Temp`].
    ///
    /// # Note
    ///
    /// - Returns the [`Operand`] at `index` before being converted to an
    ///   [`Operand::Temp`].
    /// - [`Operand::Temp`] will have their optional `instr` set to `None`.
    ///
    /// # Panics
    ...
    ///
    /// This is done by converting those locals to [`StackOperand::Temp`] and
    /// yielding them.
    ///
    /// # Note
    ///
    /// The users must fully consume all items yielded by the returned iterator
    ...
    /// Unlinks the [`StackOperand::Local`] `operand` identified by the
    ...
    /// Updates the `prev_local` of the [`StackOperand::Local`] at `local_index`
    /// to `prev_index`.
    ///
    /// # Panics
    ///
    ...
    /// Updates the `next_local` of the [`StackOperand::Local`] at `local_index`
    /// to `prev_index`.
    ///
    /// # Panics
    ///
...
/// # Note
///
/// This intentionally iterates backwards from the last pushed stack operand to
/// the first one. Together with the remaining number of local operands on the
/// stack this achieves armortized constant O(1) preservation for all locals via
/// [`OperandStack::preserve_all_locals`].
///
/// The reason for this is that a single call to
/// [`OperandStack::preserve_all_locals`] has the effect that there are no more
/// local operands on the stack. New locals are always pushed to the top of the
/// stack. A single Wasm `local.get` operation (or similar) may only push a
/// single local operand on the stack. This iterator yields once there are no
/// more local operands and since it iterates from the back (top-most) operand
/// it will find the newly inserted locals in armortized constant O(1) time.
#[derive(Debug)]
