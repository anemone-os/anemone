...
impl Stack {
    ...
    /// Returns `true` is fuel metering is enabled for the associated
    /// [`Engine`].
    ...
    /// # Note
    ///
    /// This inherits the `consume_fuel` [`Instr`] from the parent
    /// [`ControlFrame`].
    ///
    /// # Errors
    ///
    ...
    /// - If any of the Wasm `loop` operand parameters are _not_
    ///   [`Operand::Temp`].
    ///
    /// # Errors
    ///
    ...
    /// Pushes the top-most `else` operands from the control stack onto the
    /// operand stack.
    ///
    /// # Panics (Debug)
    ///
    ...
    /// body Wasm `block` and other control frames in order to know whether a
    /// branching target returns or branches.
    ///
    /// # Panics
    ///
    /// If `depth` is out of bounds for `self`.
    ...
    ///
    /// This is done by converting those locals to [`Operand::Temp`] and
    /// yielding them.
    ///
    /// # Note
    ///
