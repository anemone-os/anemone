...
use crate::{
    ...
    engine::translator::{
        comparator::{
            CmpSelectFusion, CompareResult as _, TryIntoCmpSelectInstr as _,
...
impl InstrEncoder {
    ...
    /// Tries to replace the result of the last instruction with `new_result` if
    /// possible.
    ///
    /// # Note
    ///
    /// - `old_result`: just required for additional safety to check if the last
    ///   instruction really is the source of the `local.set` or `local.tee`.
