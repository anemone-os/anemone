...
/// Convenience trait used to circumvent the need for `#[cfg]` where bounds.
///
/// Wasm `simd` is enabled, thus this trait forwards to
/// [`wasmparser::VisitSimdOperator`].
#[cfg(feature = "simd")]
pub trait VisitSimdOperator<'a>:
    ...
    VisitOperator<'parser, Output = Result<(), Error>> + VisitSimdOperator<'parser>
{
    ...
    /// Sets up the translation process for the Wasm `bytes` and Wasm `module`
    /// header.
    ///
    /// - Returns `true` if the [`WasmTranslator`] is done with the translation
    ///   process.
    /// - Returns `false` if the [`WasmTranslator`] demands the translation
    ///   driver to proceed with the process of parsing the Wasm module and
    ///   feeding parse pieces to the [`WasmTranslator`].
    ///
    /// # Note
    ///
    /// - This method requires `bytes` to be the slice of bytes that make up the
    ///   entire Wasm function body (including local variables).
    /// - Also `module` must be a reference to the Wasm module header that is
    ///   going to be used for translation of the Wasm function body.
    fn setup(&mut self, bytes: &[u8]) -> Result<bool, Error>;

    /// Returns a reference to the [`WasmFeatures`] used by the
    /// [`WasmTranslator`].
    ...
    /// Informs the [`WasmTranslator`] that the Wasm function header translation
    /// is finished.
    ///
    /// # Note
    ///
    /// - After this function call no more locals and parameters may be
    ///   registered to the [`WasmTranslator`] via
    ///   [`WasmTranslator::translate_locals`].
    /// - After this function call the [`WasmTranslator`] expects its
    ///   [`VisitOperator`] trait methods to be called for translating the Wasm
    ///   operators of the translated function.
    ///
    /// # Dev. Note
    ///
    /// This got introduced to properly calculate the fuel costs for all local
    /// variables and function parameters.
    fn finish_translate_locals(&mut self) -> Result<(), Error>;

    /// Updates the [`WasmTranslator`] about the current byte position within
    /// translated Wasm binary.
    ///
    /// # Note
    ///
    /// This information is mainly required for properly locating translation
    /// errors.
...
impl<T> ValidatingFuncTranslator<T> {
    ...
    /// Returns the current position within the Wasm binary while parsing
    /// operators.
    ...
    T: WasmTranslator<'parser>,
{
    ...
    fn setup(&mut self, bytes: &[u8]) -> Result<bool, Error> {
        ...
        // Note: Wasm validation always need to be driven, therefore returning
        // `Ok(false)`       even if the underlying Wasm translator does not
        // need a translation driver.
        Ok(false)
    }
...
/// A lazy Wasm function translator that defers translation when the function is
/// first used.
#[derive(Debug)]
...
impl LazyFuncTranslator {
    ...
    /// Create a new [`LazyFuncTranslator`] that does not validate Wasm upon
    /// lazy translation.
