use crate::{
    ...
    ir::{Const16, Op, Sign},
    Error, ExternRef, Func, Ref, ValType,
};
...
pub trait BumpFuelConsumption {
    /// Increases the fuel consumption of the [`Op::ConsumeFuel`] instruction by
    /// `delta`.
    ///
    /// # Error
    ///
...
impl Instr {
    ...
    /// - Returns `1` if `self` is adjacent to `other` in the sequence of
    ///   instructions.
    /// - etc..
