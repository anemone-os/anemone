...
pub struct Config {
    ...
    consume_fuel: bool,
    /// Is `true` if Wasmi shall ignore Wasm custom sections when parsing Wasm
    /// modules.
...
impl Config {
    ...
    /// Sets the maximum recursion depth of the [`Engine`]'s stack during
    /// execution.
    ///
    /// # Note
    ///
    ...
    /// Sets the minimum (or initial) height of the [`Engine`]'s value stack in
    /// bytes.
    ///
    /// # Note
    ///
    ...
    /// # Panics
    ///
    /// If `value` is greater than the current maximum height of the value
    /// stack.
    ///
    /// [`Engine`]: [`crate::Engine`]
    ...
    /// Enable or disable the [`mutable-global`] Wasm proposal for the
    /// [`Config`].
    ///
    /// # Note
    ///
    ...
    /// Enable or disable the [`sign-extension`] Wasm proposal for the
    /// [`Config`].
    ///
    /// # Note
    ///
    ...
    /// Enable or disable the [`reference-types`] Wasm proposal for the
    /// [`Config`].
    ///
    /// # Note
    ///
    ...
    /// Enable or disable the [`extended-const`] Wasm proposal for the
    /// [`Config`].
    ///
    /// # Note
    ///
    ...
    /// Enable or disable the [`custom-page-sizes`] Wasm proposal for the
    /// [`Config`].
    ///
    /// # Note
    ///
    ...
    /// Enable or disable the [`wide-arithmetic`] Wasm proposal for the
    /// [`Config`].
    ///
    /// Disabled by default.
    ///
    ...
    /// Configures whether Wasmi will consume fuel during execution to either
    /// halt execution as desired.
    ///
    /// # Note
    ///
    /// This configuration can be used to make Wasmi instrument its internal
    /// bytecode so that it consumes fuel as it executes. Once an execution
    /// runs out of fuel
    /// a [`TrapCode::OutOfFuel`](crate::TrapCode::OutOfFuel) trap is raised.
    /// This way users can deterministically halt or yield the execution of
    /// WebAssembly code.
    ///
    /// - Use [`Store::set_fuel`](crate::Store::set_fuel) to set the remaining
    ///   fuel of the [`Store`] before executing some code as the [`Store`]
    ///   start with no fuel.
    /// - Use [`Caller::set_fuel`](crate::Caller::set_fuel) to update the
    ///   remaining fuel when executing host functions.
    ///
    /// Disabled by default.
    ///
    /// [`Store`]: crate::Store
    ...
    /// Returns `true` if the [`Config`] enables fuel consumption by the
    /// [`Engine`].
    ///
    /// [`Engine`]: crate::Engine
    ...
    /// Configures whether Wasmi will ignore custom sections when parsing Wasm
    /// modules.
    ///
    /// Default value: `false`
    ...
    /// Returns `true` if the [`Config`] mandates to ignore Wasm custom sections
    /// when parsing Wasm modules.
    ...
    /// Sets the [`EnforcedLimits`] enforced by the [`Engine`] for Wasm module
