...
use crate::{
    ...
    ir::SlotSpan,
    AsContext, AsContextMut, Engine, Error, TrapCode, Val, WasmResults,
};
...
impl ResumableHostTrapError {
    ...
    /// Returns the caller results [`SlotSpan`] of the
...
impl ResumableCall {
    /// Creates a [`ResumableCall`] from the [`Engine`]'s base
    /// [`ResumableCallBase`].
...
pub struct ResumableCallCommon {
    ...
    /// # Note
    ///
    /// - This handle is required to resolve function types of both `func` and
    ///   `host_func` fields as well as in the `Drop` impl to recycle the stack.
    engine: Engine,
    ...
    /// # Note
    ///
    /// - We need to keep the stack around since the user might want to resume
    ///   the execution.
    /// - This stack is borrowed from the engine and needs to be given back to
    ///   the engine when the [`ResumableCallHostTrap`] goes out of scope.
    stack: Stack,
}
...
impl ResumableCallCommon {
    ...
    /// Replaces the internal stack with an empty one that has no heap
    /// allocations.
    ...
    /// # Errors
    ///
    /// If the number of items in `outputs` does not match the number of results
    /// of the resumed function.
...
// # Safety
//
// `ResumableCallCommon` is not `Sync` because of the following sequence of
// fields:
//
// - `ResumableCallCommon`'s `Stack` is not `Sync`
// - `Stack`'s `CallStack` is not `Sync`
//     - `CallStack`'s `CallFrame` sequence is not `Sync`
//     - `CallFrame`'s `InstructionPtr` is not `Sync`: Thi is because it is a
//       raw pointer to an `Op` buffer owned by the [`Engine`].
//
// Since `Engine` is owned by `ResumableCallCommon` it cannot be outlived.
// Also the `Op` buffers that are pointed to by the `InstructionPtr` are
// immutable.
//
...
pub struct ResumableCallHostTrap {
    ...
    /// # Note
    ///
    /// - This is required to receive its result values that are needed to be
    ///   fed back in manually by the user. This way we avoid heap memory
    ///   allocations.
    /// - The results of this function must always match with the arguments
    ///   given when resuming the call.
    host_func: Func,
    ...
    /// be a Wasm trap.
    host_error: Error,
    /// The registers where to put provided host function results upon
    /// resumption.
    ///
    /// # Note
    ///
...
impl ResumableCallHostTrap {
    ...
    /// Updates the [`ResumableCallHostTrap`] with the new `host_func`,
    /// `host_error` and `caller_results`.
    ///
    /// # Note
    ///
    /// This should only be called from the register-machine Wasmi engine
    /// backend.
    ...
    /// # Note
    ///
    /// This is `Some` only for [`ResumableCallHostTrap`] originating from the
    /// register-machine Wasmi engine.
    pub(crate) fn caller_results(&self) -> SlotSpan {
        self.caller_results
    }

    /// Validates that the `inputs` types are valid for the host function
    /// resumption.
    ///
    /// # Errors
    ///
    ...
    /// - If the types or the number of values in `inputs` does not match the
    ///   types and number of result values of the erroneous host function.
...
impl<Results> TypedResumableCall<Results> {
    /// Creates a [`TypedResumableCall`] from the [`Engine`]'s base
    /// [`ResumableCallBase`].
...
impl<Results> TypedResumableCallHostTrap<Results> {
    /// Creates a [`TypedResumableCallHostTrap`] wrapper for the given
    /// [`ResumableCallHostTrap`].
...
/// State required to resume a [`TypedFunc`] invocation after running out of
/// fuel.
///
/// [`TypedFunc`]: [`crate::TypedFunc`]
