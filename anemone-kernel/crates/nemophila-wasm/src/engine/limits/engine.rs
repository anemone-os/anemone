...
/// Stores customizable limits for the [`Engine`] when parsing or compiling Wasm
/// modules.
///
...
pub struct EnforcedLimits {
    ...
    ///
    /// - This is checked in [`Module::new`].
    /// - This is only relevant if the Wasm `reference-types` proposal is
    ///   enabled.
    /// - `None` means the limit is not enforced.
    ///
    /// [`Module::new`]: crate::Module::new
    ...
    /// Number of linear memory data segments a single Wasm module can have at
    /// most.
    ///
    /// # Note
    ///
    ...
    ///
    /// - This is checked in [`Module::new`].
    /// - This limitation might seem arbitrary but is important to defend
    ///   against malicious inputs targeting lazy compilation.
    /// - `None` means the limit is not enforced.
    ///
    /// [`Module::new`]: crate::Module::new
...
/// The limit for average bytes per function limit and the threshold at which it
/// is enforced.
#[derive(Debug, Copy, Clone)]
pub struct AvgBytesPerFunctionLimit {
    /// The number of Wasm module bytes at which the limit is actually enforced.
    ///
    /// This represents the total number of bytes of all Wasm function bodies in
    /// the Wasm module combined.
    ///
    /// # Note
    ///
    /// - A `req_funcs_bytes` of 0 always enforces the
    ///   `min_avg_bytes_per_function` limit.
...
impl EnforcedLimits {
    ...
    /// to safeguard themselves against malicious actors trying to attack the
    /// Wasmi compilation procedures.
    pub fn strict() -> Self {
        Self {
