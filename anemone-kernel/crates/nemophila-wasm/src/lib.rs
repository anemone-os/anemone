...
//! fn main() -> Result<(), nemophila_wasm::Error> {
//!     let wasm = r#"
//!         (module
...
//!     // Now we can compile the above Wasm module from the given Wasm source.
...
//! | `prefer-btree-collections` | `wasmi`<br>`wasmi_collections` | Enforces use of btree-map based collections in Wasmi internals. This may yield performance improvements and memory consumption decreases. Also it enables Wasmi to run on platforms that have no random source. <br><br> Disabled by default. |
...
// Executor invariants are a production safety boundary. Keep this compile-time
// guard next to the crate attributes so consumers cannot accidentally trade
// executor invariant checks for speed by disabling default features.
#[cfg(not(feature = "extra-checks"))]
compile_error!("nemophila-wasm requires the `extra-checks` safety feature");
...
mod core {
    ...
    pub use wasmi_core::{
        hint, wasm, DecodeUntypedSlice, ElementSegment as CoreElementSegment, EncodeUntypedSlice,
        Fuel, FuelCostsProvider, FuncType as CoreFuncType, Global as CoreGlobal, IndexType,
        LimiterError, Memory as CoreMemory, MemoryType as CoreMemoryType,
        MemoryTypeBuilder as CoreMemoryTypeBuilder, ReadAs, ResourceLimiterRef, Table as CoreTable,
        TableType as CoreTableType, Typed, TypedVal, UntypedError, UntypedVal, WriteAs,
    };
}
...
pub use self::{
    engine::{
        ...
        ResumableCallHostTrap, ResumableCallOutOfFuel, TypedResumableCall,
        TypedResumableCallHostTrap, TypedResumableCallOutOfFuel,
    },
    error::Error,
    func::{
        Caller, Func, FuncType, IntoFunc, TypedFunc, WasmParams, WasmResults, WasmRet, WasmTy,
        WasmTyList,
    },
    global::Global,
    ...
    module::{
        CustomSection, CustomSectionsIter, ExportType, ImportType, Module, ModuleExportsIter,
        ModuleImportsIter, Read,
    },
