use crate::{
    collections::map::Iter as MapIter, AsContext, Func, FuncType, Global, GlobalType, Memory,
...
impl ExternType {
    /// Returns the underlying [`GlobalType`] or `None` if it is of a different
    /// type.
    ...
    /// Returns the underlying [`TableType`] or `None` if it is of a different
    /// type.
    ...
    /// Returns the underlying [`MemoryType`] or `None` if it is of a different
    /// type.
    ...
    /// Returns the underlying [`FuncType`] or `None` if it is of a different
    /// type.
...
/// [`Instance::exports`](crate::Instance::exports) method and describes what
/// names and items are exported from a Wasm [`Instance`](crate::Instance).
#[derive(Debug, Clone)]
...
impl<'instance> Export<'instance> {
    ...
    /// Returns the underlying [`Func`], if the [`Export`] is a function or
    /// `None` otherwise.
    pub fn into_func(self) -> Option<Func> {
        self.definition.into_func()
    }

    /// Returns the underlying [`Table`], if the [`Export`] is a table or `None`
    /// otherwise.
    pub fn into_table(self) -> Option<Table> {
        self.definition.into_table()
    }

    /// Returns the underlying [`Memory`], if the [`Export`] is a linear memory
    /// or `None` otherwise.
    pub fn into_memory(self) -> Option<Memory> {
        self.definition.into_memory()
    }

    /// Returns the underlying [`Global`], if the [`Export`] is a global
    /// variable or `None` otherwise.
    pub fn into_global(self) -> Option<Global> {
        self.definition.into_global()
    }
}

/// An iterator over the [`Extern`] declarations of an
