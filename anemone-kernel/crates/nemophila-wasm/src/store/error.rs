...
pub enum StoreError<E> {
    /// An error representing an internal error, a.k.a. a bug or invalid
    /// behavior within Wasmi.
...
/// A [`Store`] or [`StoreInner`] internal error, a.k.a. bug or invalid behavior
/// within Wasmi.
#[derive(Debug, Copy, Clone)]
...
impl InternalStoreError {
    ...
    /// An error indicating that a [`Store`] resource does not originate from
    /// the store.
    #[cold]
    #[inline]
    ...
    /// An error indicating that restoring a [`PrunedStore`] to a [`Store<T>`]
    /// mismatched `T`.
    #[cold]
    #[inline]
...
enum InternalStoreErrorKind {
    /// An error when restoring a [`PrunedStore`] with an incorrect `T` for
    /// [`Store<T>`].
    RestoreTypeMismatch(RestoreTypeMismatchError),
    /// An error indicating that a store resource does not originate from the
    /// given store.
    StoreMismatch,
...
/// Error occurred when restoring a [`PrunedStore`] to a [`Store<T>`] with an
/// mismatching `T`.
#[derive(Debug, Copy, Clone)]
