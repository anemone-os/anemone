...
use crate::{
    ...
    Engine, Error, Instance, Memory, ResourceLimiter,
};
use alloc::boxed::Box;
use core::{
...
pub struct Store<T> {
    ...
    /// The inner parts of the [`Store`] that are generic over a host provided
    /// `T`.
...
impl<T> Store<T> {
    ...
    /// Returns a shared reference to the user provided data owned by this
    /// [`Store`].
    pub fn data(&self) -> &T {
        &self.typed.data
    }

    /// Returns an exclusive reference to the user provided data owned by this
    /// [`Store`].
    ...
    /// Calls the given [`HostFuncEntity`] with the `params` and `results` on
    /// `instance`.
    ///
    /// # Errors
    ///
    ...
    /// Returns `true` if it is possible to create `additional` more instances
    /// in the [`Store`].
    ...
    /// Returns `true` if it is possible to create `additional` more linear
    ...
    /// Returns `true` if it is possible to create `additional` more tables in
    /// the [`Store`].
    ...
    /// # Note
    ///
    /// This methods mostly exists to satisfy certain use cases that otherwise
    /// would conflict with the borrow checker.
    ...
    /// Sets the remaining fuel of the [`Store`] to `value` if fuel metering is
    /// enabled.
    ///
    /// # Note
    ///
    ...
    /// Allocates a new [`TrampolineEntity`] and returns a [`Trampoline`]
    /// reference to it.
    ...
    /// given [`Memory`] and an exclusive reference to the user provided
    /// host state.
    ///
    /// # Note
    ///
...
pub enum CallHook {
    ...
    ReturningFromWasm,
    /// Indicates that a host function is being called from a WebAssembly
    /// function.
    CallingHost,
    /// Indicates that a host function called from a WebAssembly function is
    /// returning.
    ReturningFromHost,
}
