...
use crate::{
    ...
    CallHook, Error, Instance, Memory, Store, Table,
};
use core::{
    fmt::{self, Debug},
    mem,
...
impl<'a, T> From<&'a mut Store<T>> for &'a mut PrunedStore {
    ...
    fn from(store: &'a mut Store<T>) -> Self {
        ...
        //   a reference and since `Store<T>` and `Store<Pruned>` have the same size and
        //   API.
        // - We make sure in `PrunedStore` to never access the typed parts of the
        //   original `Store<T>` and check in the restoration process the type-ID of the
        //   target `T`.
