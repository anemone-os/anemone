use crate::{
    ...
    DataSegmentEntity, DataSegmentIdx, ElementSegment, ElementSegmentIdx, Engine, Error, Func,
    FuncEntity, FuncIdx, FuncType, Global, GlobalIdx, Instance, InstanceEntity, InstanceIdx,
    Memory, MemoryIdx, Table, TableIdx,
};
use core::{
    fmt::Debug,
...
impl StoreInner {
    ...
    /// Returns the remaining fuel of the [`StoreInner`] if fuel metering is
    /// enabled.
    ///
    /// # Note
    ///
    /// Enable fuel metering via
    ...
    /// Sets the remaining fuel of the [`StoreInner`] to `value` if fuel
    /// metering is enabled.
    ///
    /// # Note
    ///
    ...
    /// This way wrapped indices cannot be misused with incorrect [`StoreInner`]
    /// instances.
    ...
    /// Allocates a new [`DataSegmentEntity`] and returns a [`DataSegment`]
    /// reference to it.
    ...
    /// Allocates a new [`CoreElementSegment`] and returns a [`ElementSegment`]
    /// reference to it.
    ...
    /// Allocates a new [`ExternRefEntity`] and returns a [`ExternRef`]
    /// reference to it.
    ...
    /// Allocates a new uninitialized [`InstanceEntity`] and returns an
    /// [`Instance`] reference to it.
    ///
    /// # Note
    ///
    /// - This will create an uninitialized dummy [`InstanceEntity`] as a place
    ///   holder for the returned [`Instance`]. Using this uninitialized
    ///   [`Instance`] will result in a runtime panic.
    /// - The returned [`Instance`] must later be initialized via the
    ///   [`StoreInner::initialize_instance`] method. Afterwards the
    ...
    /// Calls `f` on the [`FuncType`] associated to the given [`DedupFuncType`]
    ...
    /// Returns a shared reference to the [`CoreGlobal`] associated to the given
    /// [`Global`].
    ///
    /// # Errors
    ///
    ...
    /// Returns an exclusive reference to the [`CoreGlobal`] associated to the
    /// given [`Global`].
    ///
    /// # Errors
    ///
    ...
    /// Returns a shared reference to the [`CoreTable`] associated to the given
    /// [`Table`].
    ///
    /// # Errors
    ///
    ...
    /// Returns an exclusive reference to the [`CoreTable`] associated to the
    /// given [`Table`].
    ///
    /// # Errors
    ///
    ...
    /// Returns an exclusive reference to the [`CoreTable`] and
    /// [`CoreElementSegment`] associated to `table` and `elem`.
    ///
    /// # Errors
    ///
    /// - If the [`Table`] does not originate from this [`StoreInner`].
    ...
    /// Returns both
    ///
    /// - an exclusive reference to the [`CoreTable`] associated to the given
    ///   [`Table`]
    ...
    ///
    /// - A shared reference to the [`InstanceEntity`] associated to the given
    ///   [`Instance`].
    /// - An exclusive reference to the [`CoreTable`] associated to the given
    ///   [`Table`].
    /// - A shared reference to the [`CoreElementSegment`] associated to the
    ///   given [`ElementSegment`].
    /// - An exclusive reference to the [`Fuel`] of the [`StoreInner`].
    ///
    /// # Note
    ///
    ...
    /// Returns a shared reference to the [`CoreElementSegment`] associated to
    /// the given [`ElementSegment`].
    ///
    /// # Errors
    ///
    /// - If the [`ElementSegment`] does not originate from this [`StoreInner`].
    ...
    /// Returns an exclusive reference to the [`CoreElementSegment`] associated
    /// to the given [`ElementSegment`].
    ///
    /// # Errors
    ///
    /// - If the [`ElementSegment`] does not originate from this [`StoreInner`].
    ...
    /// Returns a shared reference to the [`CoreMemory`] associated to the given
    /// [`Memory`].
    ///
    /// # Errors
    ///
    ...
    /// Returns an exclusive reference to the [`CoreMemory`] associated to the
    /// given [`Memory`].
    ///
    /// # Errors
    ///
    ...
    /// Returns the following data:
    ///
    /// - An exclusive reference to the [`CoreMemory`] associated to the given
    ///   [`Memory`].
    /// - A shared reference to the [`DataSegmentEntity`] associated to the
    ///   given [`DataSegment`].
    /// - An exclusive reference to the [`Fuel`] of the [`StoreInner`].
    ///
    /// # Note
    ...
    /// Returns an exclusive pair of references to the [`CoreMemory`] associated
    /// to the given [`Memory`]s.
    ///
    /// # Errors
    ///
    /// - If the [`Memory`] does not originate from this [`StoreInner`].
    ...
    /// Returns an exclusive reference to the [`DataSegmentEntity`] associated
    /// to the given [`DataSegment`].
    ///
    /// # Errors
    ///
    /// - If the [`DataSegment`] does not originate from this [`StoreInner`].
    ...
    /// Returns a shared reference to the [`InstanceEntity`] associated to the
    /// given [`Instance`].
    ///
    /// # Errors
    ///
    ...
    /// Returns a shared reference to the [`ExternRefEntity`] associated to the
    /// given [`ExternRef`].
    ///
    /// # Errors
    ///
    ...
    /// Allocates a new Wasm or host [`FuncEntity`] and returns a [`Func`]
    /// reference to it.
    ...
    /// Returns a shared reference to the associated entity of the Wasm or host
    /// function.
    ///
    /// # Errors
    ///
