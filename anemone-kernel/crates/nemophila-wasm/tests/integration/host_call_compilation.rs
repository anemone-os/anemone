//! This tests that a host function called from Wasm can compile Wasm modules
//! and does not deadlock.

use nemophila_wasm::{AsContextMut, Caller, Engine, Linker, Module, Store};

fn compile_module(engine: &Engine) -> nemophila_wasm::Module {
    let wasm = r#"
        (module
...
fn test_compile_in_host_call() {
    ...
    linker
        .func_wrap(
            "env",
            "compile",
            |mut caller: Caller<()>| -> Result<(), nemophila_wasm::Error> {
