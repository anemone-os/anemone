...
fn test_memory_does_not_grow_on_limited_growth() -> Result<(), Error> {
    ...
    // Second call goes past the limit, so fails to grow the memory, but returns
    // Ok(-1)
...
fn test_memory_traps_on_limited_growth() -> Result<(), Error> {
    ...
    // Check memory size is what we expect.
    assert_eq!(test.memory_size.call(&mut test.store, ())?, 3);
    // Second call goes past the limit, so fails to grow the memory, and we've
    // configured it to trap.
    assert!(matches!(
        test.memory_grow
...
fn test_table_does_not_grow_on_limited_growth() -> Result<(), Error> {
    ...
    // Second call goes past the limit, so fails to grow the table, but returns
    // Ok(-1)
