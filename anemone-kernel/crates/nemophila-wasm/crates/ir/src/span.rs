...
/// - For the sake of space efficiency the actual number of [`Slot`] of the
///   [`SlotSpan`] is stored externally and provided in [`SlotSpan::iter`] when
///   there is a need to iterate over the [`Slot`] of the [`SlotSpan`].
///
...
impl SlotSpanIter {
    /// Creates a [`SlotSpanIter`] from then given raw `start` and `end`
    /// [`Slot`].
    ...
    /// Creates a new [`SlotSpanIter`] for the given `start` [`Slot`] and length
    /// `len`.
    ///
    /// # Panics
    ///
    ...
    /// Returns the remaining number of [`Slot`]s yielded by the
    /// [`SlotSpanIter`].
