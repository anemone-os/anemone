...
pub trait FuelCosts {
    ...
    /// Returns the amount of bytes that can be copied for a single unit of
    /// fuel.
...
impl FuelCostsProvider {
    /// Applies `f` to either `self.custom` or [`DefaultFuelCosts`] if
    /// `self.custom` is `None`.
    ...
    ///
    /// - On overflow this returns [`u64::MAX`].
    /// - [`UntypedVal`] might be 64-bit or 128-bit depending on the crate's
    ///   config.
    /// - The following Wasmi IR instructions may make use of this:
...
impl Fuel {
    ...
    /// - This API is intended for use cases where it is clear that fuel
    ///   metering is enabled and where a check would incur unnecessary overhead
    ///   in a hot path. An example of this is the execution of consume fuel
    ///   instructions since those only exist if fuel metering is enabled.
    ///
    /// # Errors
    ///
    /// If out of fuel.
