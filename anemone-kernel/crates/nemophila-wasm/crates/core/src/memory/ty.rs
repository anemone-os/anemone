...
impl MemoryTypeInner {
    ...
    /// maximum value is returned dependent on the index type of the memory
    /// type.
    ///
    /// # Errors
    ///
    ...
    /// Returns the maximum size in bytes allowed by the `index_type` of this
    /// memory type.
    ///
    /// # Note
    ///
    ...
    /// Returns the absolute maximum size in pages that a linear memory is
    /// allowed to have.
...
///
/// Constructed via [`MemoryType::builder`] or via
/// [`MemoryTypeBuilder::default`]. Allows to incrementally build-up a
/// [`MemoryType`]. When done, finalize creation via a call to
/// [`MemoryTypeBuilder::build`].
...
impl MemoryTypeBuilder {
    ...
    /// Sets the optional maximum number of pages the built [`MemoryType`]
    /// supports.
    ///
    ...
    ///
    /// The default value is 16, which results in the default Wasm page size of
    /// 64KiB (aka 2^16 or 65536).
    ///
    /// Currently, the only allowed values are 0 (page size of 1) or 16 (the
    /// default). Future Wasm proposal extensions might change this
    /// limitation.
    ///
    ...
    /// # Errors
    ///
    /// If the chosen configuration for the constructed [`MemoryType`] is
    /// invalid.
...
impl MemoryType {
    ...
    /// Returns a [`MemoryTypeBuilder`] to incrementally construct a
    /// [`MemoryType`].
    ...
    /// Returns `true` if the [`MemoryType`] is a subtype of the `other`
    /// [`MemoryType`].
    ///
    /// # Note
    ///
