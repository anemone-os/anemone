...
pub struct ByteBuffer {
    ...
    ///
    /// - **Vec:** `vec.len()`
    /// - **Static:** The accessible subslice of the entire underlying static
    ///   byte buffer.
    pub(super) len: usize,
    ...
    capacity: usize,
    /// Whether the [`ByteBuffer`] was initialized from a `&'static [u8]` or a
    /// `Vec<u8>`.
    is_static: bool,
}
...
impl ByteBuffer {
    ...
    ///     - `static`: If `new_size` is larger than it's the static buffer
    ///       capacity.
    ...
    /// Grow the byte buffer to the given `new_size` when backed by a `&'static
    /// [u8]`.
    ...
    /// Returns the underlying `Vec<u8>` if the byte buffer is not backed by a
    /// static buffer.
    ///
    ...
    /// # Note
    ///
    /// - The returned `Vec` will free its memory and thus the memory of the
    ///   [`ByteBuffer`] if dropped.
    /// - The returned `Vec` is returned as [`ManuallyDrop`] to prevent its
    ///   buffer from being freed automatically upon going out of scope.
    fn get_vec(&mut self) -> Option<ManuallyDrop<Vec<u8>>> {
        ...
        // - The returned `Vec` is returned as [`ManuallyDrop`] to prevent its buffer
        //   from being free upon going out of scope.
...
mod test {
    ...
    fn test_basic_data_manipulation() {
        ...
        assert_eq!(data, &[0, 0, 0, 0, 4, 0, 0, 0, 0, 0]); // test we can read
                                                           // the data
                                                           // test drop is okay
    }
