...
impl Memory {
    ...
    /// # Errors
    ///
    /// If creation of the linear memory fails or is disallowed by the
    /// `limiter`.
    pub fn new(
        memory_type: MemoryType,
    ...
    /// # Note
    ///
    /// This uses `buffer` to store its bytes and won't perform heap
    /// allocations.
    ///
    /// # Errors
    ///
    ...
    pub fn grow(
        ...
        //
        // This is deliberately done right before the actual growth operation in order
        // to not charge fuel if there is any other deterministic failure
        // preventing the expensive growth operation.
    ...
    /// Returns the base pointer, in the host’s address space, that the
