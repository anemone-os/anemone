...
impl Table {
    ...
    /// # Panics
    ///
    /// If the [`ElementSegment`] element type does not match the [`Table`]
    /// element type. Note: This is a panic instead of an error since it is
    /// asserted at Wasm validation time.
    pub fn init(
        &mut self,
