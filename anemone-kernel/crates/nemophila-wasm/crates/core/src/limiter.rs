...
pub trait ResourceLimiter {
    ...
    /// is allowed. This means that the wasm `memory.grow` or `table.grow`
    /// instructions will return with the `desired` size, in wasm pages.
    /// Note that even if `Ok(true)` is returned, though, if `desired`
    /// exceeds `maximum` then the growth operation will still fail.
    ///
    ...
    /// will behave as if a trap has been raised. Note that this is not
    /// necessarily compliant with the WebAssembly specification but it can
    /// be a handy and useful tool to get a precise backtrace at "what
    /// requested so much memory to cause a growth failure?".
    ///
    ...
    /// # Errors
    ///
    /// See the details on the return values for
    /// [`ResourceLimiter::memory_growing`] for what the return values of
    ...
    /// The maximum number of linear memories that can be created for a Wasm
    /// store.
    ///
...
impl ResourceLimiterRef<'_> {
    /// Returns an exclusive reference to the underlying [`ResourceLimiter`] if
    /// any.
