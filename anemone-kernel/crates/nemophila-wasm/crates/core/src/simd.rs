...
/// Internal helper trait to help the type inference to do its jobs with fewer
/// type annotations.
///
/// # Note
///
...
trait Lanes {
    ...
    ///
    /// Storing [`Self::ALL_ONES`] if `f` evaluates to `true` or
    /// [`Self::ALL_ZEROS`] otherwise per item.
    fn lanewise_comparison(self, other: Self, f: impl Fn(Self::Item, Self::Item) -> bool) -> Self;

    /// Apply `f(i, n, acc)` for all lane items `i` at pos `n` in `self` and
    /// return the result.
...
///
/// For example a `i64x2` vector can be constructed from the two lower lanes of
/// a `i32x4`.
trait FromNarrow<NarrowLanes: Lanes>: Lanes {
    /// Construct `Self` from the pairwise application of `f` of items in
    /// `narrow`.
    fn pairwise_unary(
        narrow: NarrowLanes,
    ...
    /// Construct `Self` from the pairwise application of `f` of items in `lhs`
    /// and `rhs`.
    fn pairwise_binary(
        lhs: NarrowLanes,
        rhs: NarrowLanes,
    ...
    /// Construct `Self` from the application of `f` to the lower half lanes of
    /// `narrow`.
    fn low_unary(narrow: NarrowLanes, f: impl Fn(NarrowLanes::Item) -> Self::Item) -> Self;

    /// Construct `Self` from the application of `f` to the higher half lanes of
    /// `narrow`.
    fn high_unary(narrow: NarrowLanes, f: impl Fn(NarrowLanes::Item) -> Self::Item) -> Self;

    /// Construct `Self` from the binary application of `f` to the lower half
    /// lanes of `narrow_lhs` and `narrow_rhs`.
    fn low_binary(
    ...
    /// Construct `Self` from the binary application of `f` to the higher half
    /// lanes of `narrow_lhs` and `narrow_rhs`.
    fn high_binary(
        narrow_lhs: NarrowLanes,
        narrow_rhs: NarrowLanes,
...
trait FromWide<WideLanes: Lanes>: Lanes {
    /// Construct `Self` from the application of `f` to the wide `low` and
    /// `high` items.
    fn from_low_high(
        low: WideLanes,
        high: WideLanes,
    ...
    /// Construct `Self` from the application of `f` to the wide `low` or
    /// evaluate `high`.
    fn from_low_or(
        low: WideLanes,
...
impl V128 {
    ...
    /// Convenience method to help implement lanewise reduce methods with a
    /// loop-index.
...
impl V128 {
    /// Interprets `bits` as array of `Narrow` and distribute the (sign)
    /// extended items as [`V128`].
