...
/// # Note
///
/// This has saturating semantics for when the integer cannot represent the
/// float.
///
/// Returns
///
...
/// Extension trait for `f32` and `f64` to turn a NaN value into a quiet-NaN
/// value.
#[cfg(feature = "std")]
trait IntoQuietNan: Sized {
    /// Converts `self` into a quiet-NaN if `self` is a NaN, otherwise returns
    /// `None`.
...
mod tests {
    ...
    fn copysign_regression_works() {
        // This test has been directly extracted from a WebAssembly Specification
        // assertion.
