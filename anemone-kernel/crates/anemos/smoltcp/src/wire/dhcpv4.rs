...
use crate::wire::{EthernetAddress, Ipv4Address, arp::Hardware};
...
pub struct Repr<'a> {
    ...
    /// The DHCP IP renew duration (T1 interval), in seconds, if specified in
    /// the packet.
    pub renew_duration: Option<u32>,
    /// The DHCP IP rebind duration (T2 interval), in seconds, if specified in
    /// the packet.
