...
/// A wrapper around [`VirtQueue`] that owns all the buffers that are passed to
/// the queue.
#[derive(Debug)]
...
impl<H: Hal, const SIZE: usize, const BUFFER_SIZE: usize> OwningQueue<H, SIZE, BUFFER_SIZE> {
    ...
    ///
    /// This will allocate `SIZE` buffers of `BUFFER_SIZE` bytes each and add
    /// them to the queue.
    ///
    /// The caller is responsible for notifying the device if `should_notify`
    /// returns true.
    pub fn new(mut queue: VirtQueue<H, SIZE>) -> Result<Self> {
        ...
        for (i, queue_buffer) in buffers.iter_mut().enumerate() {
            ...
            // SAFETY: The buffer lives as long as the queue, as specified in the function
    ...
    /// # Safety
    ///
    /// The buffer must not currently be in the RX queue, and no other
    /// references to it must exist between when this method is called and
    /// when it is popped from the queue.
    unsafe fn add_buffer_to_queue(&mut self, index: u16, transport: &mut impl Transport) -> Result {
        // SAFETY: The buffer lives as long as the queue, and the caller guarantees that
        // it's not currently in the queue or referred to anywhere else until it
        // is popped.
        unsafe {
            let buffer = self
                .buffers
            ...
            // If the RX buffer somehow gets assigned a different token, then our safety
            // assumptions are broken and we can't safely continue to do
    ...
    fn pop(&mut self) -> Result<Option<(&[u8], u16)>> {
        ...
        // SAFETY: The device has told us it has finished using the buffer, and there
        // are no other references to it.
        let buffer = unsafe { self.buffers[usize::from(token)].as_mut() };
        // SAFETY: We maintain a consistent mapping of tokens to buffers, so we pass the
        // same buffer to `pop_used` as we previously passed to `add` for the
        // token. Once we add the buffer back to the RX queue then we don't
        // access it again until next time it is popped.
    ...
    /// Checks whether there are any buffers which the device has marked as used
    /// so the driver should now process. If so, passes the first one to the
    /// `handle` function and then adds it back to the queue.
    ///
    /// Returns an error if there is an error accessing the queue or `handler`
    /// returns an error. Returns `Ok(None)` if there are no pending buffers
    /// to handle, or if `handler` returns `Ok(None)`.
    ///
    /// If `handler` panics then the buffer will not be added back to the queue,
    ...
    pub fn poll<T>(
        ...
        // SAFETY: The buffer was just popped from the queue so it's not in it, and
        // there won't be any other references until next time it's popped.
        unsafe {
    ...
    for OwningQueue<H, SIZE, BUFFER_SIZE>
{
    fn drop(&mut self) {
        for buffer in self.buffers {
            // SAFETY: We obtained the buffer pointer from `Box::into_raw`, and it won't be
            // used anywhere else after the queue is destroyed.
