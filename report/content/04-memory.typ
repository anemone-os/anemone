...
Anemone对于OOM具备一定的防御能力。在内核启动时，我们会创建OOM Killer进程。一旦内核的使用内存的水位高于一个可配置的特定值，OOM Killer内核线程将被唤醒，它会寻找系统中占用内存最多的进程，并将其杀死，以释放内存资源。OOM Killer会根据进程的内存使用情况、优先级等因素，选择最合适的进程进行终止，从而保证系统的稳定性和可用性。

OOM Killer将在内存水位安全后继续睡眠，这样，寻常时刻它不会被调度，从而避免了占用CPU空转，消耗系统资源。
