...
Anemone是一个多核、支持多架构的类Unix操作系统内核，目标是*在不引入大量历史包袱和污染内部实现的情况下，尽可能兼容Linux UAPI，从而支持现有的用户态应用程序和工具链*。我们从两个层面建设系统的灵活性：Kconfig、系统目标和平台配置负责构建期的精确组合，Nemophila则允许经过授权的WebAssembly模块在运行期间装载、接入内核观测点并卸载。配合丰富的内核参数和调试接口，这套设计既方便用户定制系统，也使我们能够用同一套主体代码持续适配新的架构与平台。目前，Anemone已经支持RISC-V64和LoongArch64两种架构，并适配了对应的QEMU虚拟平台。

== Anemone 整体架构

Anemone分成三层：最底层是架构与平台接入层，负责内核自举、trap/中断、上下文保存、时钟中断、机器适配、平台设备发现等；接着是基础设施层，这里包括内核的各个核心子系统：进程管理、调度、时间、内存管理、文件系统、网络、IPC、设备驱动模型等；最后是面向用户的系统调用层，在这里，我们兼容Linux的UAPI，并将其映射到Anemone自己的内部对象，避免Linux的语义侵入我们的内核，从而造成污染。Nemophila作为贯穿这些层次的受管理扩展能力，由各子系统主动提供观测位置，同时保持原有状态与行为仍由子系统自己掌握。
...
#code-block(
  ...
  └── crates/
      ...
      ├── nemophila-wasm          # 内核内 WebAssembly 执行引擎
