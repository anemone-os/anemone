...
我们还设计并实现了动态内核扩展框架 Nemophila。它以 WebAssembly 作为跨架构模块载体，通过受约束的宿主接口和 Weave 动态观测机制，让经过授权的扩展能够在运行期间装载、接入内核路径并安全卸载。同一份模块制品可以运行在 RISC-V64 与 LoongArch64 内核中，使 Anemone 的可扩展性从构建期配置进一步延伸到运行期演化。

Anemone 的开发全程独立自主。在架构上，我们抛弃了历史包袱，完全从零设计。我们编写了基于侵入式结构的 buddy 内存分配器以解决页分配器对堆反向依赖的问题，以及同样基于侵入式结构的、对堆无依赖的设备树解析库以便内核早期使用，以及LoongArch64 硬件支持库等，它们都独立于且被抽离了内核，形成单独的 crate，经过了 Miri、cargo-fuzz 等工具的验证，我们期望这些 crate 能为未来的开源社区贡献更多的基础设施。
...
#figure(
  table(
    ...
    [*模块*], [*完成情况*],
    [Nemophila],
    [实现基于 WebAssembly 的动态模块装载、Weave 观测、故障隔离与卸载闭环，提供 Rust SDK，并完成双架构同一制品验证。],
