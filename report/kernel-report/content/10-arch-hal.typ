...
#report-figure(
  image("../assets/hal.png", width: 90%),
  caption: [Anemone定义了相当多的组件化的硬件抽象层],
)
...
进入决赛阶段后，这套启动边界同时覆盖了 QEMU virt、VisionFive 2 和 Loongson 2K1000 等平台。平台差异主要集中在早期页表、中断控制器、CPU 唤醒、计时源和设备树交接；进入通用初始化后，调度、VFS、网络和用户程序启动继续走同一条路径。我们因此能够在扩展真实硬件时复用大部分内核，而不是维护两套彼此分叉的系统。
...
系统调用需要频繁访问用户指针。早期实现会先检查页表，再通过内核指针读写用户内存；但“检查时有效”并不代表真正访问时仍然不会缺页或被并发修改。决赛阶段，我们改为由真实的 load/store 决定访问结果：RISC-V64 与 LoongArch64 分别提供短小的 copy 入口和异常恢复位置，只有这些明确标记的访问发生 fault 时，内核才把它转换成 `EFAULT` 或解析一次合法缺页。

通用的 `UserReadPtr`、`UserWritePtr` 和 slice 接口建立在这套架构能力之上。文件读写可以保留 fault 前已经完成的前缀，要求事务完整性的系统调用也可以先 fault-in 输出区域。这种设计既更贴近硬件真实行为，也让两个架构共享相同的 Linux 用户内存语义，是 glibc、编译器和复杂系统调用能够稳定运行的重要基础。

== 架构专属能力

共享 HAL 并不意味着抹平 ISA 的特色。RISC-V64 实现了 `riscv_hwprobe` 与 `riscv_flush_icache`，让用户态可以查询可用的基础指令扩展，并在生成或修改代码后同步指令缓存；全局 `membarrier` 则利用多核 IPI 为运行时提供内存顺序保证。

LoongArch64 侧，我们补齐了 128-bit LSX 用户上下文。任务第一次使用 LSX 时，内核为它启用完整向量寄存器保存，此后的 syscall、中断、调度切换、fork/exec 和 signal frame 都能正确维持这份上下文。这项能力对现代编译器尤其重要：GCC 等程序会主动使用向量指令，缺少完整上下文切换时往往不是立即报错，而是以难以复现的数据损坏表现出来。

这些能力仍然遵循同一个原则：通用子系统定义自己需要的语义，架构层负责用本 ISA 的寄存器、异常和指令实现它。由此，Anemone 既能保持双架构主体共享，也能让每个架构拥有真正可用而不是只有启动代码的专属支持。
