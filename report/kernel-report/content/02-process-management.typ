...
等待路径对应 `wait4` / `waitid` 等接口。父进程可以等待任意子进程、指定 TGID 的子进程，或指定进程组内的子进程。除了回收已经退出的 zombie，决赛阶段的实现还可以报告子进程的停止和继续状态，并通过 `wait4` 返回资源使用量。这样，shell 既能等待普通前台命令结束，也能跟踪被挂起或重新恢复的作业。

这些基本动作，就构成了Anemone进程管理的核心生命周期路径。实际上这也是类Unix内核的通用做法。

== Unix 作业控制

进入决赛阶段后，我们在原有进程组、会话和信号机制之上补齐了 Unix 作业控制主路径。一个交互式 shell 可以把一组相关进程放进同一个 process group，将其中一个组设为终端前台作业，并通过 `SIGTSTP`、`SIGSTOP`、`SIGCONT` 等信号停止或恢复整个作业。父进程再通过 `wait4` / `waitid` 获得 stopped、continued 和 exited 状态，从而实现 `jobs`、`fg`、`bg`、Ctrl-C 和 Ctrl-Z 等常见交互。

这项能力看起来像是若干系统调用和信号的组合，实际上贯穿了进程拓扑、调度和终端三个子系统。我们让线程组保存作业整体的运行、停止和继续状态，调度器负责确保停止后的线程不再返回用户态，TTY 则只负责判断前后台关系并生成相应信号。这样，一个多线程进程仍然作为一个完整作业被观察，而不会出现某些线程已经停止、另一些线程却继续执行的混乱状态。

作业控制也是 Anemone 从“能启动程序”走向“能运行交互式 Unix 用户环境”的重要一步。BusyBox `ash` 可以使用真实 controlling terminal 管理前台进程组，终端输入产生的信号会沿 process group 正确投递，父进程也能获得后续状态变化。这套机制没有为 shell 编写特判，而是由通用的 topology、signal、wait 和 TTY 能力共同完成。
