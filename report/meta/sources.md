...
这些材料主要用于核对事实、确认完成情况和选择代表性例子。报告正文中优先使用机制、
模块名、文档标题或稳定的公开概念来表达；除非必须精确定位，不要把具体文件路径、
函数名、字段名或脆弱行号写成正文主体。
...
使用代码作为来源时，正文默认不写源码路径。只有核心结构体展示、关键代码片段、
代表性 syscall/ABI、公开接口名或具体调试案例需要精确定位时，才写出具体标识符。
普通机制说明应转写为“资源如何继承”“生命周期如何推进”“错误码如何选择”“锁或
拓扑如何保证一致性”等评委可读的表达。
