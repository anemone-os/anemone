...
impl Cmd {
    ...
    /// Echo the command to standard error.
    ///
    /// Most methods [Cmd] provides won't echo the command, which is sometimes useful for debugging.
    /// This method allows echoing the command when needed.
    pub fn echo(self) -> Cmd {
        eprintln!("$ {}", self);
        self
    }
