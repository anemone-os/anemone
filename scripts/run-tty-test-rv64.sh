...
readonly runtime_dir=build/runtime/tty-acceptance-rv64
readonly sdcard_target=$runtime_dir/disk-x0.img
...
for runtime_parent in build build/runtime "$runtime_dir"; do
    [[ ! -L $runtime_parent ]] || fail "runtime directory component must not be a symlink: $runtime_parent"
done

mkdir -p -- "$staging_dir" "$(dirname -- "$log_file")" "$runtime_dir"
...
[[ $sdcard_source != "$sdcard_destination" ]] \
    || fail "sdcard source and runtime destination must be different files"
...
inputs = {
    ...
    b"@@TTY READY fionread-canonical-pending@@": b"abc",
    b"@@TTY READY fionread-canonical-commit@@": b"\nsecond\n",
    b"@@TTY READY fionread-empty-eof@@": b"\x04",
    b"@@TTY READY fionread-raw@@": b"raw",
    ...
    b"@@TTY READY tcflush-canonical-pending@@": b"drop",
    b"@@TTY READY tcflush-canonical-after-pending@@": b"keep\n",
    b"@@TTY READY tcflush-canonical-committed@@": b"drop\n",
    b"@@TTY READY tcflush-raw@@": b"old",
    b"@@TTY READY tcflush-raw-after@@": b"new",
