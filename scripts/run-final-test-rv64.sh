...
usage() {
    cat <<'EOF'
Usage: run-final-test-rv64.sh <disk-image> [log-file]

Builds an rv64 kernel with the fixed embedded BusyBox shell, stages a writable
...
disk_image=$1
log_file=${2:-build/final-test-rv64.log}
runtime_dir=build/runtime/final-rv64
disk_target=$runtime_dir/disk-x0.img
selection=(
    --target qemu-virt-rv64-final
...
log_progress "FINAL" "target qemu-virt-rv64-final (riscv64)"
