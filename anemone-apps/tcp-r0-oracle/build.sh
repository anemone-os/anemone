...
if [[ "$ANEMONE_ARCH" == host ]]; then
    ...
    printf 'TCP-R0-BUILD:family=host:compiler=%s\n' "$compiler_path"
    "$host_compiler" --version | head -n 1
    "$host_compiler" -O2 -std=c11 -Wall -Wextra -Werror -pthread main.c -o "$output_dir/tcp-r0-host"
    file "$output_dir/tcp-r0-host"
    sha256sum "$output_dir/tcp-r0-host"
    exit 0
fi
...
build_oracle() {
    ...
    printf 'TCP-R0-BUILD:family=%s:compiler=%s\n' "$family" "$compiler_path"
    "$compiler" --version | head -n 1
    printf 'TCP-R0-BUILD:family=%s:sysroot=%s\n' "$family" "$sysroot"
    printf 'TCP-R0-BUILD:family=%s:libc=%s\n' "$family" "$libc_archive"
    sha256sum "$libc_archive"
    "$compiler" -static -O2 -std=c11 -Wall -Wextra -Werror -pthread main.c -o "$output"
    file "$output"
    sha256sum "$output"
}

build_oracle glibc "$glibc_compiler" "$output_dir/tcp-r0-glibc"
build_oracle musl "$musl_compiler" "$output_dir/tcp-r0-musl"
