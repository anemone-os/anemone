...
use anemone_rs::{
    abi::{
        fs::linux::{
            ...
            open::{O_CREAT, O_RDONLY, O_TRUNC, O_WRONLY},
        },
        process::linux::{
    ...
    os::linux::{
        fs::{
            AtFd, Fd, PipeFlags, close, dup3, fstat, fstatat, openat, pipe2, read, unlinkat, write,
        },
        process::{
...
const TESTS: &[(&str, TestFn)] = &[
    ...
    (
        "emfile-precedes-open-side-effects",
        emfile_precedes_open_side_effects,
    ),
...
fn emfile_precedes_open_side_effects_child() -> Result<(), Errno> {
    const CREATE_PATH: &str = "/rlimit-emfile-create";
    const TRUNCATE_PATH: &str = "/rlimit-emfile-truncate";
    const CONTENT: &[u8] = b"preserve-me";

    let create_path = Path::new(CREATE_PATH);
    let truncate_path = Path::new(TRUNCATE_PATH);
    let _ = unlinkat(AtFd::Cwd, create_path, 0);
    let _ = unlinkat(AtFd::Cwd, truncate_path, 0);
    ...
    let result = (|| {
        ...
        expect(write(fd, CONTENT)? == CONTENT.len())?;
        close(fd)?;
        ...
        expect_errno(
            openat(AtFd::Cwd, create_path, O_CREAT | O_WRONLY, 0o600),
            EMFILE,
        )?;
        expect_errno(
            openat(AtFd::Cwd, truncate_path, O_TRUNC | O_WRONLY, 0),
            EMFILE,
        )?;

        close_all(opened)?;
        set_limit(initial)?;
        expect_errno(fstatat(AtFd::Cwd, create_path), ENOENT)?;
        expect(fstatat(AtFd::Cwd, truncate_path)?.st_size == CONTENT.len() as i64)?;

        let fd = openat(AtFd::Cwd, truncate_path, O_RDONLY, 0)?;
        let mut content = [0u8; CONTENT.len()];
        let read_result = read(fd, &mut content);
        let close_result = close(fd);
        expect(read_result? == CONTENT.len() && content == CONTENT)?;
        close_result
    })();

    let _ = set_limit(initial);
    let _ = unlinkat(AtFd::Cwd, create_path, 0);
    let _ = unlinkat(AtFd::Cwd, truncate_path, 0);
    result
}

fn emfile_precedes_open_side_effects() -> Result<(), Errno> {
    run_in_child(emfile_precedes_open_side_effects_child)
}
